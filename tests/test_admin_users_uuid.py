...
def test_admin_users_uuid_crud(client, db_session):
    ...
    role_patch_resp = client.patch(
        f"/api/admin/users/{user.id}",
        headers=headers,
        json={"platform_role_id": editor_role.id},
    )
    assert role_patch_resp.status_code == 200
    role_patch_data = role_patch_resp.json()
    assert role_patch_data["platform_role_id"] == editor_role.id
    assert role_patch_data["role_id"] == editor_role.id

    alias_role_resp = client.patch(
        f"/api/admin/users/{user.id}/role",
        headers=headers,
        params={"platform_role_id": editor_role.id},
    )
    assert alias_role_resp.status_code == 200
    alias_role_data = alias_role_resp.json()
    assert alias_role_data["platform_role_id"] == editor_role.id
    assert alias_role_data["user"]["platform_role_id"] == editor_role.id
