...
class TestAutomationsFlows:
    def test_create_flow(self, full):
        assert _ok(
            full["c"]
            .post(
                "/api/crm/automations/flows", json={"name": "Test", "trigger_type": "stage_change"}, headers=full["h"]
            )
            .status_code
        )

    def test_validate_path(self, full):
        assert _ok(full["c"].post("/api/crm/automations/flows/validate-path", json={}, headers=full["h"]).status_code)
...
class TestKanbanDragDrop:
    def test_not_found(self, full):
        assert (
            full["c"]
            .post(
                "/api/crm/pipeline/kanban/drag-drop/events",
                json={
...
class TestAutomationsValidate:
    def test_graph(self, full):
        assert _ok(
            full["c"]
            .post(
                "/api/crm/automations/validate-graph",
    ...
    def test_validate(self, full):
        assert _ok(
            full["c"]
            .post(
                "/api/crm/automations/flows/validate",
                json={"flow_data": {"nodes": [{"id": "a"}], "edges": []}},
                headers=full["h"],
            )
            .status_code
    ...
    def test_node(self, full):
        assert _ok(
            full["c"]
            .post("/api/crm/automations/flows/validate-node", json={"node": {"id": "a"}}, headers=full["h"])
            .status_code
        )
    ...
    def test_traverse(self, full):
        assert _ok(
            full["c"]
            .post(
                "/api/crm/automations/branching/traverse",
...
class TestKanbanFilterExtended:
    ...
    def test_filter_by_priority(self, full, db_session):
        ...
        db_session.commit()
        resp = full["c"].get(
            f"/api/crm/pipeline/kanban/filter?priority={PrioridadCasoEnum.ALTA.value}", headers=full["h"]
        )
        assert resp.status_code == 200
    ...
    def test_filter_by_status(self, full, db_session):
        ...
        db_session.commit()
        resp = full["c"].get(
            f"/api/crm/pipeline/kanban/filter?status={EstadoCasoEnum.ABIERTO.value}", headers=full["h"]
        )
        assert resp.status_code == 200
...
class TestGetGraphEdgeSource:
    def test_edge_source_id_field(self):
        ...
        nodes, edges = get_graph_from_payload_or_db(
            {"flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source_id": "a", "target_id": "b"}]}},
            None,
        )
        assert len(edges) == 1
...
class TestValidatePathExtended:
    def test_validate_path_short(self, full):
        resp = full["c"].post(
            ...
            json={"flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source": "a", "target": "b"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
...
class TestFlowsExtended:
    ...
    def test_max_nodes_ok(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/max-nodes-check",
            json={"flow_data": {"nodes": [{"id": "a"}], "edges": []}},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_validate_types(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-types", json={"nodes": [{"type": "new_persona"}]}, headers=full["h"]
        )
        assert resp.status_code == 200

    def test_validate_types_invalid(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-types", json={"nodes": [{"type": "bad_type"}]}, headers=full["h"]
        )
...
class TestKanbanStageExtended:
    ...
    def test_limit_cases_with_stage(self, full, db_session):
        ...
        db_session.commit()
        resp = full["c"].get(
            f"/api/crm/pipeline/kanban/stage/limit-cases?stage_id={etapa.id}&limit=50&offset=0", headers=full["h"]
        )
        assert resp.status_code == 200
...
class TestDragDropOther:
    def test_same_stage_not_found(self, full):
        resp = full["c"].post(
            "/api/crm/pipeline/kanban/drag-drop/same-stage",
    ...
    def test_invalid_stage_not_found(self, full):
        resp = full["c"].post(
            "/api/crm/pipeline/kanban/drag-drop/invalid-stage",
    ...
    def test_missing_id(self, full):
        resp = full["c"].post("/api/crm/pipeline/kanban/drag-drop/missing-id", json={}, headers=full["h"])
        assert resp.status_code == 400

    def test_missing_id_with_value(self, full):
        resp = full["c"].post(
            "/api/crm/pipeline/kanban/drag-drop/missing-id", json={"caso_id": str(uuid.uuid4())}, headers=full["h"]
        )
        assert resp.status_code == 200
    ...
    def test_recovery(self, full):
        resp = full["c"].post("/api/crm/pipeline/kanban/drag-drop/recovery", json={}, headers=full["h"])
        assert resp.status_code == 200

    def test_recovery_invalid_uuid(self, full):
        resp = full["c"].post(
            "/api/crm/pipeline/kanban/drag-drop/recovery", json={"caso_id": "not-a-uuid"}, headers=full["h"]
        )
        assert resp.status_code == 400
...
class TestFlowsMoreValidation:
    def test_validate_path_length(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-path-length", json={"nodes_count": 5}, headers=full["h"]
        )
        assert resp.status_code == 200

    def test_validate_path_length_short(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-path-length", json={"nodes_count": 1}, headers=full["h"]
        )
        assert resp.status_code == 200
        assert resp.json()["valid"] is False
    ...
    def test_validate_multiple_inputs(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-multiple-inputs",
            json={"flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source": "a", "target": "b"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_validate_multiple_outputs(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-multiple-outputs",
            json={"flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source": "a", "target": "b"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_clean_orphans(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/clean-orphans",
            json={"flow_data": {"nodes": [{"id": "a"}], "edges": [{"source": "a", "target": "b"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_validate_complex_dag(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-complex-dag",
            json={"flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source": "a", "target": "b"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_branching_null_vars(self, full):
        resp = full["c"].post(
            "/api/crm/automations/branching/null-vars", json={"variables": {"a": None, "b": "hello"}}, headers=full["h"]
        )
        assert resp.status_code == 200
    ...
    def test_branching_missing_else_ok(self, full):
        resp = full["c"].post(
            ...
            json={
                "node_id": "a",
                "edges": [
                    {"source": "a", "source_port": "true", "target": "b"},
    ...
    def test_branching_unexpected_op(self, full):
        resp = full["c"].post(
            ...
            json={"conditions": [{"operator": "bad_op"}]},
            headers=full["h"],
        )
        assert resp.status_code == 400
    ...
    def test_branching_unexpected_op_ok(self, full):
        resp = full["c"].post(
            "/api/crm/automations/branching/unexpected-op",
            json={"conditions": [{"operator": "equals"}]},
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_validate_node(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-node",
            json={
                "node_id": "a",
                "flow_data": {"nodes": [{"id": "a"}, {"id": "b"}], "edges": [{"source": "a", "target": "b"}]},
            },
            headers=full["h"],
        )
        assert resp.status_code == 200
    ...
    def test_validate_node_self_ref(self, full):
        resp = full["c"].post(
            "/api/crm/automations/flows/validate-node",
            json={"node_id": "a", "flow_data": {"nodes": [{"id": "a"}], "edges": [{"source": "a", "target": "a"}]}},
            headers=full["h"],
        )
        assert resp.status_code == 200
        assert resp.json()["valid"] is False
