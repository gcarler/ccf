...
from backend.models_crm_pipeline import CasoCRM, PipelineCRM, EtapaPipeline
...
def test_atomic_sort_reorder_target_stage_sede_isolation(db_session):
    ...
    pipeline_a = PipelineCRM(
        sede_id=sede_a.id,
        nombre="Pipeline Sede A",
        tipo="NUEVOS_VISITANTES"
    )
    pipeline_b = PipelineCRM(
        sede_id=sede_b.id,
        nombre="Pipeline Sede B",
        tipo="CONSEJERIA"
    )
    ...
    # Create stages
    stage_a = EtapaPipeline(
        pipeline_id=pipeline_a.id,
        nombre="Stage A",
        orden=1
    )
    stage_b = EtapaPipeline(
    ...
    payload = [
        {
            "id": str(case_a.id),
            "target_stage_id": str(stage_b.id)
        }
    ]
...
def test_automation_engine_operators(db_session):
    ...
    def check_condition(cond_type, cond_key, cond_val, should_match):
        ...
        root = CrmAutomation(name="Root", trigger_event="trigger", action_type="send_email", delay_minutes=0, is_active=True)
        target = CrmAutomation(name="Target", trigger_event="trigger", action_type="send_email", delay_minutes=10, is_active=True)
        ...
        action = PendingCrmAction(
            ...
            execute_at=models_shared_utcnow(),
            status="pending"
        )
        ...
        queued = db_session.query(PendingCrmAction).filter(
            PendingCrmAction.automation_id == target.id,
            PendingCrmAction.target_persona_id == persona_id
        ).first()
        
        is_matched = queued is not None
        assert is_matched == should_match, f"Condition type '{cond_type}' with key '{cond_key}' and val '{cond_val}' expected match: {should_match}, got: {is_matched}"
