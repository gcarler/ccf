...
class TestDonations:
    ...
    def test_certificate_not_found(self, full):
        assert full["c"].get(f"/api/donations/{uuid.uuid4()}/certificate", headers=full["h"]).status_code == 404
...
class TestAgenda:
    ...
    def test_create(self, full, db_session):
        ...
        assert _ok(
            c.post(
                "/api/agenda/events", json={"title": "Reunion", "start_at": "2026-07-01T10:00:00Z"}, headers=h
            ).status_code
        )
...
class TestMessaging:
    ...
    def test_mark_all_read(self, full):
        assert _ok(full["c"].post("/api/messaging/notifications/mark-all-read", json={}, headers=full["h"]).status_code)
