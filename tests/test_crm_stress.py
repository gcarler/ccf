...
def _seed_massive_data(db_session, num_cases=500):
    ...
    etapa = EtapaPipeline(id=uuid.uuid4(), pipeline_id=pipeline.id, nombre="Etapa Massive", orden=1)
...
def test_stress_recursion_depth_limit(db_session):
    ...
    for i in range(150):
        node = CrmAutomation(
            id=_uuid.uuid4(), name=f"Node {i}", trigger_event="manual", action_type="send_email", is_active=True
        )
        db_session.add(node)
        nodes.append(node)
    ...
    edges = []
    for i in range(149):
        edge = CrmAutomationEdge(id=_uuid.uuid4(), source_id=nodes[i].id, target_id=nodes[i + 1].id)
        db_session.add(edge)
        edges.append(edge)
    ...
    cycle_edge = CrmAutomationEdge(id=_uuid.uuid4(), source_id=nodes[-1].id, target_id=nodes[0].id)
