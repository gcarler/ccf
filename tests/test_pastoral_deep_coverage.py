...
def deep(client, db_session):
    ...
    for p in personas[:5]:
        t = models.TareaCRM(
            title=f"Task {p.first_name}", description="Test", persona_id=p.id, status="pending", priority="medium"
        )
        db_session.add(t)
        tasks.append(t)
    db_session.commit()
...
class TestConsolidationSubResourcesDeep:
    def test_create_assignment(self, deep):
        c, h, cases = deep["c"], deep["h"], deep["cases"]
        resp = c.post(
            f"/api/crm/casos/{cases[0].id}/assignments",
            json={
    ...
    def test_create_interaction(self, deep):
        c, h, cases = deep["c"], deep["h"], deep["cases"]
        resp = c.post(
            f"/api/crm/casos/{cases[0].id}/interactions",
            json={
    ...
    def test_create_task(self, deep):
        c, h, cases = deep["c"], deep["h"], deep["cases"]
        resp = c.post(
            f"/api/crm/casos/{cases[0].id}/tasks",
            json={
                "title": "New Task",
                "status": "pending",
            },
    ...
    def test_create_call(self, deep):
        c, h, cases = deep["c"], deep["h"], deep["cases"]
        resp = c.post(
            f"/api/crm/casos/{cases[0].id}/calls",
            json={
                "notas": "Test call",
                "resultado": "contacted",
            },
...
class TestVolunteersDeep:
    ...
    def test_update(self, deep):
        c, h, personas = deep["c"], deep["h"], deep["personas"]
        resp = c.patch(
            f"/api/crm/volunteers/{personas[0].id}",
            json={
                "first_name": "Updated",
            },
            headers=h,
