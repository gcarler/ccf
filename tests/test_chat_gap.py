...
class TestChat:
    ...
    def test_create_conv(self, full, db_session):
        ...
        assert _ok(
            full["c"]
            .post("/api/chat/conversations", json={"participant_ids": [str(p.id)]}, headers=full["h"])
            .status_code
        )
    ...
    def test_create_and_msg(self, full, db_session):
        ...
        assert _ok(
            c.post(f"/api/chat/conversations/{conv['id']}/messages", json={"content": "Hi"}, headers=h).status_code
        )
