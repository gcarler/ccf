    ...
    for i, pr in enumerate(projects[:4]):
        ...
        db_session.add(
            models.ProjectComment(
                project_id=pr.id, author_id=str(personas[i].id), content=f"Comment {i}", is_resolved=i % 2 == 0
            )
        )
        db_session.add(
    ...
    for i in range(8):
        ...
        db_session.add(
            models.CounselingTicket(
                persona_id=personas[i].id, subject=f"CT{i}", status=["open", "resolved"][i % 2], notes=f"Notes {i}"
            )
        )
        db_session.add(
        ...
        db_session.add(
            models.CommunicationLog(
                persona_id=personas[i].id, channel=["email", "whatsapp", "sms"][i % 3], content=f"Msg{i}"
            )
        )
        db_session.add(
...
class TestCRMFlow:
    def test_persona_lifecycle(self, full):
        ...
        for field in ["titulo", "estado", "prioridad", "notas", "last_contact_at", "next_contact_at"]:
            ...
            c.patch(f"/api/crm/casos/{full['cases'][0].id}", json={field: val}, headers=h)
        # List cases with all filters
        for f in ["source=EVANGELISMO", "status=active", f"persona_id={pid}", "page=1&page_size=5"]:
            c.get(f"/api/crm/casos?{f}", headers=h)
        # Create case interaction
        c.post(
            f"/api/crm/casos/{full['cases'][0].id}/interactions", json={"tipo": "Llamada", "notas": "Called"}, headers=h
        )
        # List interactions
        c.get(f"/api/crm/casos/{full['cases'][0].id}/interactions", headers=h)
        # Create case task
        resp = c.post(f"/api/crm/casos/{full['cases'][0].id}/tasks", json={"titulo": "Follow up"}, headers=h)
    ...
    def test_case_call_log(self, full):
        c, h = full["c"], full["h"]
        c.post(
            f"/api/crm/casos/{full['cases'][0].id}/calls",
            json={"outcome": "completed", "notes": "Talked", "duration_seconds": 300},
            headers=h,
        )
    ...
    def test_counseling_flow(self, full):
        ...
        if resp.status_code in (200, 201):
            ...
            if tid:
                c.patch(
                    f"/api/crm/counseling/{tid}",
                    json={"status": "resolved", "notes": "Done", "priority_level": "high"},
                    headers=h,
                )
    ...
    def test_tasks_flow(self, full):
        ...
        if resp.status_code in (200, 201):
            ...
            if tid:
                ...
                c.patch(
                    f"/api/crm/tasks/{tid}",
                    json={"titulo": "Updated", "status": "completed", "priority": "low"},
                    headers=h,
                )
    ...
    def test_groups_crud(self, full):
        ...
        c.put(
            f"/api/crm/grupos/{groups[0].id}",
            json={
                "nombre": "U",
                "ubicacion": "New",
                "participante_ids": [str(full["personas"][0].id)],
            },
            headers=h,
        )
    ...
    def test_supplies_attachments(self, full):
        ...
        if tasks.status_code == 200 and tasks.json():
            ...
            if tid:
                ...
                c.post(
                    f"/api/projects/{pid}/tasks/{tid}/supplies", json={"item_name": "Item", "quantity": 5}, headers=h
                )
...
class TestGruposFlow:
    ...
    def test_seasons_flow(self, full):
        ...
        resp = c.post(
            ...
            json={"name": f"S_{uuid.uuid4().hex[:6]}", "start_date": start, "end_date": end},
            headers=h,
        )
        if resp.status_code in (200, 201):
...
class TestCMSFlow:
    ...
    def test_enterprise_crud(self, full):
        ...
        for ep in [
            ...
            payload = (
                {"url": f"https://{uid}.t", "events": ["page.published"]}
                if "webhook" in ep
                else {"source_path": f"/o_{uid}", "target_path": f"/n_{uid}", "status_code": 301}
                if "redirect" in ep
                else {"name": f"CT_{uid}", "schema": {"fields": []}}
                if "custom" in ep
                else {"term": f"T_{uid}", "definition": "D"}
                if "glossary" in ep
                else {"name": f"F_{uid}"}
            )
