...
def evan_user(db_session):
    ...
    user = Usuario(
        id=p.id, sede_id=sede.id, username="evangelist",
...
class TestBulkAttendanceFull:
    def test_bulk_with_valid_data(self, evan_full, db_session):
        ...
        p = models.Persona(id=uuid.uuid4(), first_name="BulkP", last_name="Test",
                          sede_id=s.id)
        db_session.add(p)
        db_session.commit()
    ...
    def test_bulk_mark_absent(self, evan_full, db_session):
        ...
        p = models.Persona(id=uuid.uuid4(), first_name="AbsM", last_name="Test",
                          sede_id=s.id)
        db_session.add(p)
        db_session.flush()
...
class TestSingleAttendance:
    def test_register_single_attendance(self, evan_full, db_session):
        ...
        p = models.Persona(id=uuid.uuid4(), first_name="Single", last_name="Att",
                          sede_id=s.id)
        db_session.add(p)
        db_session.commit()

        resp = c.post("/api/evangelism/attendance",
            json={
                "event_id": str(evt.id),
                "persona_id": str(p.id),
                "session_date": "2026-08-15",
...
class TestSyncAssignments:
    def test_sync_assignments(self, evan_full, db_session):
        ...
        p = models.Persona(id=uuid.uuid4(), first_name="Assign", last_name="Test",
                          sede_id=s.id)
        db_session.add(p)
        db_session.commit()

        resp = c.post(f"/api/evangelism/events/{evt.id}/assignments",
            json={
                "session_date": "2026-08-15",
