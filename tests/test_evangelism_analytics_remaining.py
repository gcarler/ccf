...
class TestAlertsData:
    def test_all_alert_types(self, full, db_session):
        ...
        # Personas
        personas = [models.Persona(id=uuid.uuid4(), first_name=f"A{i}", last_name="T", sede_id=s.id) for i in range(4)]
        db_session.add_all(personas)
        db_session.flush()

        # Strategy
        strat = c.post("/api/evangelism/strategies", json={"name": f"AN-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        sid = uuid.UUID(strat["id"])

        # --- Grupo A: only 1 session -> len(recent) < 3 -> line 686 ---
        g_a = models.GrupoEvangelismo(
            id=uuid.uuid4(), nombre="GA", sede_id=s.id, lider_persona_id=personas[0].id, estrategia_id=sid
        )
        db_session.add(g_a)
        db_session.flush()
        ses_a = models.SesionGrupo(
            id=uuid.uuid4(), grupo_id=g_a.id, fecha_sesion=now - timedelta(days=45), estado="REALIZADA"
        )
        db_session.add(ses_a)

        # --- Grupo B: 3+ sessions, low attendance (< 60%) -> lines 695, 698-699 ---
        g_b = models.GrupoEvangelismo(
            id=uuid.uuid4(), nombre="GB", sede_id=s.id, lider_persona_id=personas[1].id, estrategia_id=sid, capacidad=10
        )
        db_session.add(g_b)
        for i in range(3):
            ses = models.SesionGrupo(
                id=uuid.uuid4(), grupo_id=g_b.id, fecha_sesion=now - timedelta(weeks=i), estado="REALIZADA"
            )
            db_session.add(ses)
            db_session.flush()
            # 2 attendances, 1 present -> 50% < 60%
            db_session.add(
                models.Asistencia(id=uuid.uuid4(), sesion_id=ses.id, persona_id=personas[0].id, estado="ASISTIO")
            )
            db_session.add(
                models.Asistencia(id=uuid.uuid4(), sesion_id=ses.id, persona_id=personas[1].id, estado="FALTO")
            )

        # --- Grupo C: near capacity -> line 730 ---
        g_c = models.GrupoEvangelismo(
            id=uuid.uuid4(), nombre="GC", sede_id=s.id, lider_persona_id=personas[2].id, estrategia_id=sid, capacidad=3
        )
        db_session.add(g_c)
            ...
            for pi in personas:
                db_session.add(
                    models.ParticipanteGrupo(
                        id=uuid.uuid4(), grupo_id=g.id, persona_id=pi.id, rol_base="miembro", activo=True
                    )
                )
        db_session.commit()
...
class TestFullEndpointDeep:
    def test_full_with_irt_and_demographics(self, full, db_session):
        ...
        p1 = Persona(id=uuid.uuid4(), first_name="Adult", last_name="T", sede_id=s.id, birthday=date(1990, 6, 15))
        p2 = Persona(id=uuid.uuid4(), first_name="Young", last_name="T", sede_id=s.id, birthday=date(1995, 3, 10))
        p3 = Persona(id=uuid.uuid4(), first_name="Senior", last_name="T", sede_id=s.id, birthday=date(1955, 11, 20))
        db_session.add_all([p1, p2, p3])
        db_session.flush()

        strat = c.post("/api/evangelism/strategies", json={"name": f"FD-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        sid = uuid.UUID(strat["id"])
