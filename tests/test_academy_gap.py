...
class TestAcademyCourses:
    ...
    def test_assessments_not_found(self, full):
        assert full["c"].get(f"/api/academy/courses/{uuid.uuid4()}/assessments",
...
class TestAcademyCertificates:
    ...
    def test_validate_not_found(self, full):
        assert full["c"].get("/api/academy/certificates/validate/invalid-code",
            headers=full["h"]).status_code == 404
...
class TestAcademyForum:
    ...
    def test_get_thread_not_found(self, full):
        assert full["c"].get(f"/api/academy/forum/threads/{uuid.uuid4()}",
            headers=full["h"]).status_code == 404
...
class TestAcademyLesson:
    def test_lesson_progress_not_found(self, full):
        assert full["c"].get(f"/api/academy/lessons/{uuid.uuid4()}/progress",
            headers=full["h"]).status_code == 404
    def test_resources_not_found(self, full):
        assert full["c"].get(f"/api/academy/lessons/{uuid.uuid4()}/resources",
            headers=full["h"]).status_code == 404
