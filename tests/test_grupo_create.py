...
class TestGrupoCreate:
    def test_create_simple(self, full, db_session):
        ...
        c, h = full["c"], full["h"]
        resp = c.post("/api/evangelism/grupos", json={"name": f"G-{uuid.uuid4().hex[:6]}"}, headers=h)
    ...
    def test_create_with_strategy(self, full, db_session):
        ...
        resp = c.post(
            ...
            json={"name": f"GS-{uuid.uuid4().hex[:6]}", "evangelism_strategy_id": strat["id"]},
            headers=h,
        )
    ...
    def test_create_with_leader(self, full, db_session):
        ...
        db_session.commit()
        resp = c.post(
            "/api/evangelism/grupos", json={"name": f"GL-{uuid.uuid4().hex[:6]}", "leader_id": str(p.id)}, headers=h
        )
    ...
    def test_extra_field_422(self, full):
        ...
        resp = full["c"].post("/api/evangelism/grupos", json={"name": "Test", "extra": "x"}, headers=full["h"])
    ...
    def test_update(self, full):
        c, h = full["c"], full["h"]
        g = c.post("/api/evangelism/grupos", json={"name": f"GU-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        resp = c.put(f"/api/evangelism/grupos/{g['id']}", json={"name": "Updated"}, headers=h)
