...
class TestEnterpriseCrudDirect:
    def test_create_audit_log(self, authed_client):
        ...
        log = AuditLog(
            id=uuid.uuid4(), actor_persona_id=persona.id, action="test.create", entity_type="test", entity_id="123"
        )
        db.add(log)
        db.flush()
    ...
    def test_create_notification(self, authed_client):
        ...
        notif = CmsNotification(
            id=uuid.uuid4(), recipient_persona_id=persona.id, notification_type="test", title="Test Notif"
        )
        db.add(notif)
        db.flush()
    ...
    def test_create_custom_entry(self, authed_client):
        ...
        entry = CmsCustomEntry(
            id=uuid.uuid4(), site_key="faro", type_key="wiki", slug="test-article", title="Test Article"
        )
        db.add(entry)
        db.flush()
    ...
    def test_create_glossary_term(self, authed_client):
        ...
        term = CmsGlossaryTerm(
            id=uuid.uuid4(), site_key="faro", term="CRM", definition="Customer Relationship Management"
        )
        db.add(term)
        db.flush()
    ...
    def test_create_media_folder(self, authed_client):
        ...
        folder = MediaFolder(
            id=uuid.uuid4(), site_key="faro", name="Test Folder", slug="test-folder", path="/test-folder/"
        )
        db.add(folder)
        db.flush()
