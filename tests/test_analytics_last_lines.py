...
class TestVelocityOrder:
    def test_velocity_order_line(self, full, db_session):
        ...
        g = models.GrupoEvangelismo(id=uuid.uuid4(), nombre="VG",
            sede_id=s.id, lider_persona_id=p.id, estrategia_id=sid)
        db_session.add(g)
        db_session.flush()
...
class TestRetentionLine268:
    def test_retention_with_prev_active(self, full, db_session):
        ...
        strat = c.post("/api/evangelism/strategies",
            json={"name": f"RT-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        sid = uuid.UUID(strat["id"])

        g = models.GrupoEvangelismo(id=uuid.uuid4(), nombre="RG",
            sede_id=s.id, lider_persona_id=p.id, estrategia_id=sid)
        db_session.add(g)
        db_session.flush()
        ...
        ses = models.SesionGrupo(id=uuid.uuid4(), grupo_id=g.id,
            fecha_sesion=now - timedelta(days=15), estado="REALIZADA")
        db_session.add(ses)
        db_session.flush()
...
class TestDim9And10:
    def test_dim9_retencion_and_dim10(self, full, db_session):
        ...
        p_origen = models.Persona(id=uuid.uuid4(), first_name="Camp",
        ...
        strat = c.post("/api/evangelism/strategies",
            json={"name": f"D9-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        sid = uuid.UUID(strat["id"])
        ...
        g = models.GrupoEvangelismo(id=uuid.uuid4(), nombre="D9G",
            sede_id=s.id, lider_persona_id=p_origen.id, estrategia_id=sid,
            capacidad=5, activo=True, ubicacion="Zona Norte")
        db_session.add(g)
        db_session.flush()
        ...
        for j in range(4):
            ...
            db_session.flush()
            db_session.add(models.Asistencia(id=uuid.uuid4(), sesion_id=ses.id,
                            persona_id=p_origen.id, estado="ASISTIO"))

        # ParticipanteGrupo
        db_session.add(models.ParticipanteGrupo(id=uuid.uuid4(), grupo_id=g.id,
                        persona_id=p_origen.id, rol_base="miembro", activo=True))
