...
class TestEventCreate:
    def test_create(self, full):
        ...
        resp = full["c"].post("/api/evangelism/events",
    ...
    def test_create_and_get(self, full):
        ...
        evt = full["c"].post("/api/evangelism/events",
            json={"name": name, "event_date": "2026-09-01T10:00:00Z"},
            headers=full["h"]).json()
        eid = evt["id"]
    ...
    def test_create_and_update(self, full):
        evt = full["c"].post("/api/evangelism/events",
            ...
            headers=full["h"]).json()
        resp = full["c"].put(f"/api/evangelism/events/{evt['id']}",
            json={"name": "Updated"}, headers=full["h"])
        assert _ok(resp.status_code)
