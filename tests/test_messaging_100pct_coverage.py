...
def test_create_log_leader_id_variations():
    ...
    # 1. leader_id as UUID
    log1 = _create_log(db, persona_id=persona_id, channel="WhatsApp", content="Test 1", leader_id=leader_uuid)
    assert log1.leader_id == leader_uuid

    # 2. leader_id as valid UUID string
    log2 = _create_log(db, persona_id=persona_id, channel="WhatsApp", content="Test 2", leader_id=str(leader_uuid))
    assert log2.leader_id == leader_uuid

    # 3. leader_id as invalid UUID string
    log3 = _create_log(db, persona_id=persona_id, channel="WhatsApp", content="Test 3", leader_id="invalid-uuid")
