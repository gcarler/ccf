...
class TestFinance:
    ...
    def test_create_fund(self, full):
        assert _ok(
            full["c"]
            .post("/api/finance/admin/funds", json={"name": f"F-{uuid.uuid4().hex[:6]}"}, headers=full["h"])
            .status_code
        )
    ...
    def test_create_and_update_fund(self, full):
        ...
        assert _ok(
            full["c"]
            .patch(f"/api/finance/admin/funds/{r.json()['fund_id']}", json={"name": "Upd"}, headers=full["h"])
            .status_code
        )
    ...
    def test_update_fund_not_found(self, full):
        assert (
            full["c"]
            .patch(f"/api/finance/admin/funds/{uuid.uuid4()}", json={"name": "X"}, headers=full["h"])
            .status_code
            == 404
        )

    def test_delete_fund_not_found(self, full):
        assert full["c"].delete(f"/api/finance/admin/funds/{uuid.uuid4()}", headers=full["h"]).status_code == 404
