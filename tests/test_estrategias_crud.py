...
class TestEstrategiasCRUD:
    ...
    def test_create_with_name(self, full):
        ...
        resp = full["c"].post(
            "/api/evangelism/strategies", json={"name": f"E-{uuid.uuid4().hex[:6]}"}, headers=full["h"]
        )
    ...
    def test_create_and_get(self, full):
        ...
        created = full["c"].post("/api/evangelism/strategies", json={"name": name}, headers=full["h"]).json()
        sid = created["id"]
    ...
    def test_create_and_update(self, full):
        created = (
            full["c"]
            .post("/api/evangelism/strategies", json={"name": f"EU-{uuid.uuid4().hex[:6]}"}, headers=full["h"])
            .json()
        )
        sid = created["id"]
        resp = full["c"].put(f"/api/evangelism/strategies/{sid}", json={"name": "Updated Name"}, headers=full["h"])
    ...
    def test_create_and_delete(self, full):
        created = (
            full["c"]
            .post("/api/evangelism/strategies", json={"name": f"ED-{uuid.uuid4().hex[:6]}"}, headers=full["h"])
            .json()
        )
        sid = created["id"]
    ...
    def test_create_with_extra_field_422(self, full):
        ...
        resp = full["c"].post(
            "/api/evangelism/strategies", json={"name": "Test", "extra_field": "should_fail"}, headers=full["h"]
        )
        assert resp.status_code == 422

    def test_create_without_name_422(self, full):
        """name is required."""
        resp = full["c"].post("/api/evangelism/strategies", json={"description": "Missing name"}, headers=full["h"])
        assert resp.status_code == 422
