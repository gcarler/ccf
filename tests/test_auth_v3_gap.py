...
class TestAuthV3:
    def test_login(self, full):
        assert _ok(
            full["c"].post("/api/v3/auth/login", json={"email": "auth@test.com", "password": "testpass123"}).status_code
        )

    def test_login_bad_credentials(self, full):
        assert (
            full["c"].post("/api/v3/auth/login", json={"email": "auth@test.com", "password": "wrong"}).status_code
            == 401
        )
    ...
    def test_patch_me(self, full):
        assert _ok(full["c"].patch("/api/v3/auth/me", json={"first_name": "Updated"}, headers=full["h"]).status_code)

    def test_check_email(self, full):
        assert _ok(full["c"].get("/api/v3/auth/check-email?email=auth@test.com", headers=full["h"]).status_code)
    ...
    def test_refresh(self, full):
        ...
        login = full["c"].post("/api/v3/auth/login", json={"email": "auth@test.com", "password": "testpass123"}).json()
        rt = login.get("refresh_token")
        if rt:
            assert _ok(full["c"].post("/api/v3/auth/refresh", json={"refresh_token": rt}).status_code)
