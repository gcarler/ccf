...
class TestPastoralRemaining:
    ...
    def test_caso_patch_not_found(self, full):
        assert (
            full["c"].patch(f"/api/crm/casos/{uuid.uuid4()}", json={"stage": "call"}, headers=full["h"]).status_code
            == 404
        )
    ...
    def test_caso_task_not_found(self, full):
        assert (
            full["c"]
            .post(f"/api/crm/casos/{uuid.uuid4()}/tasks", json={"title": "Test"}, headers=full["h"])
            .status_code
            == 404
        )
