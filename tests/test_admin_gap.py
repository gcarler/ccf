...
class TestAdminRoles:
    ...
    def test_create(self, full):
        assert _ok(
            full["c"]
            .post("/api/admin/roles", json={"name": f"R-{uuid.uuid4().hex[:6]}"}, headers=full["h"])
            .status_code
        )
...
class TestAdminLocations:
    ...
    def test_create(self, full):
        assert _ok(
            full["c"]
            .post("/api/admin/locations", json={"name": f"L-{uuid.uuid4().hex[:6]}"}, headers=full["h"])
            .status_code
        )
...
class TestAdminVariables:
    ...
    def test_create(self, full):
        assert _ok(
            full["c"]
            .post("/api/admin/variables", json={"key": f"k_{uuid.uuid4().hex[:6]}", "value": "test"}, headers=full["h"])
            .status_code
        )
