...
class TestCheckin:
    ...
    def test_checkin_scan(self, full):
        resp = full["c"].post("/api/evangelism/events/checkin/scan",
            json={"qr_token": str(uuid.uuid4())}, headers=full["h"])
...
class TestParticipants:
    def test_create_participant_not_found(self, full):
        ...
        resp = full["c"].post(f"/api/evangelism/events/{uuid.uuid4()}/participants",
            json={"persona_id": str(uuid.uuid4()), "role": "asistente"},
            headers=full["h"])
        assert resp.status_code == 404

    def test_list_participants_not_found(self, full):
        resp = full["c"].get(f"/api/evangelism/events/{uuid.uuid4()}/participants",
            headers=full["h"])
        assert resp.status_code == 404

    def test_delete_participant_not_found(self, full):
        resp = full["c"].delete(f"/api/evangelism/events/{uuid.uuid4()}/participants/{uuid.uuid4()}",
            headers=full["h"])
        assert resp.status_code == 404
