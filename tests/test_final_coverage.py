...
class TestGruposMain:
    ...
    def test_create_group(self, full):
        ...
        resp = c.post(
            ...
            json={
                "nombre": f"New_{uuid.uuid4().hex[:6]}",
                "codigo": f"N{uuid.uuid4().hex[:6]}",
                "ubicacion": "Place",
...
class TestEvangelismAPI:
    def test_counseling_crud(self, full):
        ...
        resp = c.post(
            ...
            json={"persona_id": str(personas[0].id), "subject": f"S_{uuid.uuid4().hex[:6]}"},
            headers=h,
        )
        assert _ok(resp.status_code)
        if resp.status_code in (200, 201):
    ...
    def test_prayer_crud(self, full):
        ...
        resp = c.post(
            ...
            json={"requester_name": "P", "request_text": "H", "sede_id": str(sede.id)},
            headers=h,
        )
        assert _ok(resp.status_code)
        if resp.status_code in (200, 201):
    ...
    def test_messaging(self, full):
        ...
        for ch in ["email", "whatsapp", "sms"]:
            c.post(
                ...
                json={"channel": ch, "persona_id": str(personas[0].id), "content": f"M {ch}"},
                headers=h,
            )
        for seg in ["active", "groups"]:
            c.post(
                "/api/evangelism/messaging/send",
                json={"channel": "email", "content": "S", "target_segments": [seg], "campaign_name": "C"},
                headers=h,
            )
        c.post(
            "/api/evangelism/messaging/send",
            json={"channel": "email", "content": "NT", "target_segments": ["nonexistent"]},
            headers=h,
        )
