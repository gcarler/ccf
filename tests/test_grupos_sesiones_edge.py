...
class TestCreateSessionEdge:
    def test_create_session_missing_grupo_id(self, evan_full):
        ...
        resp = evan_full["c"].post(
            "/api/evangelism/sessions", json={"session_date": "2026-07-15", "topic": "Test"}, headers=evan_full["h"]
        )
    ...
    def test_create_session_grupo_not_found(self, evan_full):
        resp = evan_full["c"].post(
            ...
            json={"session_date": "2026-07-15", "grupo_id": str(uuid.uuid4()), "topic": "Test"},
            headers=evan_full["h"],
        )
    ...
    def test_create_session_valid(self, evan_full, db_session):
        ...
        resp = c.post(
            "/api/evangelism/sessions",
            json={"session_date": "2026-07-15", "grupo_id": str(g.id), "topic": "Test", "status": "Realizada"},
            headers=h,
        )
...
class TestUpdateSession:
    def test_update_session(self, evan_full, db_session):
        ...
        db_session.add(ses)
        db_session.commit()
        resp = c.put(f"/api/evangelism/sessions/{ses.id}", json={"topic": "Updated Topic"}, headers=h)
        assert _ok(resp.status_code), f"update: {resp.status_code} {resp.text[:200]}"

    def test_update_not_found(self, evan_full):
        resp = evan_full["c"].put(
            f"/api/evangelism/sessions/{uuid.uuid4()}", json={"topic": "Test"}, headers=evan_full["h"]
        )
        assert resp.status_code == 404
...
class TestHabTodas:
    def test_habilitar_todas(self, evan_full, db_session):
        ...
        strat = c.post("/api/evangelism/strategies", json={"name": f"HT-{uuid.uuid4().hex[:6]}"}, headers=h).json()
    ...
    def test_deshabilitar_todas(self, evan_full, db_session):
        c, h, s = evan_full["c"], evan_full["h"], evan_full["s"]
        strat = c.post("/api/evangelism/strategies", json={"name": f"DT-{uuid.uuid4().hex[:6]}"}, headers=h).json()
        sid = uuid.UUID(strat["id"])
        g = _setup_grupo(db_session, s)
        g.estrategia_id = sid
        db_session.commit()
    ...
    def test_habilitar_todas_no_grupos(self, evan_full):
        resp = evan_full["c"].post(f"/api/evangelism/strategies/{uuid.uuid4()}/habilitar-todas", headers=evan_full["h"])
        assert resp.status_code == 404
