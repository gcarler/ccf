...
def test_drag_drop_nonexistent_stage(client, db_session):
    ...
    payload = {"caso_id": str(caso1.id), "target_stage_id": str(uuid.uuid4())}
...
def test_drag_drop_stage_different_pipeline(client, db_session):
    ...
    pipeline2 = PipelineCRM(id=uuid.uuid4(), sede_id=sede.id, nombre="Pipeline Test 2", tipo="CONSEJERIA", activo=True)
    db_session.add(pipeline2)
    db_session.flush()

    # Stage of pipeline 2
    etapa_diff = EtapaPipeline(id=uuid.uuid4(), pipeline_id=pipeline2.id, nombre="Etapa Diff", orden=1)
    db_session.add(etapa_diff)
    db_session.commit()

    payload = {"caso_id": str(caso1.id), "target_stage_id": str(etapa_diff.id)}
...
def test_drag_drop_stage_different_sede_tenant(client, db_session):
    ...
    payload = {"caso_id": str(caso1_a.id), "target_stage_id": str(etapa2_a.id)}
...
def test_cycle_detection_self_loop(client, db_session):
    ...
    payload = {"nodes": ["n1"], "edges": [{"source": "n1", "target": "n1"}]}
