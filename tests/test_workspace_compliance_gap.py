...
class TestCompliance:
    ...
    def test_snapshot_download(self, full):
        assert _ok(
            full["c"].get("/api/workspace/flags/compliance/snapshot?download=true", headers=full["h"]).status_code
        )

    def test_snapshot_no_record(self, full):
        assert _ok(
            full["c"].get("/api/workspace/flags/compliance/snapshot?record=false", headers=full["h"]).status_code
        )
    ...
    def test_update_policy(self, full):
        assert _ok(
            full["c"]
            .put(
                "/api/workspace/flags/compliance/policy", json={"critical_feature_flags": ["feat_x"]}, headers=full["h"]
            )
            .status_code
        )
    ...
    def test_delete_suppression_not_found(self, full):
        assert (
            full["c"].delete("/api/workspace/flags/compliance/suppressions/nonexistent", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_history_invalid_since(self, full):
        assert (
            full["c"].get("/api/workspace/flags/compliance/history?since=bad-date", headers=full["h"]).status_code
            == 422
        )

    def test_weekly_summary(self, full):
        assert _ok(
            full["c"].get("/api/workspace/flags/compliance/history/weekly-summary", headers=full["h"]).status_code
        )

    def test_history_item_not_found(self, full):
        assert (
            full["c"].get("/api/workspace/flags/compliance/history/nonexistent-id", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_cleanup(self, full):
        assert _ok(
            full["c"]
            .post("/api/workspace/flags/compliance/history/cleanup", json={"retain_days": 90}, headers=full["h"])
            .status_code
        )
