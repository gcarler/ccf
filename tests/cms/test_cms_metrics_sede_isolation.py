...
def _seed_two_sedes(db_session):
    admin_a, persona_a, sede_a = seed_admin(db_session, email="cmsMetricsA@example.com", password="testpass123")
    admin_b, persona_b, sede_b = seed_admin(db_session, email="cmsMetricsB@example.com", password="testpass123")
...
def _seed_testimonial_in_sede(db, author_persona, sede_id, content, is_approved=False):
...
def test_content_metrics_overview_scopes_testimonials_by_sede(client, db_session):
    ...
    _seed_testimonial_in_sede(db_session, persona_a, sede_a.id, "t-a 1 — local sede_a")
    _seed_testimonial_in_sede(db_session, persona_a, sede_a.id, "t-a 2 — local sede_a")
    _seed_testimonial_in_sede(db_session, persona_b, sede_b.id, "t-b 1 — cross-sede leak")
    _seed_testimonial_in_sede(db_session, persona_b, sede_b.id, "t-b 2 — cross-sede leak")
    db_session.commit()
    ...
    assert body["testimonials_total"] == 2, (
        f"LEAK: admin A ve testimonials_total={body['testimonials_total']}, debería ser 2 (sólo sede_a). Body={body}"
    )


def test_content_metrics_overview_scopes_announcements_by_sede(client, db_session):
    """Axioma 3: announcements_total acotado por sede del staff."""
    (admin_a, persona_a, sede_a), (_, persona_b, sede_b) = _seed_two_sedes(db_session)
    _seed_announcement_in_sede(db_session, persona_a, sede_a.id, "a-a-1", "local sede_a")
    _seed_announcement_in_sede(db_session, persona_b, sede_b.id, "a-b-1", "cross-sede leak")
    _seed_announcement_in_sede(db_session, persona_b, sede_b.id, "a-b-2", "cross-sede leak")
    db_session.commit()
    ...
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["announcements_total"] == 1, f"LEAK: announcements_total={body['announcements_total']} cross-sede"


def test_content_metrics_overview_scopes_media_by_sede(client, db_session):
    """Axioma 3: media_total se acota por sede del staff."""
    (admin_a, persona_a, sede_a), (_, persona_b, sede_b) = _seed_two_sedes(db_session)
    _seed_media_in_sede(db_session, persona_a, sede_a.id, "https://cdn.example.com/local-a.png")
    _seed_media_in_sede(db_session, persona_b, sede_b.id, "https://cdn.example.com/cross-b.png")
    _seed_media_in_sede(db_session, persona_b, sede_b.id, "https://cdn.example.com/cross-b2.jpg")
    db_session.commit()
    ...
    # retornaría 3.
    assert body["media_total"] == 1, f"LEAK: media_total={body['media_total']} debe ser 1 (sólo sede_a)"
    assert body["media_images"] == 1, f"LEAK: media_images={body['media_images']} debe ser 1"


def test_content_metrics_overview_superadmin_sees_all(client, db_session, monkeypatch):
    ...
    from backend.api.cms import v1 as _cms_api_module
    ...
    (admin_a, persona_a, sede_a), (_, persona_b, sede_b) = _seed_two_sedes(db_session)
    _seed_testimonial_in_sede(db_session, persona_a, sede_a.id, "global-a")
    _seed_testimonial_in_sede(db_session, persona_b, sede_b.id, "global-b")
    db_session.commit()
    ...
    assert body["testimonials_total"] == 2, f"Superadmin debe ver 2 testimonials; got {body['testimonials_total']}"
...
def test_agents_analytics_summary_pending_testimonials_scoped_by_sede(client, db_session):
    ...
    _seed_testimonial_in_sede(db_session, persona_a, sede_a.id, "a-pending")
    _seed_testimonial_in_sede(db_session, persona_a, sede_a.id, "a-approved", is_approved=True)
    ...
    assert body["pending_testimonials"] == 1, (
        f"LEAK: pending_testimonials={body['pending_testimonials']} debe ser 1 (sólo sede_a); got body={body}"
    )


def test_agents_analytics_summary_superadmin_sees_all_pending(client, db_session, monkeypatch):
...
def test_agents_analytics_summary_pending_testimonials_unaffected_when_zero(client, db_session):
