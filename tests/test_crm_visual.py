...
def _seed_test_data(db_session):
    ...
    pipeline = PipelineCRM(
        id=uuid.uuid4(), sede_id=sede.id, nombre="Pipeline Test", tipo=TipoPipelineEnum.NUEVOS_VISITANTES, activo=True
    )
    db_session.add(pipeline)
    db_session.flush()

    etapa1 = EtapaPipeline(id=uuid.uuid4(), pipeline_id=pipeline.id, nombre="Etapa 1", orden=1)
    etapa2 = EtapaPipeline(id=uuid.uuid4(), pipeline_id=pipeline.id, nombre="Etapa 2", orden=2)
...
def test_tier1_f2_drag_and_drop_event_listener(client, db_session):
    admin, persona, sede, pipeline, etapa1, etapa2, caso1, caso2 = _seed_test_data(db_session)
    headers = auth_headers(client, email=admin.email)
    payload = {"caso_id": str(caso1.id), "source_stage_id": str(etapa1.id), "target_stage_id": str(etapa2.id)}
...
def test_tier1_f6_three_node_connection_edge_model():
    assert hasattr(Base.metadata.tables.get("crm_automation_edges"), "validate_three_node_path"), (
        "F6: validate_three_node_path method not implemented on crm_automation_edges table"
    )


def test_tier1_f6_three_node_connection_source_target_keys():
    edges_table = Base.metadata.tables.get("crm_automation_edges")
    assert edges_table is not None and "source_node_id" in edges_table.columns, (
        "F6: crm_automation_edges missing source_node_id column"
    )


def test_tier1_f6_three_node_connection_ports():
    nodes_table = Base.metadata.tables.get("crm_automation_nodes")
    assert nodes_table is not None and "ports_config" in nodes_table.columns, (
        "F6: crm_automation_nodes missing ports_config column"
    )


def test_tier1_f6_three_node_connection_integrity():
    edges_table = Base.metadata.tables.get("crm_automation_edges")
    assert edges_table is not None and "on_delete_cascade" in edges_table.columns, (
        "F6: crm_automation_edges missing cascade constraint column"
    )
...
def test_tier1_f7_backend_branching_multiple_conditions():
    ...
    assert branches_table is not None and "conditions_logic" in branches_table.columns, (
        "F7: crm_flow_branches missing conditions_logic column"
    )
...
def test_tier2_f2_drag_drop_invalid_stage(client, db_session):
    admin, persona, sede, pipeline, etapa1, etapa2, caso1, caso2 = _seed_test_data(db_session)
    headers = auth_headers(client, email=admin.email)
    payload = {"caso_id": str(caso1.id), "target_stage_id": "00000000-0000-0000-0000-000000000000"}
...
def test_tier2_f3_reorder_endpoint_duplicate_ids(client, db_session):
    ...
    response = client.patch(
        "/api/crm/pipeline/casos/reorder", json=[{"id": str(caso1.id)}, {"id": str(caso1.id)}], headers=headers
    )
...
def test_tier2_f3_reorder_endpoint_nonexistent_ids(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.patch(
        "/api/crm/pipeline/casos/reorder", json=[{"id": "00000000-0000-0000-0000-000000000000"}], headers=headers
    )
...
def test_tier2_f3_reorder_endpoint_extreme_size(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.patch(
        "/api/crm/pipeline/casos/reorder", json=[{"id": str(caso1.id)} for i in range(5000)], headers=headers
    )
...
def test_tier2_f4_atomic_reorder_null_sort_order():
    assert hasattr(CasoCRM, "handle_null_sort_order"), (
        "F4 Tier 2: CasoCRM model handle_null_sort_order method not implemented"
    )


def test_tier2_f4_atomic_reorder_duplicate_index():
    assert hasattr(CasoCRM, "resolve_duplicate_sort_index"), (
        "F4 Tier 2: CasoCRM model resolve_duplicate_sort_index method not implemented"
    )


def test_tier2_f4_atomic_reorder_partial_failure():
    assert hasattr(CasoCRM, "reorder_transaction_rollback"), (
        "F4 Tier 2: CasoCRM reorder transaction rollback hook not implemented"
    )


def test_tier2_f4_atomic_reorder_negative_index():
    assert hasattr(CasoCRM, "allow_negative_sort_indices"), (
        "F4 Tier 2: CasoCRM negative sort index validation not implemented"
    )


def test_tier2_f4_atomic_reorder_cross_sede_leakage():
    assert hasattr(CasoCRM, "verify_sede_isolation_on_reorder"), (
        "F4 Tier 2: CasoCRM Sede isolation during reordering not implemented"
    )
...
def test_tier2_f5_flow_builder_disconnected_node(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/flows/disconnected-nodes",
        json={"nodes": ["n1", "n2", "n3"], "edges": [{"source": "n1", "target": "n2"}]},
        headers=headers,
    )
    assert response.status_code == 200
...
def test_tier2_f5_flow_builder_invalid_node_type(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/flows/validate-types",
        json={"nodes": [{"id": "n1", "type": "invalid_type"}]},
        headers=headers,
    )
    assert response.status_code == 400
...
def test_tier2_f7_backend_branching_null_vars(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/branching/null-vars",
        json={"variables": {"nombre": None, "email": "test@example.com"}},
        headers=headers,
    )
    assert response.status_code == 200
...
def test_tier2_f7_backend_branching_type_mismatch(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/branching/type-mismatch",
        json={
...
def test_tier2_f7_backend_branching_missing_else(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/branching/missing-else",
        json={"node_id": "n1", "edges": [{"source": "n1", "target": "n2", "source_port": "true"}]},
        headers=headers,
    )
    assert response.status_code == 400
...
def test_tier2_f7_backend_branching_infinite_nesting(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/branching/infinite-nesting",
        json={
            "nodes": ["n1", "n2", "n3"],
            "edges": [
                {"source": "n1", "target": "n2"},
...
def test_tier2_f7_backend_branching_unexpected_operator(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/branching/unexpected-op",
        json={"conditions": [{"key": "nombre", "operator": "invalid_op", "value": "Juan"}]},
        headers=headers,
    )
    assert response.status_code == 400
...
def test_tier2_f8_loop_cycle_deep_nesting(client, db_session):
    ...
    response = client.post(
        "/api/crm/automations/flows/check-cycles",
        json={
            "nodes": ["n1", "n2", "n3", "n4"],
            "edges": [
                {"source": "n1", "target": "n2"},
...
def test_tier3_kanban_reorder_sync(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.post(
        "/api/crm/pipeline/kanban/sync-reorder", json={"pipeline_id": str(pipeline.id)}, headers=headers
    )
    assert response.status_code == 200
    db_session.refresh(caso1)
    db_session.refresh(caso2)
...
def test_tier3_drag_drop_atomic_reorder(db_session):
    assert hasattr(CasoCRM, "atomic_drag_drop_reorder"), (
        "Tier 3: Atomic drag-and-drop model transaction helper not implemented"
    )
    admin, persona, sede, pipeline, etapa1, etapa2, caso1, caso2 = _seed_test_data(db_session)
...
def test_tier3_flow_builder_three_node(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.post(
        "/api/crm/automations/flow-builder/three-node-render", json={"flow_id": str(flow.id)}, headers=headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_tier3_reorder_flow_trigger(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.post(
        "/api/crm/pipeline/casos/reorder-trigger-automation", json={"caso_id": str(caso1.id)}, headers=headers
    )
    assert response.status_code == 200
...
def test_tier3_branching_three_node_connection(client, db_session):
    ...
    headers = auth_headers(client, email=admin.email)
    response = client.post(
        "/api/crm/automations/branching/three-node-traversal", json={"pipeline_id": str(pipeline.id)}, headers=headers
    )
    assert response.status_code == 200
...
def test_tier3_atomic_branching_traversal(db_session):
    assert hasattr(CasoCRM, "atomic_reorder_branching_eval"), (
        "Tier 3: Atomic reorder with branching path evaluation not implemented"
    )
    ...
    payload = [{"id": str(caso1.id), "sort_order": 1, "drag_target_etapa_id": str(etapa2.id)}]
    ...
    pending_actions = (
        ...
        .filter(PendingCrmAction.automation_id == automation.id, PendingCrmAction.target_persona_id == caso1.persona_id)
        .all()
    )
...
def test_tier4_scenario1_lead_qualification(client, db_session):
    admin, persona, sede, pipeline, etapa1, etapa2, caso1, caso2 = _seed_test_data(db_session)
    headers = auth_headers(client, email=admin.email)
    payload = {"caso_id": str(caso1.id), "target_etapa_id": str(etapa2.id), "qualification_data": {"qualified": True}}
...
def test_tier4_scenario2_support_ticket_routing(client, db_session):
    ...
    ticket = SupportTicket(
        user_id=persona.id, subject=f"Trouble with {caso1.titulo_caso}", description="Support needed"
    )
    db_session.add(ticket)
    db_session.commit()
    headers = auth_headers(client, email=admin.email)
    payload = {"caso_id": str(caso1.id), "routing_rules": {"category": "support"}}
...
def test_tier4_scenario5_multitenant_isolation_crm(client, db_session):
    ...
    user_b, persona_b, sede_b = seed_user_with_role(
        db_session, role_name="ADMIN", email="admin_b@example.com", sede_id=uuid.uuid4()
    )
