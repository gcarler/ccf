...
def test_zero_attendance_opportunities(db_session):
    ...
    score, status = recalculate_and_persist_pastoral_health(db_session, persona_a.id)
    assert score == 0
    ...
    score, status = recalculate_and_persist_pastoral_health(db_session, persona_b.id)
    assert score == 10
    assert status == "EN_RIESGO"
    ...
    score, status = recalculate_and_persist_pastoral_health(db_session, persona_c.id)
    assert score == 50
...
def test_attendance_status_normalization_and_mix(db_session):
    ...
    db_session.add_all(
        [
            ...
            models.CourseAttendance(
                id=uuid.uuid4(), enrollment_id=enrollment.id, session_date=datetime.now(timezone.utc), status="PRESENT"
            ),
            models.CourseAttendance(
                id=uuid.uuid4(), enrollment_id=enrollment.id, session_date=datetime.now(timezone.utc), status="absent"
            ),
            models.CourseAttendance(
                id=uuid.uuid4(), enrollment_id=enrollment.id, session_date=datetime.now(timezone.utc), status="tardy"
            ),
        ]
    )
    db_session.commit()
...
def test_boundaries_and_rounding(db_session):
    ...
    for _ in range(3):
        db_session.add(
            models.SpiritualMilestone(
                id=uuid.uuid4(), persona_id=persona_under.id, sede_id=sede.id, type="Milestone", event_date=date.today()
            )
        )
    for i in range(250):
        estado = "presente" if i < 47 else "falto"
        db_session.add(
            models.Asistencia(id=uuid.uuid4(), persona_id=persona_under.id, sesion_id=uuid.uuid4(), estado=estado)
        )
    db_session.commit()

    score_under, status_under = recalculate_and_persist_pastoral_health(db_session, persona_under.id)
    ...
    for _ in range(3):
        db_session.add(
            models.SpiritualMilestone(
                id=uuid.uuid4(), persona_id=persona_exact.id, sede_id=sede.id, type="Milestone", event_date=date.today()
            )
        )
    for i in range(100):
        estado = "presente" if i < 19 else "falto"
        db_session.add(
            models.Asistencia(id=uuid.uuid4(), persona_id=persona_exact.id, sesion_id=uuid.uuid4(), estado=estado)
        )
    db_session.commit()

    score_exact, status_exact = recalculate_and_persist_pastoral_health(db_session, persona_exact.id)
    ...
    persona_stable_high = models.Persona(
        id=uuid.uuid4(),
        first_name="StableHigh",
        last_name="Bound",
        email="stablehigh@example.com",
        sede_id=sede.id,
        is_baptized=False,
    )
    ...
    for _ in range(3):
        db_session.add(
            models.SpiritualMilestone(
                id=uuid.uuid4(),
                persona_id=persona_stable_high.id,
                sede_id=sede.id,
                type="Milestone",
    ...
    for _ in range(4):
        db_session.add(
            models.CommunicationLog(
                id=uuid.uuid4(), persona_id=persona_stable_high.id, channel="Call", content="Test", outcome="sent"
            )
        )
    for i in range(250):
        estado = "presente" if i < 147 else "falto"
        db_session.add(
            models.Asistencia(id=uuid.uuid4(), persona_id=persona_stable_high.id, sesion_id=uuid.uuid4(), estado=estado)
        )
    db_session.commit()

    score_sh, status_sh = recalculate_and_persist_pastoral_health(db_session, persona_stable_high.id)
    assert score_sh == 79
    ...
    for _ in range(3):
        db_session.add(
            models.SpiritualMilestone(
                id=uuid.uuid4(), persona_id=persona_comp.id, sede_id=sede.id, type="Milestone", event_date=date.today()
            )
        )
    for _ in range(4):
        db_session.add(
            models.CommunicationLog(
                id=uuid.uuid4(), persona_id=persona_comp.id, channel="Call", content="Test", outcome="sent"
            )
        )
    for i in range(100):
        estado = "presente" if i < 59 else "falto"
        db_session.add(
            models.Asistencia(id=uuid.uuid4(), persona_id=persona_comp.id, sesion_id=uuid.uuid4(), estado=estado)
        )
    db_session.commit()

    score_comp, status_comp = recalculate_and_persist_pastoral_health(db_session, persona_comp.id)
...
def test_multi_tenant_and_cross_persona_isolation(db_session):
    ...
    db_session.add(
        models.SpiritualMilestone(
            id=uuid.uuid4(), persona_id=persona_b.id, sede_id=sede_b.id, type="Milestone1", event_date=date.today()
        )
    )
    db_session.add(
        models.SpiritualMilestone(
            id=uuid.uuid4(), persona_id=persona_b.id, sede_id=sede_b.id, type="Milestone2", event_date=date.today()
        )
    )
    # Comm logs for B: 2 logs
    db_session.add(
        models.CommunicationLog(
            id=uuid.uuid4(), persona_id=persona_b.id, channel="WhatsApp", content="Hello B", outcome="sent"
        )
    )
    db_session.add(
        models.CommunicationLog(
            id=uuid.uuid4(), persona_id=persona_b.id, channel="Email", content="Hello B", outcome="sent"
        )
    )
