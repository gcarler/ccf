...
def full(client, db_session):
    ...
    for i, pr in enumerate(projects[:3]):
        for j in range(3):
            t = models.ProjectTask(project_id=pr.id, title=f"T{i}_{j}", status=["pending", "in_progress", "done"][j])
            db_session.add(t)
        db_session.add(
    ...
    for i, pr in enumerate(projects[:2]):
        db_session.add(
            models.ProjectComment(
                project_id=pr.id, author_id=str(personas[i].id), content=f"Comment on {pr.title}", is_resolved=i == 0
            )
        )
        db_session.add(
            ...
            type(
                ...
                (),
                {
                    ...
                    "base_attendees_with_roles": [
                        type(
                            "A", (), {"persona_id": str(personas[0].id), "role": "lider", "rol_personalizado_id": None}
                        )(),
                        type(
                            "A",
                            (),
                            {"persona_id": str(personas[1].id), "role": "asistente", "rol_personalizado_id": None},
                        )(),
                        type(
                            "A",
                            (),
                            {"persona_id": str(personas[2].id), "role": "anfitrion", "rol_personalizado_id": None},
                        )(),
                    ],
                    "base_attendee_ids": None,
                },
...
class TestProjectsFinal:
    def test_create_project(self, full):
        ...
        resp = c.post(
            "/api/projects",
            json={
                "title": f"Proj_{uuid.uuid4().hex[:6]}",
                "description": "Deep",
            },
            headers=h,
        )
    ...
    def test_supplies_crud(self, full):
        ...
        if tasks.status_code == 200 and tasks.json():
            ...
            if tid:
                ...
                resp = c.post(
                    f"/api/projects/{pid}/tasks/{tid}/supplies",
                    json={
                ...
                if resp.status_code in (200, 201):
                    ...
                    if sid:
                        c.patch(
                            f"/api/projects/{pid}/tasks/{tid}/supplies/{sid}",
                            json={
                                "item_name": "Updated",
                                "quantity": 10,
                            },
