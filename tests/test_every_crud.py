    ...
    for i, pr in enumerate(projects[:4]):
        ...
        db_session.add(
            models.ProjectComment(
                project_id=pr.id, author_id=str(personas[i].id), content=f"C{i}", is_resolved=i % 2 == 0
            )
        )
        db_session.add(
            models.ProjectActivityLog(
                project_id=pr.id, persona_id=str(personas[i].id), action_type="created", description=f"C {pr.title}"
            )
        )
    db_session.commit()
...
class TestCRMEveryFunction:
    def test_every_function(self, full):
        ...
        _c(
            ...
            db,
            PersonaCreate(
                first_name="Full", last_name="LongName", email=f"f_{uuid.uuid4().hex[:6]}@t.com", phone="+573009999999"
            ),
        )
        _c(
            crm.create_persona,
            db,
            PersonaCreate(first_name="Dup", last_name="DupPhone", phone=full["personas"][0].phone),
        )
...
class TestCMSEveryFunction:
    def test_every_function(self, full):
        ...
        _c(cms.list_cms_media_items, db, query="t")
        _c(cms.list_cms_sites, db)
...
class TestProjectsEveryFunction:
    def test_every_function(self, full):
        ...
        db = full["db"]
        _c(projects.get_projects, db)
        _c(projects.get_projects, db, sede_id=str(full["sede"].id))
        _c(projects.get_project_tasks, db, 1)
        _c(projects.get_project_phases, db, 1)
        _c(projects.get_project_comments, db, 1)
        _c(projects.get_project_milestones, db, 1)
        _c(projects.get_project_whiteboard, db, 1)
        _c(projects.get_project_wiki, db, 1)
        _c(projects.get_task_supplies, db, 1)
        _c(projects.get_task_attachments, db, 1)
        _c(projects.get_project_activities, db, 1)
        _c(projects.get_all_activities, db)
        _c(projects.get_all_activities, db, sede_id=str(full["sede"].id))
        _c(projects.get_portfolio_summary, db)
        _c(projects.get_workload_summary, db)
