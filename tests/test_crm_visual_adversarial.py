...
def test_reorder_cross_pipeline_leak_within_same_sede(client, db_session):
    ...
    pipeline_a = PipelineCRM(
        id=uuid.uuid4(), sede_id=sede.id, nombre="Pipeline A", tipo=TipoPipelineEnum.NUEVOS_VISITANTES, activo=True
    )
    # Create Pipeline B
    pipeline_b = PipelineCRM(
        id=uuid.uuid4(), sede_id=sede.id, nombre="Pipeline B", tipo=TipoPipelineEnum.CONSEJERIA, activo=True
    )
    db_session.add_all([pipeline_a, pipeline_b])
    db_session.flush()

    etapa_a = EtapaPipeline(id=uuid.uuid4(), pipeline_id=pipeline_a.id, nombre="Etapa A", orden=1)
...
def test_validate_empty_graph_fallback_to_db_cycles(client, db_session):
    ...
    auto2 = CrmAutomation(
        id=uuid.uuid4(), name="Auto 2", trigger_event="birthday", action_type="send_email", is_active=True
    )
...
def test_atomic_reorder_branching_eval_lacks_atomicity(db_session, monkeypatch):
    ...
    payload = [{"id": str(caso1_id), "sort_order": 1, "drag_target_etapa_id": str(etapa2_id)}]
...
def test_evaluate_condition_case_sensitivity_discrepancy(client, db_session):
    ...
    next_actions = (
        ...
        .filter(PendingCrmAction.automation_id == auto2.id, PendingCrmAction.target_persona_id == persona.id)
        .all()
    )
...
def test_enviar_plantilla_exception_leakage(client, db_session, monkeypatch):
    ...
    headers = auth_headers(client, email=admin.email)
    payload = {"destinatario_id": str(persona.id), "caso_id": str(caso1.id), "variables": {"name": "Juan"}}
