...
class TestNotificationsComprehensive:
    def test_with_real_data(self, full, db_session):
        ...
        leaders = []
        for i in range(2):
            p = models.Persona(id=uuid.uuid4(), first_name=f"Lead{i}", last_name="Tst", sede_id=s.id)
            db_session.add(p)
            db_session.flush()
        ...
        g_a = GrupoEvangelismo(
            id=uuid.uuid4(), nombre="Tomorrow Group", sede_id=s.id, lider_persona_id=leaders[0].id, activo=True
        )
        db_session.add(g_a)
        db_session.flush()
        ...
        ses_a = SesionGrupo(
            ...
            estado="PENDIENTE",
            tema_estudio="Test Session",
        )
        db_session.add(ses_a)

        # --- Grupo B: active but NO recent attendance report ---
        g_b = GrupoEvangelismo(
            id=uuid.uuid4(), nombre="Inactive Group", sede_id=s.id, lider_persona_id=leaders[1].id, activo=True
        )
        db_session.add(g_b)
        db_session.flush()
        ...
        ses_b = SesionGrupo(id=uuid.uuid4(), grupo_id=g_b.id, fecha_sesion=old_date, estado="REALIZADA")
    ...
    def test_empty_no_data(self, full):
        ...
        resp = full["c"].post("/api/evangelism/notifications/send-reminders", json={}, headers=full["h"])
