...
class TestCategorias:
    ...
    def test_create(self, full):
        ...
        resp = full["c"].post("/api/crm/resources/categorias", json={"nombre": name}, headers=full["h"])
    ...
    def test_update(self, full):
        name = f"Cat-{uuid.uuid4().hex[:6]}"
        r = full["c"].post("/api/crm/resources/categorias", json={"nombre": name}, headers=full["h"])
        cat_id = r.json()["id"]
        new_name = f"{name}-upd"
        resp = full["c"].patch(f"/api/crm/resources/categorias/{cat_id}", json={"nombre": new_name}, headers=full["h"])
        assert _ok(resp.status_code)
        assert resp.json()["nombre"] == new_name

    def test_update_not_found(self, full):
        resp = full["c"].patch(
            f"/api/crm/resources/categorias/{uuid.uuid4()}", json={"nombre": "Nope"}, headers=full["h"]
        )
...
class TestPlantillas:
    ...
    def test_update(self, full, db_session):
        ...
        pid = r.json()["id"]
        resp = full["c"].patch(
            f"/api/crm/resources/plantillas/{pid}", json={"titulo": "Updated Title"}, headers=full["h"]
        )
        assert _ok(resp.status_code)
    ...
    def test_update_not_found(self, full):
        resp = full["c"].patch(
            f"/api/crm/resources/plantillas/{uuid.uuid4()}", json={"titulo": "Nope"}, headers=full["h"]
        )
        assert resp.status_code == 404
...
class TestSystemTemplates:
    ...
    def test_apply_system_template(self, full):
        ...
        tid = templates[0]["id"]
        resp = full["c"].post("/api/crm/resources/system-templates/apply", json={"template_id": tid}, headers=full["h"])
        assert resp.status_code == 201
...
class TestAutomations:
    ...
    def test_update(self, full):
        ...
        aid = r.json()["id"]
        resp = full["c"].patch(
            f"/api/crm/resources/automations/{aid}", json={"name": "Updated Auto"}, headers=full["h"]
        )
        assert _ok(resp.status_code)
    ...
    def test_update_not_found(self, full):
        resp = full["c"].patch(
            f"/api/crm/resources/automations/{uuid.uuid4()}", json={"name": "Nope"}, headers=full["h"]
        )
        assert resp.status_code == 404
    ...
    def test_trigger_no_automations(self, full):
        resp = full["c"].post(
            "/api/crm/resources/automations/trigger",
