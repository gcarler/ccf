...
def _make_strategy(db, sede_id):
    ...
    s = EstrategiaEvangelismo(
        id=uuid.uuid4(), nombre="Estrategia CRUD", sede_id=sede_id,
        categoria_id=cat.id,
...
class TestExcusasCRUD:
    ...
    def test_get_motivos_excusa(self, full):
        crud.evangelism.create_motivo_excusa(
            full["db"], descripcion="Test", actor_user_id=full["admin"].id
        )
        full["db"].commit()
    ...
    def test_delete_motivo_excusa(self, full):
        excusa = crud.evangelism.create_motivo_excusa(
            full["db"], descripcion="Borrar", actor_user_id=full["admin"].id
        )
        full["db"].commit()
        result = crud.evangelism.delete_motivo_excusa(
            full["db"], excusa.id, actor_user_id=full["admin"].id
        )
        full["db"].commit()
    ...
    def test_delete_motivo_excusa_not_found(self, full):
        result = crud.evangelism.delete_motivo_excusa(
            full["db"], uuid.uuid4(), actor_user_id=full["admin"].id
        )
...
class TestSeguimientoCRUD:
    ...
    def _make_asistencia_with_session(self, full):
        ...
        g = GrupoEvangelismo(
            ...
            estrategia_id=strategy.id, sede_id=full["sede"].id,
            activo=True, capacidad=20,
        )
        full["db"].add(g)
        full["db"].flush()
        s = SesionGrupo(
            ...
            estado="REALIZADA", estado_habilitacion="HABILITADO",
        )
        full["db"].add(s)
        full["db"].flush()
        a = Asistencia(
            id=uuid.uuid4(), sesion_id=s.id, persona_id=full["persona"].id,
            estado="ASISTIO",
        )
        full["db"].add(a)
        full["db"].flush()
    ...
    def test_create_seguimiento(self, full):
        ...
        data = RegistroSeguimientoCreate(
            tipo="LLAMADA", observaciones="Llamada de seguimiento",
            asistencia_id=asistencia.id,
        )
        seg = crud.evangelism.create_seguimiento(
            full["db"], data=data, actor_user_id=full["admin"].id
        )
        full["db"].commit()
    ...
    def test_get_seguimientos(self, full):
        ...
        full["db"].commit()
        data = RegistroSeguimientoCreate(
            tipo="MENSAJE_WHATSAPP", observaciones="Mensaje",
            asistencia_id=asistencia.id,
        )
        crud.evangelism.create_seguimiento(
            full["db"], data=data, actor_user_id=full["admin"].id
        )
        full["db"].commit()
    ...
    def test_update_seguimiento(self, full):
        ...
        full["db"].commit()
        data = RegistroSeguimientoCreate(
            tipo="LLAMADA", observaciones="Original",
            asistencia_id=asistencia.id,
        )
        seg = crud.evangelism.create_seguimiento(
            full["db"], data=data, actor_user_id=full["admin"].id
        ...
        updated = crud.evangelism.update_seguimiento(
            full["db"], seg.id, update_data, actor_user_id=full["admin"].id
        )
        full["db"].commit()
        assert updated is not None
    ...
    def test_delete_seguimiento(self, full):
        ...
        full["db"].commit()
        data = RegistroSeguimientoCreate(
            tipo="ORACION", observaciones="Borrar",
            asistencia_id=asistencia.id,
        )
        seg = crud.evangelism.create_seguimiento(
            full["db"], data=data, actor_user_id=full["admin"].id
        ...
        full["db"].commit()
        result = crud.evangelism.delete_seguimiento(
            full["db"], seg.id, actor_user_id=full["admin"].id
        )
        full["db"].commit()
        assert result is True
    ...
    def test_get_pendientes_seguimiento(self, full):
        ...
        full["db"].commit()
        result = crud.evangelism.get_pendientes_seguimiento(
            full["db"], sede_id=str(full["sede"].id)
        )
...
class TestEstrategiaCRUDExtended:
    ...
    def test_delete_estrategia(self, full):
        s = _make_strategy(full["db"], full["sede"].id)
        full["db"].commit()
        result = crud.evangelism.delete_estrategia(
            full["db"], s.id, actor_user_id=full["admin"].id
        )
        full["db"].commit()
        assert result is True
