...
class TestSimpleCRUD:
    ...
    def test_create_shift(self, full):
        c, h = full["c"], full["h"]
        resp = c.post(
            "/api/evangelism/volunteers/shifts",
            json={
...
class TestComplexWrites:
    ...
    def test_update_prayer_request(self, full):
        c, h = full["c"], full["h"]
        resp = c.patch(
            "/api/evangelism/prayer-requests/1",
            json={
                "status": "in_progress",
            },
            headers=h,
    ...
    def test_apply_volunteer(self, full):
        c, h, personas = full["c"], full["h"], full["personas"]
        resp = c.post(
            "/api/evangelism/volunteers/apply",
            json={
