...
class TestInvoicePayments:
    def test_not_found(self, full):
        assert (
            full["c"]
            .post(
                f"/api/finance-suite/invoices/{uuid.uuid4()}/payments",
                json={
                    "amount": 100,
    ...
    def test_record(self, full, db_session):
        ...
        assert _ok(
            c.post(
                f"/api/finance-suite/invoices/{inv.id}/payments",
                json={
                    "amount": 500,
                    "payment_date": datetime.now(timezone.utc).date().isoformat(),
                    "payment_method": "transfer",
...
class TestElectronicInvoice:
    def test_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/invoices/{uuid.uuid4()}/send-electronic", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_not_configured(self, full, db_session):
        ...
        inv = models.Invoice(
            id=uuid.uuid4(),
            invoice_number=f"EI-{uuid.uuid4().hex[:6]}",
            customer_name="Test",
            subtotal=100,
            total=100,
            status="issued",
...
class TestExpenseWorkflow:
    def test_submit_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/expense-reports/{uuid.uuid4()}/submit", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_submit(self, full, db_session):
        ...
        r = models.ExpenseReport(
            id=uuid.uuid4(),
            report_number=f"EXP-{uuid.uuid4().hex[:6]}",
            employee_id=user.id,
            total_amount=100,
            status="draft",
            sede_id=sede.id,
    ...
    def test_submit_wrong_status(self, full, db_session):
        ...
        r = models.ExpenseReport(
            id=uuid.uuid4(),
            report_number=f"EW-{uuid.uuid4().hex[:6]}",
            employee_id=user.id,
            total_amount=100,
            status="submitted",
            sede_id=sede.id,
    ...
    def test_submit_wrong_owner(self, full, db_session):
        ...
        r = models.ExpenseReport(
            id=uuid.uuid4(),
            report_number=f"EWO-{uuid.uuid4().hex[:6]}",
    ...
    def test_approve_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/expense-reports/{uuid.uuid4()}/approve", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_approve_wrong_status(self, full, db_session):
        ...
        r = models.ExpenseReport(
            id=uuid.uuid4(),
            report_number=f"EA-{uuid.uuid4().hex[:6]}",
    ...
    def test_reject_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/expense-reports/{uuid.uuid4()}/reject", headers=full["h"]).status_code
            == 404
        )
    ...
    def test_reject_wrong_status(self, full, db_session):
        ...
        r = models.ExpenseReport(
            id=uuid.uuid4(),
            report_number=f"ER-{uuid.uuid4().hex[:6]}",
            employee_id=uuid.uuid4(),
            total_amount=100,
            status="draft",
            sede_id=sede.id,
...
class TestSignWorkflow:
    def test_send_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/sign-requests/{uuid.uuid4()}/send", headers=full["h"]).status_code
            == 404
        )

    def test_sign_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/sign-requests/{uuid.uuid4()}/sign", headers=full["h"]).status_code
            == 404
        )

    def test_decline_not_found(self, full):
        assert (
            full["c"].post(f"/api/finance-suite/sign-requests/{uuid.uuid4()}/decline", headers=full["h"]).status_code
            == 404
        )
...
class TestAccountingPost:
    def test_not_found(self, full):
        assert (
            full["c"].patch(f"/api/finance-suite/accounting-entries/{uuid.uuid4()}/post", headers=full["h"]).status_code
            == 404
        )

    def test_post(self, full, db_session):
        c, h, sede = full["c"], full["h"], db_session.query(models.Sede).first()
        ca = models.ChartOfAccount(
            id=uuid.uuid4(), code="2000", name="Revenue", account_type="revenue", sede_id=sede.id
        )
        db_session.add(ca)
        db_session.flush()
        ...
        db_session.flush()
        db_session.add(
            models.AccountingEntryLine(id=uuid.uuid4(), entry_id=entry.id, account_id=ca.id, debit=100, credit=0)
        )
        db_session.add(
            models.AccountingEntryLine(id=uuid.uuid4(), entry_id=entry.id, account_id=ca.id, debit=0, credit=100)
        )
        db_session.commit()
