...
def _parse_legacy_content(raw_content: str | None) -> Any:
    if not raw_content:
        return ""
...
def _legacy_content_to_html(page_key: str, title: str, raw_content: str | None) -> str:
    parsed = _parse_legacy_content(raw_content)
...
def _legacy_page_sections_payload(page_key: str, title: str, raw_content: str | None) -> dict[str, Any]:
    return {
        "layout": "grid",
        "columns": "1",
        "items": [
            {
                "type": "text",
                "content": _legacy_content_to_html(page_key, title, raw_content),
            }
        ],
    }
...
def backfill_cms_pages(db: Session, site_key: str = "faro") -> dict[str, int]:
    ...
    for legacy in legacy_rows:
        publication = publications.get(legacy.page_key)
        desired_status = _normalize_status(getattr(publication, "status", None))

        page = (
            db.query(models.CmsPage)
            .filter(models.CmsPage.site_id == site.id, models.CmsPage.slug == legacy.page_key)
            .first()
        )
        if not page:
            page = models.CmsPage(
                site_id=site.id,
                slug=legacy.page_key.strip().lower(),
        ...
        if not sections:
            section = models.CmsSection(
                page_id=page.id,
                section_key=f"{page.slug}-legacy-content",
                type="content_blocks",
                props_json=_legacy_page_sections_payload(legacy.page_key, legacy.title, legacy.content),
                sort_order=0,
                is_visible=True,
                status="active",
            )
        ...
        if not versions:
            version = models.CmsPageVersion(
                ...
                notes="Backfilled from legacy page_contents",
            )
            db.add(version)
            db.flush()
            created_versions += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill CMS v2 pages from legacy content")
