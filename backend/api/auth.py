...
def login(
    ...
    # Fallback por username si falla por email
    if not user:
    ...
    # El payload del JWT ahora usa UUID
...
def get_current_ministerial_user(current_user: models.User = Depends(require_active_user)):
    """Obtiene el perfil del usuario autenticado cruzando con Person."""
    person = current_user.person
    return {
        "user_id": str(current_user.id),
        "username": current_user.username,
        "email": person.email if person else "no-email@ccf.la",
