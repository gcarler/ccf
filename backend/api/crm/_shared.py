...
def _serialize_member_position(member_position: models.MemberPosition) -> dict:
    ...
    return {
        ...
        "start_date": member_position.start_date.isoformat() if member_position.start_date else None,
        "end_date": member_position.end_date.isoformat() if member_position.end_date else None,
        "is_active": member_position.is_active,
        "notes": member_position.notes,
        "created_at": member_position.created_at.isoformat() if member_position.created_at else None,
    }
...
def _serialize_message_group(logs: list[models.CommunicationLog]) -> dict:
    ...
    first = logs[0]
    delivered_count = sum(1 for item in logs if (item.outcome or "").lower() in {"sent", "delivered"})
    failed_count = sum(1 for item in logs if (item.outcome or "").lower() == "failed")
    target_count = len(logs)
    member_name = _member_full_name(first.member) if getattr(first, "member", None) else None
    campaign_name = first.campaign_name
    return {
        "id": first.id,
        "name": campaign_name or f"Mensaje a {member_name}" if member_name else "Mensaje",
