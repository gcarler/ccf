...
def get_member_ministries(
    ...
    rows = (
        ...
        .order_by(models.MemberMinistry.is_active.desc(), models.MemberMinistry.start_date.desc())
        .all()
    )
    result = []
    for mm in rows:
...
@router.patch("/members/{member_id}/positions/{member_position_id}", response_model=dict)
...
def assign_member_ministry(
    ...
    existing = db.query(models.MemberMinistry).filter(
...
def update_member_ministry(
    ...
    mm = db.query(models.MemberMinistry).filter(
