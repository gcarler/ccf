...
def register_maintenance(
    item_id: uuid.UUID, description: str, db: Session = Depends(get_db)
):
    """Registra un log de mantenimiento para un equipo."""
    return crud.create_maintenance_log(
        db, item_id=item_id, description=description, service_date=dt.date.today()
    )
...
def list_maintenance_tasks(db: Session = Depends(get_db)):
    ...
    logs = (
        ...
        .order_by(models.MaintenanceLog.service_date.desc())
        .all()
    )
    result = []
    for l in logs:
