...
def get_dashboard_metrics(db: Session):
    ...
    certificates_issued = (
        db.query(models.Enrollment).filter(models.Enrollment.certificate_issued).count()
    )

    formal_total = (
        db.query(models.Enrollment)
        .join(models.Course)
        .filter(models.Course.modality == "formal")
        .count()
    )
    formal_comp = (
        db.query(models.Enrollment)
    ...
    nf_total = (
        db.query(models.Enrollment)
        .join(models.Course)
        .filter(models.Course.modality == "no_formal")
        .count()
    )
    nf_comp = (
        db.query(models.Enrollment)
    ...
    top_courses_rows = (
        db.query(
            models.Course.title, func.count(models.Enrollment.id).label("enroll_count")
        )
        .join(models.Enrollment)
        .group_by(models.Course.id)
        .order_by(text("enroll_count DESC"))
        .limit(5)
        .all()
    )
    ...
    return {
        ...
        "formal_stats": {
            ...
            "rate": round(
                (formal_comp / formal_total * 100) if formal_total > 0 else 0, 2
            ),
