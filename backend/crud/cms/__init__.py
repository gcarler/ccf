...
def _commit_or_conflict(db: Session) -> bool:
    ...
    ``backend.crud._utils._commit_or_conflict_bool`` — single source of
    truth compartido con ``api/cms_v2/_shared.py`` y ``crud/academy.py``
    (consolidación de las 3 copias del helper, 2026-08-05).
    """
    from backend.crud._utils import _commit_or_conflict_bool

    return _commit_or_conflict_bool(db)
...
# ── CMS Posts & Taxonomías (→ backend/crud/cms/posts_and_taxonomies.py) ─────────
from backend.crud.cms.posts_and_taxonomies import (  # noqa: E402
