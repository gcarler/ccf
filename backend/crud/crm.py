...
def get_evangelism_strategies(db, skip: int = 0, limit: int = 100):
    from backend.models_evangelism import EstrategiaEvangelismo
    return db.query(EstrategiaEvangelismo).offset(skip).limit(limit).all()


def create_evangelism_strategy(db, strategy):
    from backend.models_evangelism import EstrategiaEvangelismo
    from backend.schemas.evangelism import EstrategiaEvangelismoCreate
    if isinstance(strategy, EstrategiaEvangelismoCreate):
        data = strategy.model_dump()
    else:
        data = strategy
    db_obj = EstrategiaEvangelismo(**data)
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update_evangelism_strategy(db, strategy_id, strategy):
    from backend.models_evangelism import EstrategiaEvangelismo
    db_obj = db.query(EstrategiaEvangelismo).filter(EstrategiaEvangelismo.id == strategy_id).first()
    if not db_obj:
        return None
    if hasattr(strategy, 'model_dump'):
        update_data = strategy.model_dump(exclude_unset=True)
    else:
        update_data = strategy
    for key, value in update_data.items():
...
def delete_evangelism_strategy(db, strategy_id) -> bool:
    from backend.models_evangelism import EstrategiaEvangelismo
    db_obj = db.query(EstrategiaEvangelismo).filter(EstrategiaEvangelismo.id == strategy_id).first()
    if not db_obj:
        return False
