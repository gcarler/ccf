...
def get_project_phases(db: Session, project_id: int):
    ...
    ).order_by(models.ProjectPhase.order_index).all()


def set_project_phases(db: Session, project_id: int, phases: list[dict]) -> list[models.ProjectPhase]:
...
def get_project_comments(db: Session, project_id: int | None = None, task_id: int | None = None):
...
def create_comment(db: Session, project_id: int, author_id: int, content: str, task_id: int | None = None):
...
def update_comment(db: Session, comment_id: int, content: str) -> Optional[models.ProjectComment]:
    row = db.query(models.ProjectComment).filter(models.ProjectComment.id == comment_id).first()
    if not row:
        return None
    row.content = content
    db.commit()
...
def get_project_milestones(db: Session, project_id: int):
    return db.query(models.ProjectMilestone).filter(
        models.ProjectMilestone.project_id == project_id
    ).order_by(models.ProjectMilestone.due_date.asc()).all()
...
def create_milestone(db: Session, project_id: int, title: str, description: str | None = None,
                     due_date=None, status: str = "pending"):
...
def update_milestone(db: Session, milestone_id: int, payload: schemas.ProjectMilestoneUpdate) -> Optional[models.ProjectMilestone]:
    row = db.query(models.ProjectMilestone).filter(models.ProjectMilestone.id == milestone_id).first()
    if not row:
        return None
    for key, value in payload.model_dump(exclude_unset=True).items():
        setattr(row, key, value)
...
def get_task_attachments(db: Session, task_id: int):
    return db.query(models.ProjectAttachment).filter(
        models.ProjectAttachment.task_id == task_id
    ).order_by(models.ProjectAttachment.created_at.desc()).all()
...
def create_attachment(db: Session, task_id: int, file_url: str, file_name: str,
                      file_size: int = 0, mime_type: str | None = None, uploader_id: int | None = None):
...
def delete_attachment(db: Session, attachment_id: int) -> bool:
    row = db.query(models.ProjectAttachment).filter(models.ProjectAttachment.id == attachment_id).first()
    if not row:
        return False
    db.delete(row)
    db.commit()
...
def get_project_whiteboard(db: Session, project_id: int):
    return db.query(models.ProjectWhiteboard).filter(
        models.ProjectWhiteboard.project_id == project_id
    ).first()


def update_project_whiteboard(db: Session, project_id: int, payload: schemas.ProjectWhiteboardUpdate) -> models.ProjectWhiteboard:
    row = db.query(models.ProjectWhiteboard).filter(
        models.ProjectWhiteboard.project_id == project_id
    ).first()
    if not row:
        row = models.ProjectWhiteboard(project_id=project_id, canvas_data=payload.canvas_data or {})
        db.add(row)
    else:
...
def get_project_wiki(db: Session, project_id: int):
    return db.query(models.ProjectDocument).filter(
        models.ProjectDocument.project_id == project_id,
        models.ProjectDocument.doc_type == "wiki"
    ).first()


def update_project_wiki(db: Session, project_id: int, content: str, updated_by: int | None = None) -> models.ProjectDocument:
    row = db.query(models.ProjectDocument).filter(
        models.ProjectDocument.project_id == project_id,
        models.ProjectDocument.doc_type == "wiki"
    ).first()
    if not row:
...
def get_task_supplies(db: Session, task_id: int):
    ...
    ).order_by(models.TaskSupply.created_at.asc()).all()
...
def create_supply(db: Session, task_id: int, name: str, quantity: int = 1,
                  estimated_cost: float = 0.0, status: str = "pending", requested_by: int | None = None):
...
def get_project_activities(db: Session, project_id: int, limit: int = 100):
    return db.query(models.ProjectActivityLog).filter(
        models.ProjectActivityLog.project_id == project_id
    ).order_by(models.ProjectActivityLog.created_at.desc()).limit(limit).all()


def create_activity_log(db: Session, project_id: int, action: str, user_id: int | None = None,
...
def update_inbox_state(db: Session, user_id: int, item_id: int, is_read: bool = True):
    row = db.query(models.ProjectInboxState).filter(
        models.ProjectInboxState.user_id == user_id,
...
def get_portfolio_summary(db: Session):
    ...
    ).outerjoin(models.ProjectTask).group_by(models.Project.status).all()
...
def get_workload_summary(db: Session):
    ...
    rows = db.query(
        ...
        sa_func.count(models.ProjectTask.id),
        sa_func.sum(sa_func.case((models.ProjectTask.status == "done", 1), else_=0))
    ).filter(
