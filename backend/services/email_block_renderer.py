...
def _render_block(block: Dict[str, Any], brand: Dict[str, str]) -> str:
    ...
    if btype == "header":
        ...
        subtitle_html = (
            f'<p style="margin:4px 0 0;font-size:14px;opacity:0.7;">{_esc(str(subtitle))}</p>' if subtitle else ""
        )
    ...
    if btype == "verse":
        ref_html = (
            f'<p style="margin:8px 0 0;font-size:12px;font-weight:bold;color:{dark};letter-spacing:2px;text-transform:uppercase;">&mdash; {_esc(str(props.get("reference", "")))}</p>'
    ...
    if btype == "columns":
        ...
        cw = 100 / count
        cols = "".join(
            f'<td style="width:{cw:.1f}%;padding:0 4px;vertical-align:top;border:1px dashed #e5e7eb;min-height:40px;"></td>'
