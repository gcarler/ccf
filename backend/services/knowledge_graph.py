...
from typing import Any, Dict, List, Set, Iterable, Optional, cast
...
def _lead_nodes(db: Session, limit: int) -> List[Dict[str, Any]]:
    leads = db.query(models.ConsolidationPipeline).order_by(models.ConsolidationPipeline.created_at.desc()).limit(limit).all()
    nodes: List[Dict[str, Any]] = []
    for lead in leads:
        nodes.append({
            "id": f"lead-{lead.id}",
...
def build_graph_snapshot(db: Session, limit: int = 50, types: Optional[Iterable[str]] = None) -> Dict[str, Any]:
    nodes: List[Dict[str, Any]] = []
    ...
    # Lead -> Pastor edges
    leads = db.query(models.ConsolidationPipeline).filter(models.ConsolidationPipeline.assigned_pastor_id.isnot(None)).limit(limit).all()
    for lead in leads:
