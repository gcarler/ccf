...
for i, line in enumerate(lines):
    ...
    if "@app.post(" in line_text and (
        "/lessons/" in line_text or "\\" in line_text or "/" in line_text
    ):
        if "resources" in line_text or "\\" in line_text:
            ...
            new_lines.append(
                b"    current_user: models.User = Depends(require_staff_or_admin)\n"
            )
            new_lines.append(b"):\n")
            new_lines.append(b"    # Save file\n")
            new_lines.append(
                b'    file_path = f"uploads/res_{lesson_id}_{file.filename}"\n'
            )
            ...
            new_lines.append(
                b'        file_url=f"/static/res_{lesson_id}_{file.filename}",\n'
            )
