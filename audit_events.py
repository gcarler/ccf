...
event_routes = [(m, r) for m, r in routes if any(k in r.lower() for k in ['event', 'faro', 'session', 'visitor', 'scanner'])]
...
print("day_of_week in GloryHouse:", 'day_of_week' in models_f[models_f.find('class GloryHouse'):models_f.find('class GloryHouse')+500])
