...
def seed_evangelism_v2():
    ...
    if 'parent_id' not in columns:
        ...
        cursor.execute("ALTER TABLE project_tasks ADD COLUMN parent_id INTEGER REFERENCES project_tasks(id) ON DELETE CASCADE")
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, ("Plan de Invasión Urbana 2026", "Estrategia masiva de alcance territorial y servicio social en zonas críticas.", "active", 3, "#ef4444", "MapPin", now.isoformat(), now.isoformat()))
    p1_id = cursor.lastrowid
    
    # Milestones (Levels) for P1
    milestones_p1 = [
        ("Preparación Espiritual", "Cadena de ayuno y oración por los equipos.", 15),
        ("Incursión Territorial", "Despliegue de brigadas en los barrios asignados.", 30),
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (p1_id, "Entrenamiento de Brigadas", "Capacitación técnica y espiritual para los voluntarios.", "todo", "high", 3, 0, now.isoformat(), now.isoformat()))
    act1_id = cursor.lastrowid
    
    # Subtasks for Activity 1
    subtasks_act1 = [
        ("Clase de Primeros Auxilios Espirituales", "Cómo abordar crisis emocionales durante el evangelismo.", 4),
        ("Mapeo de zonas calientes", "Identificación de puntos estratégicos en Barrio Sur.", 1)
    ]
    ...
    cursor.execute("""
        INSERT INTO project_tasks (project_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (p1_id, "Operativo de Compasión", "Acciones directas de servicio en la comunidad.", "todo", "urgent", 1, 1, now.isoformat(), now.isoformat()))
    ...
    for st in subtasks_act2:
        cursor.execute("""
            INSERT INTO project_tasks (project_id, parent_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (p1_id, act2_id, st[0], st[1], "todo", "high", st[2], 0, now.isoformat(), now.isoformat()))
    ...
    cursor.execute("""
        INSERT INTO projects (title, description, status, owner_id, color, icon, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, ("Misión Digital: Redes de Gracia", "Proyecto de evangelismo en plataformas digitales y redes sociales.", "active", 4, "#3b82f6", "Share2", now.isoformat(), now.isoformat()))
    p2_id = cursor.lastrowid
    
    # Milestones (Levels) for P2
    milestones_p2 = [
        ("Arquitectura de Contenido", "Definición de pilares de contenido y diseño visual.", 10),
    ...
    cursor.execute("""
        INSERT INTO project_tasks (project_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (p2_id, "Campaña TikTok 'Mi Cambio'", "Testimonios cortos de transformación de vida.", "in_progress", "medium", 4, 0, now.isoformat(), now.isoformat()))
    act3_id = cursor.lastrowid
    
    # Subtasks for Activity 1
    subtasks_act3 = [
        ("Edición de 10 testimonios", "Grabar y editar videos de máximo 60 segundos.", 4),
        ("Gestión de influencers locales", "Contactar jóvenes líderes para amplificar el mensaje.", 1)
    ]
    for st in subtasks_act3:
        cursor.execute("""
            INSERT INTO project_tasks (project_id, parent_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (p2_id, act3_id, st[0], st[1], "todo", "normal", st[2], 0, now.isoformat(), now.isoformat()))

    # Activity 2: Célula Digital (Zoom)
    cursor.execute("""
        INSERT INTO project_tasks (project_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (p2_id, "Célula Digital (Zoom)", "Reuniones virtuales de estudio bíblico y acogida.", "todo", "high", 3, 1, now.isoformat(), now.isoformat()))
    act4_id = cursor.lastrowid
    
    # Subtasks for Activity 2
    subtasks_act4 = [
        ("Capacitación de anfitriones", "Taller sobre manejo de salas y dinámicas online.", 3),
        ("Diseño de guía de estudio PDF", "Material descargable para los participantes.", 4)
    ]
    for st in subtasks_act4:
        cursor.execute("""
            INSERT INTO project_tasks (project_id, parent_id, title, description, status, priority, assignee_id, order_index, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (p2_id, act4_id, st[0], st[1], "todo", "medium", st[2], 0, now.isoformat(), now.isoformat()))
