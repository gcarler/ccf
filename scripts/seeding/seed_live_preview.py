...
def seed_live_preview():
    ...
    try:
        ...
        c1 = models.Course(
            code="PAST-01",
            title="Liderazgo Pastoral I",
            modality="formal",
            xp_per_lesson=50,
        )
        c2 = models.Course(
            code="BIBL-02",
            title="Hermenéutica Avanzada",
            modality="no_formal",
            xp_per_lesson=30,
        )
        db.add_all([c1, c2])
        ...
        enr = models.Enrollment(
            user_id=u1.id, course_id=c1.id, progress_percent=95, status="active"
        )
        db.add(enr)
        db.commit()
        ...
        t1 = models.ForumThread(
            title="Duda sobre Romanos 8:28", category="Teología", author_id=u1.id
        )
        db.add(t1)
        db.commit()
