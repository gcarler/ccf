...
def seed_total():
    ...
    try:
        ...
        for lesson in lessons:
            ...
            res = models.Resource(
                lesson_id=lesson.id,
                title=f"Guía de Estudio: {lesson.title}",
                file_url="/static/guide.pdf",
            ...
            asm = models.Assessment(
                lesson_id=lesson.id, title=f"Quiz: {lesson.title}", min_score=70
            )
            db.add(asm)
            db.commit()
            db.refresh(asm)
            ...
            q = models.AssessmentQuestion(
                ...
                question_text="¿Es este un contenido ministerial?",
                points=100,
            )
            db.add(q)
            db.commit()
            ...
            db.add_all(
                [
                    models.AssessmentOption(
                        question_id=q.id, option_text="Sí", is_correct=True
                    ),
                    models.AssessmentOption(
                        question_id=q.id, option_text="No", is_correct=False
                    ),
                ]
            )
        ...
        if len(courses) >= 2:
            db.add(
                models.CoursePrerequisite(
                    course_id=courses[1].id, prerequisite_course_id=courses[0].id
                )
            )
        ...
        for enr in enrollments[:20]:
            ...
            sub = models.AssignmentSubmission(
                ...
                lesson_id=lessons[0].id,
                file_url="/static/tarea.pdf",
                grade=95,
                teacher_feedback="Excelente trabajo.",
            )
            db.add(sub)
        ...
        # Pipeline
        for i in range(10):
            p = models.ConsolidationPipeline(
                first_name=f"Nuevo_{i}",
                last_name="Prospecto",
                phone="555-0000",
                stage="new",
                source="Web",
        ...
        thread = models.ForumThread(
            title="Bienvenidos al Foro", category="General", author_id=users[0].id
        )
        db.add(thread)
        db.commit()
        db.refresh(thread)
        db.add(
            models.ForumComment(
                ...
                content="Gracias por este espacio.",
            )
        )
        ...
        db.add(
            models.ChatMessage(
                sender_id=users[0].id, content="Hola a todos los líderes."
            )
        )
        ...
        proj = models.Project(
            title="Remodelación Auditorio", status="active", owner_id=users[0].id
        )
        db.add(proj)
        db.commit()
        db.refresh(proj)
        ...
        db.add(
            models.TaskSupply(task_id=task.id, item_name="Pintura Blanca", quantity=5)
        )
        db.add(
            models.InventoryItem(
                name="Sillas", category="Mobiliario", stock=500, status="Available"
            )
        )
        ...
        db.add(
            models.AdminAuditLog(
                actor_user_id=users[0].id, action="seed_total", resource_type="system"
            )
        )
        db.add(
            models.Notification(
                user_id=users[0].id,
                title="Bienvenida",
                content="Bienvenido a la nueva plataforma CCF.",
            )
        )
        db.add(
            models.PageContent(
                page_key="home",
                title="Inicio Ministerial",
        ...
        db.add(
            models.MediaAsset(
                filename="logo.png", url="/static/logo.png", size_bytes=1024
            )
        )
        db.add(
