...
def seed_mass_data():
    ...
    apellidos = ["Rodríguez", "García", "Martínez", "López", "González", "Pérez", "Sánchez", "Romero", "Sosa", "Torres", "Ramírez", "Ruiz", "Díaz", "Vargas", "Morales", "Castillo", "Jiménez", "Reyes", "Gutiérrez", "Castro"]
    nombres_m = ["Juan", "Carlos", "Ricardo", "Felipe", "Mateo", "Andrés", "Marcos", "David", "Jorge", "Pedro", "Santiago", "Samuel", "Daniel", "Gabriel", "Mateo", "Tomás", "Lucas", "Simón", "Matías", "Javier"]
    nombres_f = ["Elena", "María", "Sofía", "Lucía", "Isabel", "Camila", "Valeria", "Mariana", "Paola", "Claudia", "Beatriz", "Raquel", "Marta", "Silvia", "Adriana", "Gloria", "Victoria", "Natalia", "Diana", "Laura"]
    ...
    try:
        ...
        for i in range(200):
            ...
            familias.append(fam)
        db.commit()
        for f in familias: db.refresh(f)
        ...
        for i in range(600):
            ...
            if usuarios_creados < 100:
                ...
                if rol == "Pastor de Zona": user_role = "docente"
                if rol == "Líder": user_role = "coordinador"
            ...
            if i % 100 == 0: print(f"   ... {i} personas creadas")
            
        db.commit()

        # 4. Generar 10 Casas de Gloria
        print("🏘️ Creando 10 Casas de Gloria...")
        nombres_casas = ["Bethel", "Sion", "Ebenezer", "Jehová Jireh", "Monte de los Olivos", "Getsemaní", "Peniel", "Salem", "Mizpa", "Beraca"]
        for i in range(10):
            pastor = random.choice([m for m in miembros if m.church_role in ["Líder", "Pastor de Zona"]])
