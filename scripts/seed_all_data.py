...
FIRST_NAMES_MALE = [
    "Juan", "Carlos", "José", "Luis", "Fernando", "Francisco", "Pedro", "Jorge", "Miguel", "Javier",
    "Alejandro", "Manuel", "Roberto", "Raúl", "Diego", "David", "Daniel", "Alberto", "Andrés", "Mario",
    "Santiago", "Gabriel", "Hugo", "Sebastián", "Mateo", "Nicolás", "Samuel", "Ángel", "Tomás", "Ricardo",
    "Pablo", "Eduardo", "Héctor", "Sergio", "Rafael", "Antonio", "Marcos", "Adrian", "Iván", "Gustavo",
    "César", "Rubén", "Víctor", "Leonardo", "Martín", "Gonzalo", "Joaquín", "Mauricio", "Rodrigo", "Esteban"
]

FIRST_NAMES_FEMALE = [
    "María", "Ana", "Luisa", "Carmen", "Isabel", "Teresa", "Francisca", "Elena", "Marta", "Laura",
    "Cristina", "Silvia", "Patricia", "Andrea", "Mónica", "Claudia", "Sandra", "Alicia", "Beatriz", "Gloria",
    "Sofía", "Camila", "Valentina", "Isabella", "Gabriela", "Daniela", "Mariana", "Paula", "Natalia", "Clara",
    "Valeria", "Jimena", "Lucía", "Sara", "Liliana", "Diana", "Adriana", "Sonia", "Olga", "Cecilia",
    "Rosa", "Juana", "Luz", "Ángela", "Julia", "Victoria", "Carolina", "Estela", "Paola", "Diana"
]
...
LAST_NAMES = [
    ...
    "Herrera", "Aguirre", "Gutiérrez", "Castro", "Molina", "Silva", "Ortiz", "Delgado", "Mendoza", "Salazar",
    "Rojas", "Guerrero", "Paredes", "Ramos", "Suárez", "Morales", "Ortega", "Castillo", "Vargas", "Guzmán",
    "Méndez", "Núñez", "Cabrera", "Domínguez", "Herrera", "Blanco", "Jiménez", "Marín", "Cruz", "Ortiz"
]

ZONES = ["Zona Norte", "Zona Sur", "Zona Centro", "Zona Este", "Zona Oeste"]
DAYS_OF_WEEK = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"]
TIME_SLOTS = [("19:00", "20:30"), ("19:30", "21:00"), ("20:00", "21:30"), ("18:30", "20:00")]

TALENTS = [
    "Música (Guitarra)", "Música (Teclado)", "Canto", "Tecnología y Redes", "Cocina y Repostería",
    "Enseñanza", "Administración", "Diseño Gráfico", "Logística y Protocolo", "Fotografía",
    "Manualidades", "Primeros Auxilios", "Sonido y Multimedia"
]

SPIRITUAL_GIFTS = [
    "Liderazgo", "Misericordia", "Sabiduría", "Profecía", "Enseñanza", "Fe", "Servicio",
    "Evangelismo", "Administración", "Exhortación", "Dar con liberalidad", "Pastoreo"
]

COUNSELING_SUBJECTS = [
    "Crisis de pareja y comunicación", "Orientación vocacional para jóvenes", "Fortalecimiento espiritual y dudas",
    "Proceso de duelo familiar", "Superación de adicciones y malos hábitos", "Sanidad interior y perdón",
    "Manejo de ansiedad y estrés", "Finanzas familiares y principios bíblicos", "Restauración de relaciones padres-hijos"
]
...
def seed_database():
    ...
    try:
        ...
        levels = []
        for i in range(1, 6):
            lvl = models.Level(title=f"Nivel {i}", min_xp=(i-1)*1000, icon_key=f"level_{i}")
            db.add(lvl)
            levels.append(lvl)
        db.commit()
        ...
        status_weights = ["Servidor"] * 10 + ["Discípulo"] * 25 + ["Creyente"] * 40 + ["Nuevo"] * 25
        ...
        for idx in range(1, 501):
            ...
            email_local = email_local.replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u").replace("ñ", "n")
            ...
            if age < 18:
                ...
                pastoral_notes = f"Joven soltero/a. Edad: {age} años. Interesado en misiones."
            elif age < 60:
                pastoral_notes = f"Adulto. Edad: {age} años. Integrante activo de su hogar."
            else:
                pastoral_notes = f"Adulto mayor. Edad: {age} años. Aporta sabiduría al grupo."
        ...
        for m in members:
            ...
            if m.spiritual_status in ["Servidor", "Discípulo"] or (m.spiritual_status == "Creyente" and random.random() < 0.5):
                username_base = f"{m.first_name.lower()}.{m.last_name.split()[0].lower()}"
                username_base = username_base.replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u").replace("ñ", "n")
        ...
        house_names = [
            "Faro Bethel", "Faro Peniel", "Faro Ebenezer", "Faro Getsemaní", "Faro Sinaí", 
            "Faro Hermón", "Faro Jordán", "Faro Sión", "Faro Siloé", "Faro Betania", 
            "Faro Galilea", "Faro Capernaum", "Faro Emaús", "Faro Salem", "Faro Filadelfia", 
            "Faro Antioquía", "Faro Éfeso", "Faro Esmirna", "Faro Pérgamo", "Faro Tiatira", 
            "Faro Sardis", "Faro Laodicea", "Faro de Luz", "Faro de Esperanza", "Faro de Fe"
        ]
        
        for i, name in enumerate(house_names):
            code = f"FARO-{str(i+1).padStart(3, '0')}" if hasattr(str, 'padStart') else f"FARO-{str(i+1).zfill(3)}"
            zone = random.choice(ZONES)
            ...
            if leader.id == assistant.id:
                assistant = members[(m.id for m in members if m.id != leader.id).__next__()]
            if host.id == leader.id or host.id == assistant.id:
                host = members[(m.id for m in members if m.id not in [leader.id, assistant.id]).__next__()]
        ...
        for h in glory_houses:
            if h.leader_id: core_roles_ids.add(h.leader_id)
            if h.assistant_id: core_roles_ids.add(h.assistant_id)
            if h.host_id: core_roles_ids.add(h.host_id)
        ...
        for h in glory_houses:
            ...
            num_to_assign = min(random.randint(8, 18), len(assignable_members) - member_idx)
            if num_to_assign <= 0:
                break
        ...
        courses_data = [
            ("ACAD-001", "Fundamentos de la Fe", "Estudio de las verdades fundamentales del evangelio y la doctrina cristiana básica.", "presencial", True, 20),
            ("ACAD-002", "Discipulado I: Vida Cristiana", "Principios prácticos para el crecimiento diario, oración, lectura de la palabra y testimonio.", "virtual", True, 30),
            ("ACAD-003", "Discipulado II: Carácter Cristiano", "Desarrollo del carácter a través de la obediencia, mayordomía y vida comunitaria.", "virtual", True, 30),
            ("ACAD-004", "Liderazgo Ministerial", "Capacitación avanzada para servir en los diferentes ministerios de la iglesia y guiar a otros.", "presencial", True, 40),
            ("ACAD-005", "Consejería Familiar", "Herramientas bíblicas y psicológicas para orientar a familias en crisis y restaurar relaciones.", "presencial", True, 35),
            ("ACAD-006", "Teología Práctica", "Profundización doctrinal aplicada a los desafíos contemporáneos de la iglesia hoy.", "virtual", True, 45)
        ]
        ...
        users_pool = db.query(models.User).filter(models.User.role == 'estudiante').all()
        ...
        for idx, m in enumerate(counseling_pool):
            ...
            notes_text = f"Detalle de la solicitud: {subject}. " + random.choice(COUNSELING_NOTES)
        ...
        for h in glory_houses:
            ...
            gh_members = db.query(models.GloryHouseMember).filter(models.GloryHouseMember.glory_house_id == h.id).all()
            if not gh_members:
                continue
            ...
            for s_idx, s_date in enumerate(session_dates):
                ...
                for ghm in gh_members:
                    ...
                    attendance = models.GloryHouseAttendance(
                        ...
                        scanned_at=datetime.combine(s_date, datetime.min.time()) + timedelta(hours=19, minutes=random.randint(0, 20))
                    )
                    db.add(attendance)
                    attendance_records_count += 1
                    
        db.commit()
        print(f"Created weekly sessions and {attendance_records_count} attendance records.")
