...
def verify_new_logic():
    ...
    try:
        ...
        print(f"🧪 PRUEBA 1: Intentando matricular a {student.username} en {c2.title} (Debe fallar)...")
        try:
            crud.create_enrollment(db, schemas.EnrollmentCreate(user_id=student.id, course_id=c2.id))
        ...
        enrollment_base = crud.create_enrollment(db, schemas.EnrollmentCreate(user_id=student.id, course_id=c1.id))
        ...
        print(f"🧪 PRUEBA 3: Reintentando matricular en {c2.title} tras completar prerrequisito...")
        try:
            enrollment_adv = crud.create_enrollment(db, schemas.EnrollmentCreate(user_id=student.id, course_id=c2.id))
            print(f"✅ ÉXITO: Matrícula permitida tras cumplir prerrequisitos (ID: {enrollment_adv.id}).")
        except ValueError as e:
            print(f"❌ ERROR: La matrícula falló a pesar de cumplir prerrequisitos. Error: {str(e)}")

        # 8. TEST: Evaluación y Certificado
        print("📜 PRUEBA 4: Simulando aprobación de examen final y emisión de certificado...")
