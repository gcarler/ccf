...
def perform_forensic_audit():
    ...
    try:
        ...
        users_sample = [{"id": u.id, "role": u.role} for u in db.query(models.User).limit(5).all()]
        projects_sample = [{"id": p.id, "title": p.title} for p in db.query(models.Project).limit(5).all()]
