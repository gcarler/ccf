...
def check_platform_health():
    ...
    try:
        login_resp = requests.post(
            ...
            data={"username": "luis.meza@ccf.la", "password": "admin123"},
        )
        ...
        for name, endpoint in views_to_test:
            ...
            if resp.status_code == 200:
                data_count = (
                    len(resp.json()) if isinstance(resp.json(), list) else "Datos OK"
                )
        ...
        print(
            "Ahora puedes abrir tu navegador en http://localhost:3000 y verás las vistas llenas de datos."
        )
