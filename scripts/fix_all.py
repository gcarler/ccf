...
print("\n2. Database")
out, _, _ = run(
    "PGPASSWORD=ccf_password_secret_123 psql -h localhost -U ccf_admin -d ccf_db -t -A -c \"SELECT data_type FROM information_schema.columns WHERE table_name='personas' AND column_name='id'\""
)
print(f"   {OK if out == 'uuid' else FAIL} personas.id = {out}")

out, _, _ = run(
    'PGPASSWORD=ccf_password_secret_123 psql -h localhost -U ccf_admin -d ccf_db -t -A -c "SELECT count(*) FROM personas"'
)
print(f"   {OK} personas: {out} rows")

out, _, _ = run(
    "PGPASSWORD=ccf_password_secret_123 psql -h localhost -U ccf_admin -d ccf_db -t -A -c \"SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE'\""
)
...
out, _, _ = run(
    r"pm2 logs ccf-backend-staging --lines 20 --nostream 2>&1 | grep -i 'error\|traceback\|cannot import' | tail -5"
)
if out:
