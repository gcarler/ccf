...
def seed_prereqs():
    ...
    try:
        ...
        if not c1:
            c1 = models.Course(code="FUND1", title="Fundamentos I", modality="formal", is_published=True)
            db.add(c1)
            db.commit()
            db.refresh(c1)
        ...
        if not c2:
            c2 = models.Course(code="FUND2", title="Fundamentos II", modality="formal", is_published=True)
            db.add(c2)
            db.commit()
            db.refresh(c2)

        # 2. Add prerequisite: FUND2 needs FUND1
        exists = db.query(models.CoursePrerequisite).filter(
