...
def run_integrity_audit() -> int:
    ...
    try:
        ...
        events_without_name = (
            ...
            .filter(or_(models.CrmEvent.name.is_(None), func.trim(models.CrmEvent.name) == ""))
            .scalar()
            or 0
        )
        cancelled_without_reason = (
            ...
            .filter(
                ...
                or_(models.CrmEvent.cancellation_reason.is_(None), func.trim(models.CrmEvent.cancellation_reason) == ""),
            )
            .scalar()
            or 0
        )
        ...
        attendance_orphan_event = (
            ...
            .filter(~exists().where(models.CrmEvent.id == models.EventAttendance.event_id))
            .scalar()
            or 0
        )
        attendance_orphan_member = (
            db.query(func.count(models.EventAttendance.id))
            .filter(~exists().where(models.Member.id == models.EventAttendance.member_id))
            .scalar()
            or 0
        )
        attendance_status_mismatch = (
            ...
            .filter(
                or_(
                    and_(models.EventAttendance.attended.is_(True), models.EventAttendance.status != "present"),
                    and_(models.EventAttendance.attended.is_(False), models.EventAttendance.status != "absent"),
                )
            )
            .scalar()
            or 0
        ...
        print(f"- Asistencias huérfanas (evento inexistente): {attendance_orphan_event}")
        print(f"- Asistencias huérfanas (miembro inexistente): {attendance_orphan_member}")
        ...
        print(f"- Presentes en eventos cancelados: {cancelled_event_with_present_attendance}")
        issues += (
        ...
        agenda_orphan_creator = (
            ...
            .filter(
                ...
                ~exists().where(models.User.id == models.AgendaEvent.created_by_user_id),
            )
            .scalar()
            or 0
        )
        print(f"- Eventos agenda con rango inválido (end_at < start_at): {agenda_invalid_range}")
        ...
        sessions_orphan_house = (
            ...
            .filter(~exists().where(models.GloryHouse.id == models.GloryHouseSession.glory_house_id))
            .scalar()
            or 0
        )
        sessions_orphan_season = (
            ...
            .filter(
                ...
                ~exists().where(models.FaroSeason.id == models.GloryHouseSession.season_id),
            )
            .scalar()
            or 0
        )
        faro_attendance_orphan_session = (
            ...
            .filter(~exists().where(models.GloryHouseSession.id == models.GloryHouseAttendance.session_id))
            .scalar()
            or 0
        )
        faro_attendance_orphan_member = (
            db.query(func.count(models.GloryHouseAttendance.id))
            .filter(~exists().where(models.Member.id == models.GloryHouseAttendance.member_id))
            .scalar()
            or 0
        )
