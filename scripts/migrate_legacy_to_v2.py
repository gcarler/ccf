...
Migración de datos Legacy → v2 para la plataforma CCF.
...
Uso:
    ...
    ./scripts/migrate_legacy_to_v2.py [--dry-run] [--batch-size 1000]
"""
...
logger = logging.getLogger("migrate_legacy_to_v2")
...
class MigrationReport:
    ...
    def print_summary(self) -> None:
        print("\n" + "=" * 60)
        print("RESUMEN DE MIGRACIÓN LEGACY → V2")
        print("=" * 60)
...
class LegacyIdMapping:
    """Administra la tabla legacy_id_mapping en memoria + DB."""
    ...
    def _preload(self) -> None:
        rows = self.session.execute(
            text("SELECT tabla, legacy_id, v2_id FROM legacy_id_mapping")
        ).fetchall()
        for row in rows:
            self._cache.setdefault(row.tabla, {})[row.legacy_id] = row.v2_id

    def get(self, tabla: str, legacy_id: int) -> Optional[str]:
        return self._cache.get(tabla, {}).get(legacy_id)

    def set(self, tabla: str, legacy_id: int, v2_id: Any) -> None:
        v2_str = str(v2_id)
        self._cache.setdefault(tabla, {})[legacy_id] = v2_str
        self.session.execute(
            text("""
                INSERT INTO legacy_id_mapping (tabla, legacy_id, v2_id)
                VALUES (:tabla, :legacy_id, :v2_id)
                ON CONFLICT (tabla, legacy_id) DO UPDATE SET
                    v2_id = EXCLUDED.v2_id,
                    migrated_at = now()
            """),
            {"tabla": tabla, "legacy_id": legacy_id, "v2_id": v2_str},
        )
...
class Migrator:
    def __init__(self, session: Session, dry_run: bool, batch_size: int) -> None:
        ...
        self.mapping = LegacyIdMapping(session)
    ...
    def _resolve_fk(self, tabla_legacy: str, legacy_id: Any) -> Optional[Any]:
        if legacy_id is None:
            return None
        return self.mapping.get(tabla_legacy, int(legacy_id))
    ...
    def migrate_pair(
        ...
        logger.info("Iniciando %s → %s", legacy_name, v2_name)

        if not self._table_exists(legacy_name):
            logger.error("Tabla legacy no existe: %s", legacy_name)
            self.report.record(legacy_name, 0, 0, 0)
            return
        ...
        while offset < total:
            ...
            for row in rows:
                lid = getattr(row, legacy_pk)
                if self.mapping.get(legacy_name, int(lid)):
                    skipped += 1
                    continue
                ...
                try:
                    ...
                    if not self.dry_run:
                        ...
                        self.mapping.set(legacy_name, int(lid), v2_id)

                    migrated += 1
                except Exception as exc:
                    logger.error(
                        "Error migrando %s.%s=%s: %s",
                        legacy_name, legacy_pk, lid, exc,
                    )
                    failed += 1
            ...
            logger.info(
                ...
                legacy_name, offset, migrated, failed, skipped,
            )

        self.report.record(legacy_name, total, migrated, failed)
    ...
    def run(self) -> None:
        logger.info("Iniciando migración legacy→v2 (dry_run=%s)", self.dry_run)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Migración Legacy → v2 CCF")
