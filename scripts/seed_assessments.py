...
from backend.models import Course, Lesson, Assessment, AssessmentQuestion, AssessmentOption
...
def seed_assessments():
    ...
    try:
        ...
        db.add(AssessmentOption(question_id=q1.id, option_text="La gracia de Dios mediante la fe", is_correct=True))
        db.add(AssessmentOption(question_id=q1.id, option_text="Asistir todos los domingos", is_correct=False))
        ...
        # Options for Q2
        db.add(AssessmentOption(question_id=q2.id, option_text="Un rito de paso social", is_correct=False))
        db.add(AssessmentOption(question_id=q2.id, option_text="Identificación pública con la muerte y resurrección de Cristo", is_correct=True))
        db.add(AssessmentOption(question_id=q2.id, option_text="Un requisito para ser líder", is_correct=False))
