...
def reset_admin():
    try:
        ...
        if user:
            ...
            cursor.execute("UPDATE users SET password_hash = ?, is_active = 1 WHERE username = 'admin_ccf'", (hashed_password,))
            print(f"EXITO: Contraseña de 'admin_ccf' actualizada a: {new_password}")
        else:
            # Crear si no existe
            cursor.execute("INSERT INTO users (username, password_hash, is_active) VALUES (?, ?, 1)", ('admin_ccf', hashed_password))
