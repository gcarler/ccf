...
export default function PublicHomePage({ initialHomePage }: { initialHomePage?: CmsPublicPage | null }) {
    ...
    const cmsHeroBackground: PublicSlide[] = typeof heroContent?.bg_image === "string" && heroContent.bg_image.trim()
        ? [{ src: heroContent.bg_image, alt: heroEyebrow || "Comunidad Cristiana El Faro" }]
        : [];
    const homeGallery = (() => {
    ...
    const homeSlides: PublicSlide[] = [
        ...
        ...(gallerySlides.length > 0
            ? gallerySlides
            : cmsHeroSlides.length === 0 ? cmsHeroBackground : []),
