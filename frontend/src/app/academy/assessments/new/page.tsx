...
export default function NewAssessmentPage() {
    ...
    const handleSave = async () => {
        if (!title || questions.length === 0) {
            toast.error('Completa el titulo y agrega al menos una pregunta');
            return;
        }
        if (!courseId) {
            toast.error('Ingresa el ID del curso al que pertenece esta evaluacion');
            return;
        }
        ...
        <div className="flex flex-col h-full bg-[#f8fafc] dark:bg-[#1E1F21] overflow-hidden">
            <WorkspaceToolbar
                breadcrumbs={[
                    ...
                    { label: 'Nueva Evaluacion', icon: FilePlus },
                ]}
                rightActions={
