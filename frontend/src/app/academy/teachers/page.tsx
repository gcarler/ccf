                ...
                <section className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
                    ...
                    <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                        {filtered.map((teacher) => (
                            <article
                                key={teacher.id}
                                className="cursor-pointer rounded-2xl border border-slate-200 bg-slate-50 p-5 transition-all hover:-translate-y-0.5 hover:border-blue-300 hover:bg-white dark:border-white/10 dark:bg-white/[0.03] dark:hover:bg-white/[0.06]"
                                onClick={() => router.push(`/academy/teacher?teacher=${teacher.id}`)}
                            >
                                <div className="mb-4 flex items-center gap-3">
                                    <div className="flex size-11 items-center justify-center rounded-2xl bg-blue-600 text-sm font-black text-white">
                                ...
                                <div className="mt-5 grid grid-cols-2 gap-2 text-xs">
                                    <span className="rounded-xl bg-white p-3 font-black text-slate-500 dark:bg-black/20">{teacher.course_count ?? 0} cursos</span>
                                    <span className="rounded-xl bg-white p-3 font-black text-slate-500 dark:bg-black/20">{teacher.active_students ?? 0} alumnos</span>
                                </div>
                            </article>
                        ))}
