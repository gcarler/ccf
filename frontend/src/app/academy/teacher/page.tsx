    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-[#121417] overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-4 lg:p-4 space-y-4">
                {!isStaff && (
                    <div className="border border-amber-300 bg-amber-100 text-amber-800 rounded-xl px-4 py-2 text-sm font-semibold">
                ...
                {viewType === 'list' && (
                    <section className="space-y-3">
                        ...
                        {submissions.length === 0 && <div className="py-6 text-center text-slate-400 text-sm">Sin entregas registradas</div>}
                    </section>
                )}
                        ...
                        <table className="min-w-full text-sm">
                            <thead>
                                <tr className="text-left text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">
                                    <th className="px-4 py-1.5">Lección</th>
                                    <th className="px-4 py-1.5">Estudiante</th>
                                    <th className="px-4 py-1.5">Estado</th>
                                    <th className="px-4 py-1.5">Nota</th>
                            ...
                            <tbody>
                                {submissions.map((submission) => (
                                    <tr key={`table-${submission.id}`} className="border-t border-slate-100 dark:border-white/5">
                                        <td className="px-4 py-2 font-semibold text-slate-900 dark:text-white">{submission.lesson_title}</td>
                                        <td className="px-4 py-2 text-slate-400">{submission.student_name}</td>
                                        <td className="px-4 py-2 text-xs font-black uppercase tracking-widest">
                                            <span className={submission.grade == null ? 'text-amber-500' : 'text-emerald-500'}>{submission.grade == null ? 'Pendiente' : 'Calificada'}</span>
                                        </td>
                                        <td className="px-4 py-2 text-slate-700 dark:text-slate-300">{submission.grade?.toFixed(1) ?? 'N/A'}</td>
                                        <td className="px-4 py-2 text-slate-400">{new Date(submission.submitted_at).toLocaleString()}</td>
                                    </tr>
                                ))}
                                {submissions.length === 0 && (
                                    <tr><td colSpan={5} className="px-4 py-4 text-center text-slate-400">Sin información</td></tr>
                                )}
                            </tbody>
                        </table>
                    </section>
                ...
                {viewType === 'calendar' && (
                    <section className="space-y-4">
                        ...
                        {groupedByDate.length === 0 && <div className="py-6 text-center text-slate-400 text-sm">Sin agenda de entregas</div>}
                    </section>
                )}
                ...
                {viewType === 'gantt' && (
                    <section className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-4 space-y-3">
                        ...
                        {submissions.length === 0 && <div className="py-4 text-center text-slate-400 text-sm">Sin entregas para trazar</div>}
                    </section>
                )}
                ...
                {viewType === 'wiki' && (
                 ...
                 {viewType === 'grid' && (
                 <section className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111418] shadow-xl">
                     <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between px-4 py-5 border-b border-slate-100 dark:border-white/5 gap-3">
                         <div>
                             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Flujo de revisiones</p>
                             <h2 className="text-lg font-black text-slate-800 dark:text-white">Entregas de estudiantes</h2>
                         </div>
                        ...
                        {viewMode === 'courses' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 p-4 gap-4">
                                ...
                                {courses.map(course => (
                                    <div key={course.id} className="p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 group hover:border-blue-500 transition-all">
                                        ...
                                        <h4 className="text-base font-black text-slate-800 dark:text-white mb-2">{course.title}</h4>
                                        ...
                                        <button 
                                            ...
                                            className="w-full py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all"
                                        >
                                            Gestionar Curso
                                        </button>
                                    </div>
                        ...
                        {!loading && viewMode === 'pending' && pending.length === 0 && (                            <div className="px-4 py-12 text-center text-slate-500 text-sm">
                                No hay entregas pendientes por ahora. Disfruta un café ☕
                            </div>
                        )}
                        {viewMode === 'pending' && pending.map((submission) => (
                            <article key={submission.id} className="px-4 py-6 flex flex-col lg:flex-row lg:items-center gap-4">
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full text-sm">
                                    ...
                                    <tbody>
                                        {graded.length === 0 && (
                                            <tr>
                                                <td colSpan={4} className="px-4 py-6 text-slate-500 text-center">Aún no hay historial</td>
                                            </tr>
                                        )}
                                        {graded.map((submission) => (
                                            <tr key={`graded-${submission.id}`} className="border-t border-slate-100 dark:border-white/5">
                                                <td className="px-4 py-2 font-semibold text-slate-900 dark:text-white">{submission.lesson_title}</td>
                                                <td className="px-4 py-2 text-slate-400">{submission.student_name}</td>
                                                <td className="px-4 py-2">{submission.grade?.toFixed(1) ?? 'N/A'}</td>
                                                <td className="px-4 py-2 text-slate-400">{new Date(submission.submitted_at).toLocaleDateString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                {viewType === 'grid' && graded.length > 0 && (
                    <section className="rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/70 dark:bg-white/5 p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {graded.slice(0, 3).map((submission) => (
                                <div key={submission.id} className="bg-white dark:bg-[#121417] rounded-2xl border border-slate-100 dark:border-white/10 p-4 shadow-sm">
                                    ...
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-white">{submission.student_name}</h4>
...
function TeacherStat({ icon: Icon, label, value, tone }: { icon: any; label: string; value: number | string; tone: 'blue' | 'emerald' | 'amber' }) {
    ...
    return (
        <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111418] p-4 flex items-center gap-4 shadow-sm">
            <div className={clsx('size-9 rounded-2xl flex items-center justify-center', toneClasses[tone])}>
                <Icon size={22} />
            </div>
            <div>
