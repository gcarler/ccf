        ...
        <div className="relative">
            ...
            {(!value || multi) && (
                <div>
                    ...
                    {open && search && (
                        <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl max-h-60 overflow-y-auto">
...
export default function EventDetailPage() {
    ...
    if (loading || !event) {
        return <div className="p-20 text-center animate-pulse font-black text-slate-400">Cargando...</div>;
    }
            ...
            <main className="flex-1 overflow-y-auto p-4 lg:p-4">
                <div className="max-w-6xl mx-auto space-y-3">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase">{event.name}</h1>
                                    ...
                                    <div className="space-y-3">
                                        <div className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 p-4 shadow-sm">
                                            ...
                                            <div className="space-y-4">
                                                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Desglose por Ministerio / Perfil</h4>
                                                {sessionData?.metrics && Object.entries(sessionData.metrics).map(([key, val]) => (
                                                    <div key={key} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-white/5">
                                                        ...
                                                        <span className="text-sm font-black text-blue-600 bg-blue-100 dark:bg-blue-900/30 px-3 py-1 rounded-lg">{val}</span>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            {(sessionData?.total_absentees ?? 0) > 0 && (
                                                <div className="mt-6 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-500/20 flex items-center justify-between gap-4">
                                                    ...
                                                    <button
                                                        ...
                                                        className="shrink-0 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 disabled:opacity-60"
                                                    >
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
                                                <div className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 p-4">
                                                    <div className="flex items-center justify-between mb-3">
                                                        <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Personas presentes</h4>
                                                        <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600">{sessionData?.attendees?.length ?? 0}</span>
                                                    </div>
                                                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                                                        {(sessionData?.attendees || []).length === 0 ? (
                                                            ...
                                                            sessionData!.attendees.map((att) => (
                                                                <div key={`${att.member_id}-${att.role}`} className="flex items-center justify-between gap-3 rounded-xl bg-white dark:bg-[#1e1f21] border border-slate-100 dark:border-white/5 px-3 py-2">
                                                                    <div className="min-w-0">
                                                                        <p className="text-sm font-bold text-slate-800 dark:text-white truncate">{att.name}</p>
                                                                        <p className="text-[10px] uppercase font-black tracking-widest text-slate-400">{att.role}</p>
                                                                    </div>
                                                ...
                                                <div className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 p-4">
                                                    <div className="flex items-center justify-between mb-3">
                                                        <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Personas ausentes</h4>
                                                        <span className="text-[10px] font-black uppercase tracking-widest text-amber-600">{sessionData?.absentees?.length ?? 0}</span>
                                                    </div>
                                                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                                                        {(sessionData?.absentees || []).length === 0 ? (
                                                            ...
                                                            sessionData!.absentees.map((att) => (
                                                                <div key={`${att.member_id}-${att.role}`} className="flex items-center justify-between gap-3 rounded-xl bg-white dark:bg-[#1e1f21] border border-slate-100 dark:border-white/5 px-3 py-2">
                                                                    ...
                                                                    <span className="text-[10px] font-black uppercase tracking-widest text-rose-600">Ausente</span>
                                                                </div>
                                                            ))
                                                        )}
                                                    </div>
