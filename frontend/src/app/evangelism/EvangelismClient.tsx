            ...
            <div className="h-full flex flex-col relative">
                {loading ? (
                    ...
                    <div className="pb-16 flex-1">
                        {/* ── TABLE VIEW ─────────────────────────────── */}
                        {viewType === 'table' && (
                            <div className="overflow-x-auto border border-slate-200 dark:border-white/[0.06] rounded-2xl bg-white dark:bg-[#1e1f21]">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/10">
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-16">ID</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Estrategia</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-32">Estado</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-44">Tipo</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-40">Inicio</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-40">Fin</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 w-12"></th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                                        {filteredData.map((strategy, idx) => (
                                            ...
                                            >
                                                <td className="px-6 py-4 text-[12px] font-black text-slate-400 dark:text-slate-600">
                                                    #{strategy.id}
                                                </td>
                        ...
                        {/* ── BOARD VIEW ─────────────────────────────── */}
                        {viewType === 'board' && (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full min-h-[500px] items-start">
                                {/* Columns map */}
                                {(['pending', 'active', 'done'] as const).map(colStatus => {
                                    ...
                                    return (
                                        <div 
                                            key={colStatus}
                                            className="bg-slate-50/50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl p-4 flex flex-col max-h-[80vh] overflow-y-auto scrollbar-thin"
                                        >
                                            ...
                                            <div className="space-y-4">
                                                {colItems.map((strategy) => (
                                                    <motion.div
                                                        ...
                                                        onClick={() => handleSelectStrategy(strategy)}
                                                        className="group relative bg-white dark:bg-[#252528] rounded-2xl border border-slate-200/70 dark:border-white/5 p-5 shadow-sm hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-black/30 transition-all duration-300 cursor-pointer overflow-hidden active:scale-[0.99] border-t-4"
                                                    ...
                                                    >
                                                        ...
                                                        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-white/5 flex flex-wrap gap-2 items-center justify-between">
                                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400">
                                                                {strategy.strategy_type || 'General'}
                                                            </span>
                                                ...
                                                {colItems.length === 0 && (
                                                    <div className="py-8 text-center border border-dashed border-slate-200 dark:border-white/5 rounded-2xl text-[10px] font-bold uppercase tracking-widest text-slate-400">
                                                        Vacío
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <div className="space-y-4 max-w-4xl mx-auto">
                                {filteredData.map((strategy, idx) => (
                                    <motion.div
                                        ...
                                        onClick={() => handleSelectStrategy(strategy)}
                                        className="group bg-white dark:bg-[#252528] rounded-2xl border border-slate-200/70 dark:border-white/5 p-5 shadow-sm hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-black/30 transition-all duration-300 cursor-pointer flex items-center justify-between gap-6"
                                    >
                                        <div className="flex items-start gap-4 flex-1 min-w-0">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-3">
                                                    <h3 className="text-[14px] font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">
                                                        {strategy.name}
                                                    </h3>
                                                    <span className="text-[9px] font-black bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded">
                                                        #{strategy.id}
                                                    </span>
                                                </div>
                                                {strategy.description && (
                    ...
                    <div className="flex flex-col h-full overflow-hidden">
                        <header className="p-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-black/20 shrink-0">
                            ...
                            <button 
                                ...
                                className="p-2 hover:bg-rose-50 dark:hover:bg-rose-950/20 text-rose-500 hover:text-rose-600 rounded-xl transition-colors"
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    placeholder="Nombre..."
                                    className="w-full px-4 py-2.5 text-[13px] font-medium bg-white dark:bg-[#252528] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all placeholder:text-slate-400"
                                />
                            </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select
                                    ...
                                    className="w-full px-4 py-2.5 text-[13px] font-medium bg-white dark:bg-[#252528] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all text-slate-700 dark:text-slate-300"
                                >
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(['pending', 'active', 'done'] as const).map(status => (
                                        <button
                                            ...
                                            className="px-3 py-2 rounded-xl border text-[11px] font-bold transition-all flex flex-col items-center justify-center gap-1 shadow-sm active:scale-95"
                                            style={{
                            ...
                            <div className="space-y-1.5">
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-2.5 text-[13px] font-medium bg-white dark:bg-[#252528] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all placeholder:text-slate-400 resize-none"
                                />
                            </div>
                        ...
                        <footer className="p-4 border-t border-slate-100 dark:border-white/5 flex gap-4 bg-white dark:bg-[#1e1f21] shrink-0">
                            <button 
                                ...
                                className="flex-1 py-2.5 bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-300 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all"
                            >
                                Cancelar
                            </button>
                            <button 
