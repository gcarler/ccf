...
export default function GlossaryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch<GlossaryTerm[]>(`/cms/v2/glossary?site_key=faro${params}`, { silent: true });
  ...
  const create = async () => {
    ...
    await apiFetch("/cms/v2/glossary", { method: "POST", body: { site_key: "faro", ...form, aliases: form.aliases.split(",").map(a => a.trim()).filter(Boolean) }, silent: true });
