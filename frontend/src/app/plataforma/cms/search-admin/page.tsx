...
export default function SearchAdminPage() {
  ...
  const search = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiFetch<{ results: SearchResult[]; promoted: Promotion[] }>(`/cms/v2/search`, { method: "POST", body: { site_key: "faro", query }, silent: true });
  ...
  const loadPromos = async () => {
    try {
      const data = await apiFetch<Promotion[]>("/cms/v2/search/promotions?site_key=faro", { silent: true });
  ...
  const createPromo = async () => {
    ...
    await apiFetch("/cms/v2/search/promotions", { method: "POST", body: { site_key: "faro", ...promoForm }, silent: true });
