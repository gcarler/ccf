    ...
    <div className="space-y-3 p-3">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <div className="rounded-lg border border-[hsl(var(--border))] dark:border-white/10 bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--admin-bg-tertiary))] p-4 space-y-3">
          ...
          <p className="text-xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Nuevo sitio</p>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre (ej. Comunidad)" className="w-full rounded-md border border-[hsl(var(--border))] dark:border-white/10 bg-transparent px-3 py-2 text-sm" disabled={!canManage} />
          <input value={siteKey} onChange={(e) => setSiteKey(sanitizeSiteKey(e.target.value))} placeholder="site_key (ej. comunidad)" className="w-full rounded-md border border-[hsl(var(--border))] dark:border-white/10 bg-transparent px-3 py-2 text-sm" disabled={!canManage} />
          <input value={basePath} onChange={(e) => setBasePath(e.target.value)} placeholder="base_path (ej. /comunidad)" className="w-full rounded-md border border-[hsl(var(--border))] dark:border-white/10 bg-transparent px-3 py-2 text-sm" disabled={!canManage} />
        ...
        <div className="lg:col-span-2 rounded-lg border border-[hsl(var(--border))] dark:border-white/10 bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--admin-bg-tertiary))] p-4 space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Sitios registrados</p>
          {loading ? (
            <p className="text-sm text-[hsl(var(--text-secondary))]">Cargando sitios...</p>
          ) : (
            <div className="space-y-2">
              ...
              {sites.length === 0 && <p className="text-sm text-[hsl(var(--text-secondary))]">No hay sitios creados.</p>}
            </div>
          )}
        </div>
      </section>
