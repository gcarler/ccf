  ...
  if (loading) {
      return (
          <div className="flex flex-col h-full bg-[hsl(var(--bg-primary))] overflow-hidden items-center justify-center space-y-3">
              ...
              <p className="text-2xs font-bold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Verificando Sistemas...</p>
          </div>
      );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[hsl(var(--bg-primary))] overflow-hidden font-sans relative">
        <WorkspaceToolbar
            ...
            rightActions={
                <button className="flex items-center gap-2 px-3 py-1.5 bg-[hsl(var(--bg-muted))] dark:bg-[hsl(var(--bg-primary))] text-white dark:text-[hsl(var(--text-primary))] rounded-lg text-2xs font-bold uppercase tracking-wide shadow-md hover:scale-105 active:scale-95 transition-all">
                ...
                <motion.section variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-4 gap-3">
                    <div className="xl:col-span-2 relative overflow-hidden rounded-lg bg-ccf-blue-dark p-4 text-white shadow-lg group border border-[hsl(var(--primary))/0.2] flex flex-col justify-between">
                        ...
                        <div className="relative z-10 flex items-center justify-between mb-3">
                            <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-white/10 backdrop-blur-md rounded-lg text-2xs font-bold uppercase tracking-wide text-[hsl(var(--primary))] border border-white/20">
                                <Globe size={10} /> Tesorería Consolidada <span className="text-white/20">|</span> {new Date().getFullYear()}
                            </div>
                            <button className="px-3 py-1 bg-white/10 hover:bg-[hsl(var(--bg-primary))] text-white hover:text-[hsl(var(--text-secondary))] rounded-lg font-bold text-2xs uppercase tracking-wide transition-all">
                                Desglosar
                            </button>
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-3">
                            <div className="flex items-end gap-3">
                                ...
                                <div className="px-2 py-0.5 bg-[hsl(var(--success))] text-white rounded-md text-2xs font-bold flex items-center gap-1 shadow-sm">
                                     <TrendingUp size={10} strokeWidth={3} /> Activo
                                </div>
                            </div>
                            <div className="flex gap-3 items-center">
                                <div className="space-y-0.5"><p className="text-2xs font-bold text-[hsl(var(--primary))/0.6] uppercase tracking-wide">Diezmos</p><p className="text-lg font-bold leading-none">${(stats?.diezmos_mes || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p></div>
                                <div className="space-y-0.5"><p className="text-2xs font-bold text-[hsl(var(--primary))/0.6] uppercase tracking-wide">Ofrendas</p><p className="text-lg font-bold leading-none">${(stats?.ofrendas_mes || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p></div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                    ...
                    <div className="lg:col-span-8 space-y-3">
                        <motion.div variants={itemVariants} className="flex items-center justify-between px-2">
                            <h2 className="text-xs font-bold text-[hsl(var(--text-primary))] dark:text-white uppercase tracking-wide flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide">{agentTasks.length} Activos</span>
                    ...
                    <aside className="lg:col-span-4 space-y-3">
                        ...
                        <motion.div variants={itemVariants} className="p-4 rounded-lg bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--surface-1))] border border-[hsl(var(--border))] dark:border-white/5 shadow-sm relative overflow-hidden group">
                            ...
                            <div className="space-y-3 relative z-10">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="px-2 py-0.5 bg-[hsl(var(--success-muted))] dark:bg-[hsl(var(--success)/0.15)] text-[hsl(var(--success))] dark:text-[hsl(var(--success))] text-2xs font-bold uppercase tracking-wide rounded flex items-center gap-1">
                                    ...
                                    {agentInsights.length > 0 ? (
                                        agentInsights.slice(0, 3).map((insight: AgentInsight) => (
                                            <div key={insight.id} className="p-3 bg-[hsl(var(--surface-1))] dark:bg-black/20 rounded-md border border-[hsl(var(--border))] dark:border-white/5 space-y-1 group/insight cursor-pointer hover:border-[hsl(var(--primary))/0.3] hover:shadow-sm transition-all">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-xs font-bold text-[hsl(var(--text-primary))] dark:text-[hsl(var(--text-secondary))] leading-none group-hover/insight:text-[hsl(var(--primary))] transition-colors truncate">{insight.title}</p>
                                                    <span className="text-2xs font-bold px-1.5 py-0.5 bg-[hsl(var(--info-muted))] dark:bg-[hsl(var(--info)/0.1)] text-[hsl(var(--info))] rounded uppercase tracking-wide shrink-0">{insight.insight_type?.split('_')[0] || insight.insight_type || '—'}</span>
                                                </div>
                                                <p className="text-2xs text-[hsl(var(--text-secondary))] dark:text-[hsl(var(--text-secondary))] leading-tight italic font-medium line-clamp-2">&ldquo;{insight.payload}&rdquo;</p>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="p-3 bg-[hsl(var(--surface-1))] dark:bg-black/20 rounded-md border border-[hsl(var(--border))] dark:border-white/5 space-y-1">
                                            <p className="text-xs font-bold text-[hsl(var(--text-primary))] dark:text-[hsl(var(--text-secondary))] leading-none">Análisis de Sistema</p>
                                            <p className="text-2xs text-[hsl(var(--text-secondary))] dark:text-[hsl(var(--text-secondary))] leading-tight italic font-medium">&ldquo;Todos los sistemas operan dentro de los parámetros normales. No se requiere intervención inmediata.&rdquo;</p>
                                        </div>
                                    )}
                                </div>

                                <button className="w-full py-2.5 bg-[hsl(var(--primary))] text-white rounded-lg font-bold text-2xs uppercase tracking-wider shadow-sm active:scale-95 transition-all flex items-center justify-center gap-1.5">
                        ...
                        <motion.div variants={itemVariants} className="space-y-3 bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--surface-1))] p-4 rounded-lg border border-[hsl(var(--border))] dark:border-white/5 shadow-sm">
                            <h3 className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide flex items-center gap-1.5 mb-3">
                            ...
                            <button className="w-full mt-2 py-2 text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wider hover:text-[hsl(var(--text-secondary))] transition-colors">Ver bitácora completa</button>
                        </motion.div>
                    </aside>
                </div>
            </motion.div>
        ...
        <WorkspaceDrawer
            ...
            actions={
                <>
                    <button className="px-3 py-2 text-2xs font-bold text-[hsl(var(--text-secondary))] hover:text-[hsl(var(--text-primary))] transition-colors" onClick={() => setIsDrawerOpen(false)}>Descartar</button>
                    <button className="px-3 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))] text-white rounded-lg text-2xs font-bold uppercase tracking-wide shadow-md active:scale-95 transition-all">Ejecutar Acción</button>
                </>
            }
        >
            <div className="space-y-3 animate-fade-in p-2">
                <section className="grid grid-cols-2 gap-3">
                    <div className="p-4 bg-[hsl(var(--surface-1))] dark:bg-black/20 rounded-md border border-[hsl(var(--border))] dark:border-white/5 shadow-sm">
                        ...
                        <p className="text-base font-bold text-[hsl(var(--text-primary))] dark:text-white uppercase">{selectedTask?.status}</p>
                    </div>
                    <div className="p-4 bg-[hsl(var(--surface-1))] dark:bg-black/20 rounded-md border border-[hsl(var(--border))] dark:border-white/5 shadow-sm">
                        <p className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide mb-1">Prioridad</p>
                        <p className="text-base font-bold text-[hsl(var(--destructive))] uppercase">{selectedTask?.priority}</p>
                    </div>
                </section>
                <section className="space-y-3">
                    <h4 className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide flex items-center gap-1.5"><FileText size={10} className="text-[hsl(var(--primary))]"/> Descripción Técnica</h4>
                    <div className="p-4 bg-[hsl(var(--surface-1))] dark:bg-[hsl(var(--surface-1))] rounded-md text-sm text-[hsl(var(--text-secondary))] dark:text-[hsl(var(--text-secondary))] leading-relaxed font-medium shadow-inner">
    ...
    return (
        <div className="p-4 bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--surface-1))] rounded-lg border border-[hsl(var(--border))] dark:border-white/5 shadow-sm relative overflow-hidden group hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 flex flex-col justify-between">
            ...
            <div className="relative z-10 flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between">
                    <p className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide truncate mr-2">{title}</p>
                    <span className="text-2xs font-bold px-1.5 py-0.5 bg-[hsl(var(--surface-1))] dark:bg-white/5 text-[hsl(var(--text-secondary))] rounded flex items-center gap-0.5 shrink-0">
        ...
        >
            <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 pr-2">
                    <h4 className="text-sm font-bold text-[hsl(var(--text-primary))] dark:text-white tracking-tight truncate group-hover:text-[hsl(var(--primary))] transition-colors leading-tight">{task.title}</h4>
                    <p className="text-2xs text-[hsl(var(--text-secondary))] dark:text-[hsl(var(--text-secondary))] truncate font-medium mt-0.5 leading-tight">{task.description || 'Haz clic para intervenir'}</p>
                </div>
            </div>
            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0 bg-[hsl(var(--info-muted))] dark:bg-[hsl(var(--info)/0.1)] px-2.5 py-1 rounded-lg">
                <span className="text-2xs font-bold text-[hsl(var(--primary))] dark:text-[hsl(var(--primary))] uppercase tracking-wide hidden sm:inline">Revisar</span>
        ...
        <div className="flex gap-3 relative z-10 group cursor-default">                <div className={clsx("size-8 rounded-lg flex items-center justify-center shadow-inner shrink-0 transition-transform duration-300 group-hover:scale-105", bg, color)}>
            ...
            <div className="flex-1 min-w-0 border-b border-[hsl(var(--border))] dark:border-white/5 pb-3 group-last:border-0 group-last:pb-0">
                <div className="flex justify-between items-center gap-2">
                    <h5 className="text-xs font-bold text-[hsl(var(--text-primary))] dark:text-white truncate tracking-tight leading-tight">{title}</h5>
                    <span className="text-2xs font-bold text-[hsl(var(--text-secondary))] whitespace-nowrap bg-[hsl(var(--surface-1))] dark:bg-black/20 px-1.5 py-0.5 rounded uppercase tracking-wide">{time}</span>
                </div>
                <p className="text-2xs text-[hsl(var(--text-secondary))] dark:text-[hsl(var(--text-secondary))] font-medium leading-tight mt-0.5"><span className="text-[hsl(var(--text-primary))] dark:text-[hsl(var(--text-secondary))] font-bold">{user}</span></p>
            </div>
        </div>
    );
}
