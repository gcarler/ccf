                ...
                <div key={row.id} className="social-aura bg-[hsl(var(--bg-primary))] dark:bg-white/5 border border-[hsl(var(--border))] dark:border-white/10 p-3 rounded-lg flex items-center justify-between gap-3" style={{ '--aura-color': row.aura } as CSSAuraProperties}>
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-2xs font-bold text-[hsl(var(--text-secondary))] truncate">{row.channel?.url || 'Sin URL configurada'}</p>
                        </div>
                    </div>
                    <span className={clsx("px-2 py-0.5 rounded-full text-2xs font-semibold uppercase", row.channel?.url ? "bg-[hsl(var(--success-muted))] text-[hsl(var(--success))]" : "bg-[hsl(var(--warning-muted))] text-[hsl(var(--warning))]")}>{row.channel?.url ? 'Activo' : 'Pendiente'}</span>
                </div>
            ))}
        </div>
    );
        ...
        <div className="rounded-lg border border-[hsl(var(--border))] dark:border-white/10 overflow-x-auto bg-[hsl(var(--bg-primary))] dark:bg-white/5">
            <table className="w-full min-w-[480px] text-left">
                <thead className="bg-[hsl(var(--surface-1))] dark:bg-white/5">
                    <tr>
                        <th className="px-3 py-1.5 text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Canal</th>
                        <th className="px-3 py-1.5 text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))] hidden md:table-cell">URL</th>
                ...
                <tbody className="divide-y divide-[hsl(var(--border))] dark:divide-white/5">
                    {platformRows.map((row) => (
                        <tr key={row.id} className="hover:bg-[hsl(var(--surface-1))] dark:hover:bg-white/[0.03]">
                            ...
                            <td className="px-3 py-1.5 hidden md:table-cell text-xs text-[hsl(var(--text-secondary))] truncate max-w-[420px]">{row.channel?.url || '—'}</td>
                            <td className="px-3 py-1.5"><span className={clsx("px-2 py-0.5 rounded-full text-2xs font-semibold uppercase", row.channel?.url ? "bg-[hsl(var(--success-muted))] text-[hsl(var(--success))]" : "bg-[hsl(var(--warning-muted))] text-[hsl(var(--warning))]")}>{row.channel?.url ? 'Activo' : 'Pendiente'}</span></td>
                        </tr>
                    ))}
                </tbody>
            </table>
                    ...
                    <div className="space-y-3">
                        {group.rows.map((row) => (
                            <div key={row.id} className="bg-[hsl(var(--bg-primary))] dark:bg-white/[0.05] border border-[hsl(var(--border))] dark:border-white/5 rounded-lg p-4">
                                ...
                                <p className="mt-2 text-2xs font-bold text-[hsl(var(--text-secondary))] truncate">{row.channel?.url || 'Sin enlace'}</p>
                            </div>
                        ))}
                    </div>
                </section>
                                            ...
                                            <div className="flex items-center gap-3 flex-1">
                                                ...
                                                <div className="flex-1 space-y-4">
                                                    <div>
                                                        ...
                                                        <p className="text-2xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wide leading-none">Perfil de la Congregación</p>
                                                    </div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-3">
                            ...
                            <div className="flex-1 space-y-3">
                                ...
                                <div className="flex flex-col md:flex-row gap-4">
                                    ...
                                    />
                                    <button className="px-4 py-2 bg-[hsl(var(--bg-primary))] text-[hsl(var(--text-primary))] rounded-lg text-xs font-semibold uppercase tracking-wide shadow-xl hover:translate-y-[-4px] active:scale-95 transition-all">Sincronizar Señal</button>
                                </div>
                            </div>
                        </div>
                    </motion.section>
