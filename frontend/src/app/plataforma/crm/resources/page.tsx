...
const CANAL_META: Record<CanalEnvio, { label: string; icon: React.ElementType; color: string; bg: string }> = {
    ...
    EMAIL:    { label: 'Email',    icon: Mail,          color: 'text-blue-600 dark:text-blue-400',    bg: 'bg-blue-50 dark:bg-blue-900/20' },
...
const ESTADO_META: Record<string, { label: string; cls: string }> = {
    PROCESANDO: { label: 'Procesando', cls: 'bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-slate-400' },
    ENVIADO:    { label: 'Enviado',    cls: 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' },
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="text-sm font-semibold text-slate-800 dark:text-white truncate">{plantilla.titulo}</span>
                </div>
            ...
            {plantilla.variables_requeridas.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                    {plantilla.variables_requeridas.slice(0, 4).map(v => (
                        <span key={v} className="text-[10px] font-mono bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-400 px-1.5 py-0.5 rounded">
                            {`{{${v}}}`}
                        </span>
                    ))}
                    {plantilla.variables_requeridas.length > 4 && (
                        <span className="text-[10px] text-slate-400">+{plantilla.variables_requeridas.length - 4}</span>
                    )}
                </div>
            )}
            ...
            <div className="absolute top-3 right-3 hidden group-hover:flex items-center gap-1">
                <button
                    ...
                    className="size-6 flex items-center justify-center rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white shadow-sm transition-colors"
                >
                    <Pencil size={12} />
                </button>
                <button
                    onClick={e => { e.stopPropagation(); onDelete(); }}
                    className="size-6 flex items-center justify-center rounded-md bg-white dark:bg-slate-800 border border-rose-200 dark:border-rose-500/30 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 shadow-sm transition-colors"
                >
                    <Trash2 size={12} />
                </button>
            </div>
...
function DetailPanel({
    ...
    return (
        <div className="flex flex-col h-full border-l border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/50">
            {/* Header */}
            <div className="flex items-start justify-between gap-2 px-4 pt-4 pb-3 border-b border-slate-200 dark:border-white/10">
                <div className="min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-slate-800 dark:text-white leading-tight">{plantilla.titulo}</h3>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                    <button onClick={onEdit} className="size-7 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                        <Pencil size={14} />
                    </button>
                    <button onClick={onClose} className="size-7 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                        <X size={14} />
                    </button>
                </div>
            </div>
                ...
                {tab === 'preview' && (
                    <>
                        {plantilla.canal === 'EMAIL' && plantilla.asunto && (
                            <div>
                                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Asunto</p>
                                <p className="text-sm text-slate-700 dark:text-slate-300">{plantilla.asunto}</p>
                            </div>
                        )}
                                ...
                                <div className="space-y-2">
                                    {plantilla.variables_requeridas.map(v => (
                                        <div key={v} className="flex items-center gap-2">
                                            <span className="text-xs font-mono text-slate-500 w-24 shrink-0">{`{{${v}}}`}</span>
                                            <input
                                                value={vars[v] ?? ''}
                                                onChange={e => setVars(p => ({ ...p, [v]: e.target.value }))}
                                                placeholder={`Valor de ${v}`}
                                                className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-800 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-[hsl(var(--primary)/0.5)]"
                                            />
                                        </div>
                                    ))}
                                </div>
                        ...
                        {/* Preview */}
                        <div>
                            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Vista previa</p>
                            <div className="rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">
                                {previewText}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={copyText}
                                className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium h-8 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
                            >
                    ...
                    <>
                        <div className="space-y-2">
                            {plantilla.adjuntos.map(a => (
                                <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10">
                                    <div className="size-8 rounded-lg bg-slate-100 dark:bg-white/10 flex items-center justify-center shrink-0">
                                        <FileText size={14} className="text-slate-500" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-medium text-slate-700 dark:text-white truncate">{a.nombre_recurso}</p>
                                        <p className="text-[10px] text-slate-400">{a.tipo_mime} · {fmtBytes(a.peso_bytes)}</p>
                                    </div>
                                    <a href={a.url_acceso} target="_blank" rel="noopener noreferrer"
                                        className="size-6 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors">
                                        <Download size={12} />
                                    </a>
                                    <button onClick={() => handleDeleteAdjunto(a.id)}
                                        className="size-6 flex items-center justify-center rounded-lg text-slate-400 hover:text-rose-500 transition-colors">
                                        <Trash2 size={12} />
                                    </button>
                                </div>
                            ))}
                            {plantilla.adjuntos.length === 0 && (
                                <p className="text-xs text-slate-400 text-center py-6">Sin adjuntos</p>
                            )}
                        </div>
                        ...
                        <button
                            ...
                            disabled={uploading}
                            className="w-full flex items-center justify-center gap-2 h-9 rounded-xl border-2 border-dashed border-slate-300 dark:border-white/20 text-xs font-medium text-slate-500 hover:border-[hsl(var(--primary)/0.5)] hover:text-[hsl(var(--primary))] transition-colors disabled:opacity-50"
                        >
                ...
                {tab === 'historial' && (
                    <>
                        ...
                        {!loadingBit && bitacora.length === 0 && (
                            <p className="text-xs text-slate-400 text-center py-6">Sin envíos registrados</p>
                        )}
                        <div className="space-y-2">
                            {bitacora.map(b => {
                                const em = ESTADO_META[b.estado] ?? { label: b.estado, cls: '' };
                                return (
                                    <div key={b.id} className="p-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10">
                                        <div className="flex items-center justify-between mb-1">
                                            <span className={clsx('text-[10px] font-medium px-2 py-0.5 rounded-full', em.cls)}>{em.label}</span>
                                            <span className="text-[10px] text-slate-400">{fmt(b.fecha_envio)}</span>
                                        </div>
                                        <p className="text-[10px] text-slate-500 font-mono">→ {b.destinatario_id.slice(0, 8)}…</p>
                                        {b.log_error && (
...
function PlantillaDrawer({
    ...
    return (
        <>
            ...
            <div className="fixed right-0 top-0 bottom-0 w-[480px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-white/10 z-50 flex flex-col shadow-2xl">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-white/10">
                    <h2 className="text-sm font-semibold text-slate-800 dark:text-white">
                        {editing ? 'Editar plantilla' : 'Nueva plantilla'}
                    </h2>
                    <button onClick={onClose} className="size-7 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                        <X size={15} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/0.4)]"
                        >
                    ...
                    {form.canal === 'EMAIL' && (
                        <div>
                            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Asunto del email</label>
                            <input
                                value={form.asunto}
                    ...
                    {/* Variables */}
                    <div>
                        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Variables requeridas</label>
                        <div className="flex gap-2 mb-2">
                            <input
                                ...
                                className="flex-1 text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/0.4)]"
                            />
                            <button onClick={addVar} className="px-3 h-8 rounded-lg bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 text-xs font-medium hover:bg-slate-200 dark:hover:bg-white/20 transition-colors">
                                + Agregar
                            </button>
                        </div>
                        {form.variables_requeridas.length > 0 && (
                            <div className="flex flex-wrap gap-1.5">
                                {form.variables_requeridas.map(v => (
                                    <span key={v} className="inline-flex items-center gap-1 text-xs font-mono bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md">
                                        {`{{${v}}}`}
                                        <button onClick={() => removeVar(v)} className="text-slate-400 hover:text-rose-500 ml-0.5">
                                            <X size={10} />
                                        </button>
                                    </span>
                                ))}
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1">
                            <label className="text-xs font-medium text-slate-600 dark:text-slate-400">Contenido *</label>
                            {form.variables_requeridas.length > 0 && (
                                <div className="flex gap-1 flex-wrap justify-end">
                                    {form.variables_requeridas.map(v => (
                                        <button key={v} onClick={() => insertVar(v)}
                                            className="text-[10px] font-mono bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-600 dark:text-slate-400 px-1.5 py-0.5 rounded transition-colors">
                                            {`{{${v}}}`}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <textarea
                            ...
                            rows={8}
                            className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-800 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/0.4)] resize-none font-mono leading-relaxed"
                    ...
                    {/* Meta template id */}
                    <div>
                        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">ID plantilla Meta (WhatsApp Business API)</label>
                        <input
                ...
                <div className="px-5 py-4 border-t border-slate-200 dark:border-white/10 flex gap-2">
                    <button onClick={onClose} className="flex-1 h-9 rounded-lg border border-slate-200 dark:border-white/10 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        Cancelar
                    </button>
                    <button
                        onClick={submit}
...
function CatDrawer({
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/40 z-40" onClick={onClose} />
            <div className="fixed right-0 top-0 bottom-0 w-80 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-white/10 z-50 flex flex-col shadow-2xl">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-white/10">
                    <h2 className="text-sm font-semibold text-slate-800 dark:text-white">Nueva categoría</h2>
                    <button onClick={onClose} className="size-7 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                        <X size={15} />
                    </button>
                </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {DEFAULT_COLORS.map(c => (
                                <button key={c} onClick={() => setColor(c)}
                                    className={clsx('size-7 rounded-full border-2 transition-transform', color === c ? 'scale-110 border-slate-400 dark:border-white/60' : 'border-transparent')}
                                    style={{ background: c }} />
                            ))}
            ...
            <div className="fixed right-0 top-0 bottom-0 w-[460px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-white/10 z-50 flex flex-col shadow-2xl">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-white/10">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">{plantilla.titulo}</p>
                    </div>
                    <button onClick={onClose} className="size-7 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                        <X size={15} />
                    </button>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                    ...
                    {plantilla.variables_requeridas.length > 0 && (
                        <div>
                            ...
                            <div className="space-y-2">
                                {plantilla.variables_requeridas.map(v => (
                                    <div key={v} className="flex items-center gap-2">
                                        <span className="text-xs font-mono text-slate-500 w-20 shrink-0">{`{{${v}}}`}</span>
                                        <input
                                            value={vars[v] ?? ''}
                                            onChange={e => setVars(p => ({ ...p, [v]: e.target.value }))}
                                            placeholder={v}
                                            className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-[hsl(var(--primary)/0.4)]"
                                        />
                                    </div>
                                ))}
                            </div>
                    ...
                    {/* Preview */}
                    <div>
                        <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Mensaje hidratado</p>
                        <div className="rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed min-h-[80px]">
                            {previewText}
                        </div>
                    </div>
                </div>
...
export default function RecursosPage() {
    ...
    return (
        <div className="flex h-full overflow-hidden">
            {/* ── Category sidebar ── */}
            <div className="w-52 shrink-0 flex flex-col border-r border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 overflow-y-auto">
                <div className="px-4 pt-5 pb-3">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Categorías</p>
                    <button
                ...
                <div className="mt-auto px-4 pb-4">
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-1.5 h-8 rounded-lg border border-dashed border-slate-300 dark:border-white/20 text-xs text-slate-500 hover:border-[hsl(var(--primary)/0.5)] hover:text-[hsl(var(--primary))] transition-colors"
                    >
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {/* Toolbar */}
                <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 shrink-0">
                    <div className="flex gap-1.5">
                        <button
                            onClick={() => setFilterCanal(null)}
                            className={clsx('h-7 px-3 rounded-full text-xs font-medium transition-all', filterCanal === null ? 'bg-slate-800 dark:bg-white text-white dark:text-slate-900' : 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/20')}
                        >
                            Todos
                        </button>
                        {(['WHATSAPP', 'EMAIL', 'SMS'] as CanalEnvio[]).map(c => {
                            ...
                            return (
                                <button key={c} onClick={() => setFilterCanal(filterCanal === c ? null : c)}
                                    className={clsx('h-7 px-3 rounded-full text-xs font-medium flex items-center gap-1 transition-all',
                                        filterCanal === c ? `${m.bg} ${m.color} ring-1 ring-current` : 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/20')}>
                                    <Icon size={11} />{m.label}
                                </button>
                            );
                        })}
                    ...
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full pl-8 pr-3 h-8 text-xs rounded-lg bg-slate-100 dark:bg-white/10 border border-transparent text-slate-700 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/0.4)]"
                        />
                    </div>
                    <button
                        onClick={openCreate}
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    ...
                    {!loading && plantillas.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <BookOpen size={40} className="text-slate-300 dark:text-white/20 mb-3" />
                            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">Sin plantillas</p>
                            <p className="text-xs text-slate-400 mt-1">Crea tu primera plantilla con el botón de arriba</p>
                        </div>
                    )}
