...
export default function ConsolidationPipelinePage() {
    ...
    // Selección múltiple para acciones en lote (column visibility + row selection).
    const [selectedLeads, setSelectedLeads] = useState<PipelineLead[]>([]);
    ...
    const handleBulkDelete = useCallback(async () => {
        ...
        await Promise.all(
            selectedLeads.map(async (lead) => {
                try {
                    await apiFetch(`/crm/casos/${lead.id}`, { token, method: 'DELETE' });
                    ok += 1;
                } catch {
                    failed += 1;
                }
        ...
        if (ok > 0) {
            addToast(`✅ ${ok} caso${ok !== 1 ? 's' : ''} archivado${ok !== 1 ? 's' : ''}`, 'success');
            setSelectedLeads([]);
            await fetchPipeline();
        }
        if (failed > 0) {
            addToast(`No se pudieron archivar ${failed} caso${failed !== 1 ? 's' : ''}`, 'error');
        }
    }, [token, selectedLeads, addToast, fetchPipeline]);
