                                        ...
                                        <motion.tr key={m.id}
                                            ...
                                            <td className="py-3 px-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="size-8 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 flex-shrink-0">
                                                        <Church size={14} />
                                                    </div>
                                                    <p className="text-xs font-black text-slate-800 dark:text-white">{m.name}</p>
                                                </div>
                                            </td>
                                            <td className="py-3 px-4">
                                            ...
                                            <td className="py-3 px-4">
                                                {m.leader_name ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 text-[10px] font-black text-slate-600 dark:text-slate-300">
                                                        <div className="size-4 rounded-full bg-blue-600 text-white flex items-center justify-center text-[7px] font-black">
                                            ...
                                            <td className="py-3 px-4">
                                                <span className="text-xs font-black text-slate-500">{m.members_count ?? 0}</span>
                                            </td>
                                            <td className="py-3 px-4">
                        ...
                        {filtered.map((m, idx) => (
                            <motion.div key={m.id}
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div className="size-11 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600">
                                        <Church size={20} />
                                    </div>
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => openEdit(m)} className="p-2 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-xl text-slate-400 hover:text-blue-600 transition-all"><Pencil size={14} /></button>
                                        <button onClick={() => setDeleteId(m.id)} className="p-2 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl text-slate-400 hover:text-rose-600 transition-all"><Trash2 size={14} /></button>
                                    </div>
                                </div>
                                <h3 className="text-sm font-black text-slate-900 dark:text-white mb-1">{m.name}</h3>
                    ...
                    <div className="max-w-3xl mx-auto space-y-2">
                        {filtered.map((m, idx) => (
                            <motion.div key={m.id}
                                ...
                                <div className="size-10 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 flex-shrink-0"><Church size={18} /></div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-black text-slate-800 dark:text-white">{m.name}</p>
                        ...
                        <motion.aside
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 flex-shrink-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">{editing ? 'Editar' : 'Nuevo'} Ministerio</p>
                                        <h3 className="text-sm font-black text-slate-900 dark:text-white">{editing ? editing.name : 'Sin nombre'}</h3>
                                    </div>
                                </div>
                                <button onClick={() => setDrawerOpen(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl text-slate-400 transition-all">
                                    <X size={18} />
                                </button>
                            </div>
                ...
                {deleteId !== null && !drawerOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
                            ...
                            <div className="size-12 bg-rose-50 dark:bg-rose-500/10 rounded-xl flex items-center justify-center text-rose-600 mb-4">
                                <Trash2 size={20} />
                            </div>
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setDeleteId(null)} className="flex-1 py-3 rounded-xl border border-slate-200 dark:border-white/10 text-[11px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5 transition-all">
                                    Cancelar
                                </button>
                                <button onClick={() => handleDelete(deleteId!)}
                                    className="flex-1 py-3 rounded-xl bg-rose-600 text-white text-[11px] font-black uppercase tracking-widest hover:bg-rose-700 active:scale-95 transition-all shadow-lg shadow-rose-500/20">
                                    Eliminar
                                </button>
                            </div>
                        </motion.div>
