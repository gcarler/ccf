    ...
    return (
        ...
        >
            <main className="flex-1 overflow-y-auto scrollbar-thin">
                {/* Hero */}
                <div className="relative bg-gradient-to-br from-blue-600 via-sky-600 to-indigo-700 px-8 py-10 overflow-hidden">
                ...
                <div className="px-8 space-y-6 pb-12">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-3">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input
                                ...
                                className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl py-3 pl-11 pr-4 text-[13px] font-medium outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all placeholder:text-slate-400"
                            />
                        </div>
                        <button
                    ...
                    <AnimatePresence>
                        {showAddForm && (
                            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="bg-white dark:bg-[#252528] rounded-2xl border border-blue-200 dark:border-blue-700/30 p-6 shadow-xl">
                                <div className="flex items-center justify-between mb-5">
                                    <h3 className="text-[13px] font-black uppercase tracking-widest text-slate-800 dark:text-white">Registrar Servidor</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Nombre del servidor..." className="w-full px-3 py-2.5 text-[13px] font-medium bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 transition-all placeholder:text-slate-300" />
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Equipo</label>
                                        <select value={form.team} onChange={e => setForm(p => ({ ...p, team: e.target.value }))} className="w-full px-3 py-2.5 text-[13px] font-medium bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 transition-all">
                                            {TEAMS.map(t => <option key={t}>{t}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Rol en el Equipo</label>
                                        <select value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value }))} className="w-full px-3 py-2.5 text-[13px] font-medium bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 transition-all">
                                    ...
                                    <div>
                                        ...
                                        <input value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Habilidades, disponibilidad..." className="w-full px-3 py-2.5 text-[13px] font-medium bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/30 transition-all placeholder:text-slate-300" />
                                    </div>
                                </div>
                                <div className="flex justify-end gap-3 mt-5">
                                    ...
                                    <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-60">
                    ...
                    {!loading && filtered.length === 0 && (
                        <div className="py-24 text-center">
                            ...
                            <h3 className="text-lg font-black text-slate-400 uppercase tracking-widest mb-2">Sin servidores registrados</h3>
                            <p className="text-sm text-slate-400 font-medium mb-6">
                                {query ? `No se encontraron servidores con "${query}"` : 'Registra el primer servidor de la comunidad.'}
                            </p>
                            {!query && (
                                <button onClick={() => setShowAddForm(true)} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg hover:bg-blue-700 transition-all active:scale-95">
                                        ...
                                        {members.map((v, idx) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-start justify-between mb-3">
                                                    <div className={`size-11 rounded-xl bg-gradient-to-br ${getTeamColor(team)} flex items-center justify-center text-white text-[13px] font-black shadow-md`}>
                                                ...
                                                <h4 className="text-[13px] font-semibold text-slate-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                                    {v.name || 'Sin nombre'}
                                                </h4>
