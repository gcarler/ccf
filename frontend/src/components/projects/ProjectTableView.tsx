...
export default function ProjectTableView({ tasks }: { tasks: ProjectTaskRecord[] }) {
    return (
        <div className="bg-white dark:bg-white/5 rounded-lg border border-slate-100 dark:border-white/10 overflow-hidden shadow-sm">
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-slate-50/50 dark:bg-black/20 border-b border-slate-100 dark:border-white/5">
                        <th className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Tarea</th>
                        <th className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Estado</th>
                        <th className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Responsable</th>
                        <th className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Entrega</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {tasks.map((task) => (
                        <tr key={task.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                            ...
                            <td className="px-3 py-1.5 text-[11px] font-bold text-slate-400">
                            ...
                            <td className="px-3 py-1.5">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] font-semibold uppercase text-slate-400">{task.priority || 'Normal'}</span>
                                </div>
                            </td>
