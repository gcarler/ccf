...
export default function BuilderSidebar({
  ...
  return (
        <aside className="lg:col-span-3 rounded-lg border border-[hsl(var(--border))] dark:border-white/10 bg-[hsl(var(--bg-primary))] dark:bg-[hsl(var(--admin-bg-tertiary))] p-4 space-y-3">
          <label className="text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Sitio</label>
          <select value={siteKey} onChange={(e) => setSiteKey(e.target.value)} aria-label="Sitio" className="w-full rounded-md border border-[hsl(var(--border))] dark:border-white/10 bg-transparent px-3 py-2 text-sm">
          ...
          <div className="rounded-md border border-[hsl(var(--border))] dark:border-white/10 p-3 space-y-2">
            <p className="text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Nueva página</p>
            ...
            <button onClick={createPage} disabled={!canEdit} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2 text-2xs font-semibold uppercase tracking-wide text-white disabled:opacity-50"><Plus size={12} /> Crear vacía</button>
            <select value={pageTemplateKey} onChange={(e) => setPageTemplateKey(e.target.value)} aria-label="Plantilla de página" className="w-full rounded-lg border border-[hsl(var(--border))] dark:border-white/10 bg-transparent px-3 py-2 text-xs">
              {PAGE_TEMPLATES.map((template) => (
                <option key={template.key} value={template.key}>{template.label}</option>
              ))}
            </select>
            <button onClick={createPageFromTemplate} disabled={!canEdit} className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-[hsl(var(--border))] dark:border-white/10 px-3 py-2 text-2xs font-semibold uppercase tracking-wide disabled:opacity-50">
              <Plus size={12} /> Crear con plantilla
            </button>
          </div>

          <div className="space-y-2">
            <p className="text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Páginas</p>
            {pages.map((page) => (
              <button key={page.id} onClick={() => setActiveSlug(page.slug)} className={`w-full text-left rounded-lg border px-3 py-2 text-sm ${activeSlug === page.slug ? "border-primary/40 bg-primary/5" : "border-[hsl(var(--border))] dark:border-white/10"}`}>
                ...
                <p className="text-2xs uppercase tracking-wide text-[hsl(var(--text-secondary))]">/{page.slug} · {page.status}</p>
              </button>
            ))}
          </div>

          <div className="space-y-2 pt-2 border-t border-[hsl(var(--border))] dark:border-white/10">
            <p className="text-2xs font-semibold uppercase tracking-wide text-[hsl(var(--text-secondary))]">Plantillas rápidas</p>
            {popupTemplate && (
              <div className="rounded-lg border border-[hsl(var(--danger)/25%)] bg-danger-soft/70 p-3 dark:border-[hsl(var(--danger)/100%)]/20 dark:bg-[hsl(var(--danger))]/5">
                <p className="text-2xs font-semibold uppercase tracking-wide text-danger-text dark:text-[hsl(var(--danger))]">Pop-up</p>
                ...
                <button
                  ...
                  className="mt-2 w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[hsl(var(--danger))] px-3 py-2 text-2xs font-semibold uppercase tracking-wide text-white disabled:opacity-50"
                >
