        ...
        <div className="flex h-[700px] bg-white dark:bg-[#0b0d11] rounded-[2.5rem] border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm">
            <aside className="w-80 border-r border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] flex flex-col">
                ...
                <div className="p-4 space-y-2 flex-1 overflow-y-auto">
                    ...
                    {[1, 2, 3].map((index) => (
                        <div key={index} className="p-4 flex items-center justify-between group hover:bg-slate-200/50 dark:hover:bg-white/5 rounded-2xl transition-all cursor-pointer">
                            <div className="flex items-center gap-3">
                                <FileText size={16} className="text-slate-300 group-hover:text-indigo-500" />
                                <span className="text-[11px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white">Capitulo {index}</span>
                            </div>
            ...
            <main className="flex-1 flex flex-col bg-white dark:bg-[#0b0d11]">
                    ...
                    <div className="flex items-center gap-6">
                        {lastSaved && (
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Guardado a las {lastSaved.toLocaleTimeString()}</span>
                            </div>
                        )}
                        <button
                            onClick={handleSave}
                ...
                <div className="flex-1 p-10 relative">
                    ...
                    {error && (
                        <p className="absolute bottom-4 left-10 rounded-full bg-rose-50 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">
                            {error}
                        </p>
                    )}
                </div>
                ...
                <footer className="p-6 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-transparent flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Persistencia compartida</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
