...
export function EmbedSection({ section }: { section: CmsSection<"embed"> }) {
  ...
  // Usar el proxy del backend CCF que sirve el contenido sin la restricción.
  const isGooglePhotos = /photos\.app\.goo\.gl|photos\.google\.com/.test(embedUrl);
  const iframeSrc = isGooglePhotos
    ? `/api/cms/v2/embed/photos?url=${encodeURIComponent(embedUrl)}`
    : embedUrl;
  ...
  return (
    <section className="ccf-section-panel p-7 md:p-10" style={{ background: "var(--site-surface-container-low)" }}>
      ...
      {embedUrl ? (
        <div className="rounded-xl overflow-hidden" style={{ background: "var(--site-surface-container)", minHeight: "600px" }}>
          <iframe title={title} src={iframeSrc} className="w-full border-0" style={{ height: "600px" }} allowFullScreen sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" />
        </div>
      ) : (
