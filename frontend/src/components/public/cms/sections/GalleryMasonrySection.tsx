...
export function GalleryMasonrySection({ section }: { section: Partial<CmsSection<"gallery_masonry">> }) {
  ...
  return (
    <section className={`py-12 md:py-16 ${fullBleed ? "relative left-1/2 w-screen -translate-x-1/2 max-w-none px-0" : "mx-auto max-w-7xl px-4"}`}>
      {title && (
        <h2 className="mx-auto mb-8 max-w-4xl px-4 text-center text-3xl font-black tracking-tight md:mb-12 md:text-4xl lg:text-5xl" style={{ color: "var(--site-on-surface)" }}>
          {title}
        </h2>
      )}
            ...
            {images.length > 0 ? (
              <button type="button" onClick={() => setLightboxIndex(carouselIndex)} className="relative block aspect-[4/3] w-full cursor-zoom-in sm:aspect-[16/9]" aria-label={`Ampliar ${images[carouselIndex]?.alt || `imagen ${carouselIndex + 1}`}`}>
                ...
                {(images[carouselIndex]?.caption || images[carouselIndex]?.alt) && (
                  <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent px-5 pb-5 pt-12 text-left text-sm font-medium text-white">{images[carouselIndex]?.caption || images[carouselIndex]?.alt}</span>
                )}
              </button>
            ) : (
