...
export function FAROHeader() {
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-[hsl(var(--surface-2))]/80 backdrop-blur-xl border-b border-[hsl(var(--border))]/50 dark:border-white/5">
            <div className="container mx-auto px-4 py-2 flex items-center justify-between">
                <Link href="/" className="flex items-center gap-2">
                    <div className="bg-[hsl(var(--primary))] p-1.5 rounded-md">
                        <Church size={16} className="text-white" />
                    </div>
                    <span className="font-bold text-sm text-[hsl(var(--text-primary))] dark:text-white">
                        {SITE_NAME}
                    </span>
                </Link>
            </div>
