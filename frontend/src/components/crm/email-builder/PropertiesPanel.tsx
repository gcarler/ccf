...
function Align({ value, onChange }: { value: string; onChange: (v: string) => void }) { const opts = [{ v: 'left', I: AlignLeft }, { v: 'center', I: AlignCenter }, { v: 'right', I: AlignRight }]; return <div className="flex gap-1">{opts.map(({ v, I }) => <button key={v} onClick={() => onChange(v)} className={`size-7 flex items-center justify-center rounded border transition-colors ${value === v ? 'bg-[hsl(var(--primary))] text-white border-[hsl(var(--primary))]' : 'border-[hsl(var(--border))] text-[hsl(var(--text-secondary))] hover:bg-[hsl(var(--surface-2))]'}`}><I size={12} /></button>)}</div>; }
