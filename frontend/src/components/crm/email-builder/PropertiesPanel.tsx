...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div className="space-y-1"><label className="text-2xs font-semibold text-[hsl(var(--text-secondary))] uppercase tracking-wider">{label}</label>{children}</div>; }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        {block.type === 'text' && <p className="text-2xs text-[hsl(var(--text-secondary))] italic">Haz clic en el bloque para editar el texto</p>}
        ...
        {block.type === 'spacer' && <Field label="Altura"><input type="range" min={4} max={200} value={Number(block.props.height || 24)} onChange={e => onUpdate({ height: Number(e.target.value) })} className="w-full accent-[hsl(var(--primary))]" /><span className="text-2xs text-[hsl(var(--text-secondary))]">{String(block.props.height || 24)}px</span></Field>}
