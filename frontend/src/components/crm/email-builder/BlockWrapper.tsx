  ...
  return (
    <div ref={setNodeRef} style={style} className={clsx('group relative rounded-lg transition-all', isSelected ? 'ring-2 ring-[hsl(var(--primary))] ring-offset-1 ring-offset-[hsl(var(--bg-primary))]' : 'hover:ring-1 hover:ring-[hsl(var(--border))]', isDragging && 'opacity-50 z-50')} onClick={(e) => { e.stopPropagation(); onSelect(); }}>
      <div className={clsx('absolute -left-8 top-1/2 -translate-y-1/2 flex flex-col items-center gap-0.5 transition-opacity', isSelected || isDragging ? 'opacity-100' : 'opacity-0 group-hover:opacity-100')}>
        <button className="size-6 flex items-center justify-center rounded text-[hsl(var(--text-secondary))] hover:bg-[hsl(var(--surface-2))] cursor-grab active:cursor-grabbing" {...attributes} {...listeners}><GripVertical size={12} /></button>
      </div>
      <div className={clsx('absolute -top-3 right-2 flex items-center gap-1 transition-opacity z-10', isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100')}>
        <button onClick={(e) => { e.stopPropagation(); onDuplicate(); }} className="size-6 flex items-center justify-center rounded bg-[hsl(var(--surface-1))] border border-[hsl(var(--border))] text-[hsl(var(--text-secondary))] hover:text-[hsl(var(--primary))]" title="Duplicar"><Copy size={11} /></button>
        <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="size-6 flex items-center justify-center rounded bg-[hsl(var(--surface-1))] border border-[hsl(var(--border))] text-[hsl(var(--text-secondary))] hover:text-rose-500" title="Eliminar"><Trash2 size={11} /></button>
      </div>
