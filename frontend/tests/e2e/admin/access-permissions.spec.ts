...
test.describe('Admin Access - granular permissions', () => {
  ...
  test('loads the access page and shows both tabs', async ({ page }) => {
    ...
    await expect(page.locator('main')).toContainText(/Seguridad y Accesos/i);
  });
  ...
  test('switches to the Users tab and filters records', async ({ page }) => {
    ...
    await expect(page.locator('main')).toContainText(/Activos|Inactivos/i);
    ...
    if (await search.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('main')).toContainText(/Sin usuarios|No hay usuarios|no hay usuarios/i);
    }
  });
  ...
  test('opens the permission matrix when selecting a role', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Matriz de Operaciones/i })).toBeVisible();
    await expect(page.getByText(/CRM Pastoral/i)).toBeVisible();
  ...
  test('toggles a permission level and saves with mocked API', async ({ page }) => {
    ...
    const crmRow = page.locator('.permission-card').filter({ hasText: /CRM Pastoral/i });
    ...
    await expect(page.locator('text=Permisos del rol actualizados')).toBeVisible();
  });
  ...
  test('opens user drawer and shows effective vs override permissions', async ({ page }) => {
    ...
    // User drawer shows the permission matrix and platform role badge
    await expect(page.getByRole('heading', { name: /Matriz de Operaciones/i })).toBeVisible();
    await expect(page.getByText(/Rol de plataforma:/i)).toBeVisible();
  });
});
