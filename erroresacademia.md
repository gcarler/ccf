...
| M-07 | ✅ CERRADO 2026-08-10 | `crud/academy.py::list_forum_threads` acepta kwargs opt-in `skip`/`limit`, aplica sede y excluye `ForumThread.deleted_at`. El API canónico filtra hilos archivados en listado, detalle y comentarios. `ForumThread.deleted_at` + índice fueron añadidos al modelo y a la migración canónica `20260810_0001_academy_forum_threads_deleted_at`; regresión HTTP cubre listado/detalle/comentarios. | `20260810_0001` |
