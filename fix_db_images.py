...
for (table,) in tables:
    ...
    text_cols = [col[1] for col in columns if 'TEXT' in col[2].upper() or 'VARCHAR' in col[2].upper() or 'JSON' in col[2].upper()]
    ...
    for row in rows:
        ...
        if changed:
            ...
            cursor.execute(f"UPDATE {table} SET {set_clause} WHERE rowid = ?", values + [rowid])
            updated += 1
