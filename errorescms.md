...
F-03..F-05/F-07/F-08/F-10 cerrados (ver tabla detallada abajo).
Backlog COMPLETO: 0 hallazgos pendientes.
...
| F-04 | ✅ CERRADO | `CmsRedirect` ahora soporta 3 `match_type`: `exact` (default, back-compat), `wildcard` (glob con `*`/`?` via `fnmatch`) y `regex` (Python `re.search` con captura de grupos `$0`/`$1`/`$2`). Columnas nuevas `match_type` (String(20), server_default `exact`) y `priority` (Integer, server_default `0`), indexadas; migración `20260723_0006` añade columnas (additivo, reversible, back-compat vía server_default). Helper `resolve_redirect(db, *, site_key, path)` en `enterprise_cms.py` aplica prioridad exact > wildcard > regex; dentro de cada tipo recorre por `priority DESC` y luego `len(from_path) DESC` (más específico primero). El regex con grupos de captura sustituye `$1`/`$2` en `to_path`. Regex inválido en runtime se salta defensivamente (no raise). Endpoint `GET /cms/v2/resolve-redirect?site_key=...&path=...` expone la capability e incrementa `hit_count` (best-effort). `POST /redirects` valida `match_type` en (`exact`/`wildcard`/`regex`) y compila el regex para fallar `422` temprano si el patrón es inválido. `list_redirects` ahora expone `match_type` y `priority` en la respuesta. 16 tests de regresión: CRUD-direct (exact/wildcard/regex/captura/prioridad/invalid-regex/inactive/no-match/same-type) + API (invalid match_type 422, invalid regex 422, valid regex/wildcard 200, list expone campos) + endpoint `/resolve-redirect` (match, no-match 404, regex with captura). | `0d187d1b` |
| F-05 | ✅ CERRADO | El router CMS v2 (`backend/api/cms_v2.py:50`) ahora aplica `rate_limiter(limit=600, window_seconds=60)` como dependency a nivel router (`APIRouter(dependencies=[Depends(...)])`), cubriendo los 67 endpoints admin que antes no tenían rate limiting. Los 12 endpoints públicos con override propio (`/public/sites/.../theme`, `/sitemap.xml`, `/robots.txt`, `/track`, `/images/resize`, `/posts`, `/pastoral-team`) conservan su límite más restrictivo — `rate_limiter` usa `key = f"rate:{identifier}:{path}"` en Redis, por lo que dos paths distintos generan dos contadores distintos (no se falsean entre el router-level y el endpoint-level). El counter de 600/min/admin es generoso (no afecta uso humano-admin) pero protege contra un script autenticado abusivo (NO DoS anónimo — eso lo maneja slowapi+auth_v3 externamente). En pytest el limiter es NO-OP (env `PYTEST_CURRENT_TEST` bypass) para no correr 429 en la 100+ test suite. 9 tests de regresión: structural (router tiene dep, dep es `rate_limiter` closure, constante `PUBLIC_CMS_RATE_LIMIT` existe), herencia (3 endpoints públicos conservan su override de `rate_limiter`), smoke (router carga con 79 routes, prefix `/cms/v2` correcto). | `ab0fd52b` |
| F-07 | ✅ CERRADO | Endpoint `GET /cms/v2/sites/{site_key}/seo-snapshots` creado en `api/cms_v2.py`. Expone el listado paginado de snapshots SEO históricos por site (`CmsSeoSnapshot` rows crudos, no series agregadas como `get_seo_trend`). Schema `CmsSeoSnapshotRead` en `schemas/cms.py` expone `id`, `site_id`, `sede_id`, `captured_date`, `captured_at`, `average_score`, `total_pages`, `pages_with_errors`, `critical_issues`, `by_severity_json`. CRUD `list_seo_snapshots(db, *, site_id, limit=90, offset=0)` en `crud/cms.py` retorna `(rows, total)` ordenados DESC por `captured_date` (más reciente primero); exportado en `crud/__init__.py` y `schemas/__init__.py`. Endpoint usa `_get_scoped_site_or_404` para scope Axioma 3 + `_assert_role(..., CMS_EDITOR_ROLES)` para autorización, `response_model=PaginatedResponse[schemas.CmsSeoSnapshotRead]`. Paginación con `skip` (default 0, ge=0) y `limit` (default 90, 1-365 días). 8 tests de regresión: CRUD-direct (orden DESC por `captured_date`, paginación respeta `total`, filtrado por `site_id` excluye otros sites, empty cuando no hay snapshots), API (200 site existente retorna items+total+skip+limit, 404 site inexistente, paginación `skip=0&limit=10` respetada), schema roundtrip desde ORM con todos los campos. | `3636fce9` |
...
| F-08 | ✅ CERRADO | `crud/cms.py::cleanup_old_publish_logs(db, retention_days=90, dry_run=False)` purga `CmsPublishLog` con `created_at < now - retention_days`. Hard-delete safe (REGLAS.md §6 solo protege `personas` y entidades pastorales/transaccionales; `CmsPublishLog` es log de auditoría operacional con `actor_persona_id` nullable). Exportado en `crud/__init__.py`. `scheduler.py::_run_scheduling_pass` lo invoca con el default e incluye `publish_logs_purged` en `counts` y en el heartbeat `work_done` (evita heartbeat silencioso cuando el scheduler purga muchos logs viejos). 7 tests de regresión en `TestF08CleanupOldPublishLogs` (purge, dry_run, custom retention, recent-keep, empty table, multi-log batch) + `TestF08SchedulerIntegration` (la clave `publish_logs_purged` está en el dict retornado por `_run_scheduling_pass`). | `fc80da41` |
| F-10 | ✅ CERRADO | Un `CmsMediaItem` es "huérfano" cuando está activo (`status != "archived"`), pertenece a la sede del actor, y NO aparece entre los IDs recolectados por `collect_section_media_ids` de las secciones activas de los sites de esa sede. `crud/cms.py` añade `cleanup_orphan_cms_media` (API path con defense-in-depth re-check de sede via `_actor_sede_or_none_cms`), `cleanup_orphan_cms_media_scheduled` (variante para `scheduler.py` sin `actor_user_id`, preservando Axioma 3 vía scope explícito `sede_id`), y `_apply_cleanup_orphan_cms_media` interna compartida. `permanent=True` hard-deletea row + archivo físico con guard de path traversal H-05 (una url malformada como `uploads/../../etc/passwd` no sale de `/root/ccf/uploads`: el row se archiva en su lugar sin `os.remove`); default soft-archive (`status=archived`). Endpoints `api/cms.py`: `POST /api/cms/media/cleanup` con `dry_run` + `permanent` flags. Se usa POST (no DELETE) para evitar colisión de routing con `DELETE /cms/media/{item_id}` (Starlette matchea el path-param primero). Scope por sede del actor (`_actor_sede_or_none`); superadmin sin sede retorna 0 (no permite limpiar a nivel plataforma: el set de referenciados mezclaría sedes y podría archivar media que otra sede usa). `scheduler.py::_maybe_run_orphan_media_cleanup` opt-in via `CMS_ORPHAN_MEDIA_CLEANUP=1` (default OFF para no mutar media en el cron de cada minuto sin opt-in explícito del operador); el endpoint API sigue siendo la vía primaria. Counts del scheduler incluye `orphan_media_archived` (en `work_done` y en el log heartbeat). 16 tests de regresión en `TestF10CleanupOrphanCmsMedia` (dry_run count, soft-archive preserva referenciados, permanent hard-delete, path traversal archive-fallback, None sede, no-orphans, archived skip), `TestF10CrossSedeDefenseInDepth` (actor de otra sede no muta), `TestF10ScheduledVariant` (scheduled sin actor, dry_run, None sede), `TestF10SchedulerIntegration` (opt-out default, opt-in dry-run count, opt-in real purge), `TestF10ApiEndpoint` (dry_run 200 con `purged`, soft-archive via API). | `4d1ba06a` |
