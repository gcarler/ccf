...
- [ ] 12. **¿Busco personas?** Usar `filtroAPersona`/`filtroAPersonas` de `@/lib/filtroAPersonas` (acentos, nombres compuestos, `@` = username, sin `@` = nombre/apellido). No escribir filtros propios con `startsWith`/`includes` crudos.
