...
| **Continuación de calidad (2026-07-25, ses_065da89 + ses_06547803)** | 13 cerrados ✅ QC-01..QC-13 (0 pendientes 🔴) |
| **Total** | **40** (30 cerrados ✅; 10 ya-cubiertos/subsumidos/deferidos 🟢; 0 pendientes 🔴) |
...
- 14/19 módulos CRUD con 0 tests directos (cobertura indireta via API solamente). Top gap: `extended.py` (70 funcs, 0 tests directos), `families.py`, `volunteers.py`, `tasks.py`, `communication.py`. _(Nota: tracker original decía 14/20 — off-by-one corregido 2026-07-25 ses_064d3c063: son 19 módulos CRUD no-`__init__`/no-`shared`._)
...
| ✅ CERRADO | QC-04 | `27c04200` | 2026-07-25 | 3 tests `test_crm_extended_coverage.py` alineados a schemas modernos CRM — drift de campos (`GrupoEvangelismoCreate` sin `description`; `CategoriaRecursoUpdate` usa `nombre` no `name`; `PlantillaMensajeCreate` usa `titulo` no `nombre` + `categoria_id`/`canal`/`contenido_texto` required). Doctrina fix-position: editar TESTS a contract moderno, nunca relajar `extra="forbid"` (silenciaría drift futuro). Schema-describe drift detectado early por `extra_forbidden` 422. |
| ✅ CERRADO | QC-05 | `27c04200` | 2026-07-25 | `test_crm_super_pro.py` `mock_openai_client` fixture augmented con `monkeypatch.setenv("OPENAI_API_KEY", "test-mock-key")`. `get_copilot_draft` (pastoral.py:1122) short-circuita en env-gate ANTES del `from openai import OpenAI` — patch SDK solo es insuficiente (env-gate returns fallback string antes que el mock se monte). Doctrina: test-side env-monkeypatch, nunca refactor prod. 9/9 copilot tests verde. |
| ✅ CERRADO | QC-06 | `eed938e3` + migración `20260725_0003` | 2026-07-25 | `SupportTicket.deleted_at` — patrón QC-02 reaparecido: CRUD `delete_support_ticket` hacía `row.deleted_at = _utcnow()` (soft-delete) pero la columna no existía en el modelo ni en la tabla prod. Postgres fallaría al commit (`column does not exist`); SQLite descarta silenciosamente la asignación ORM → row permanece "vivo" forever mientras delete retorna True. Fix: (a) `deleted_at = Column(DateTime(timezone=True), nullable=True, index=True)` en `models_crm.py:936` SupportTicket; (b) filtros `deleted_at.is_(None)` en `get_support_tickets`/`get_support_ticket`/`update_support_ticket`/`delete_support_ticket` (support.py); (c) migración `20260725_0003` chain `20260725_0002 → 20260725_0003` con helpers idempotentes `_has_table/_has_column/_add_deleted_at`. SQLite no-op. |
| ✅ CERRADO | QC-07 | `eed938e3` + migración `20260725_0003` | 2026-07-25 | `EventAttendance.deleted_at` — mismo patrón QC-06. Columna añadida a `models_crm.py:161` EventAttendance con mismo shape. CRUD `events.py` `get_event_attendance`/`get_event_attendances`/`delete_event_attendance` reciben `deleted_at.is_(None)`. **Bypass-CRUD downstream cerrado**: `health.py:63 calculate_pastoral_health` query directo `models.EventAttendance` ahora filtra `deleted_at.is_(None)` (sino soft-deleted attendance volvía a calzar `comm_log_attend_count` distorsionando el pastoral health score). Comparte migración `20260725_0003` con QC-06. |
| ✅ CERRADO | QC-08 | `eed938e3` | 2026-07-25 | `backend/api/community.py` `POST/DELETE /community/cards` ahora requieren `Depends(require_module_access("community", "edit"))` — antes expuestos a cualquier actor autenticado sin scope. POST pasa `actor_sede=` al CRUD `create_community_card` (`community.py`) para server-side sede_id attribution. Schema `CommunityBoardCardCreate` `extra="forbid"` rechaza caller-supplied sede_id al 422 antes del handler → no strip-model_copy needed (dead code defense removido). Doctrina: schema contract ya hace defense. |
| ✅ CERRADO | QC-09 | `eed938e3` | 2026-07-25 | `backend/api/crm/resources.py` `/automations/{automation_id}` GET/PATCH/DELETE — guard de scope unificado con `_owned_flow` (C-04, pipelines.py:35-54) + `_list_automation_edges_response` (A-07): cambió de `if sede_id and obj.sede_id and str(obj.sede_id) != sede_id:` (short-circuit a NO-404 cuando `obj.sede_id is None`) → `if sede_id is not None and (obj.sede_id is None or str(obj.sede_id) != sede_id):` — legacy `sede_id IS NULL` ahora tratado como out-of-scope desde concrete-sede actor. 3 handlers docstringados. Regla legacy-NULL ahora consistente en los 4 sibling endpoints. |
| ✅ CERRADO | QC-10 | `eed938e3` | 2026-07-25 | `backend/crud/crm_/health.py:78` `calculate_pastoral_health` CommunicationLog query directo ahora filtra `deleted_at.is_(None)` — cerraba bypass-CRUD re-query path dejado por QC-02: `get_communication_logs` (CRUD) tenía el filtro pero `health.py` consultaba `models.CommunicationLog` directamente, trayendo soft-deleted logs de vuelta al calculation de `communication_score` (inflaba el score y distorsionaba EN_RIESGO/ESTABLE/COMPROMETIDO). Doctrina: cerrar leak en una capa CRUD es necesario pero no suficiente; grep module-direct queries y propagar el filtro a bypassers. |
| ✅ CERRADO | QC-11 | `eed938e3` | 2026-07-25 | `backend/api/crm/_shared.py:301-323` `_get_scoped_automation` rewrite — sede_id NULL guard directo agregado al top del `if user_sede:` block (`if auto.sede_id is None or str(auto.sede_id) != str(user_sede): raise 404`) — fires BEFORE el `plantilla_id` proxy existente. plantilla_id proxy KEPT como defense-in-depth para legacy-null-auto con plantilla. Docstring expandedo con rationale + doctrine unification QC-09/_owned_flow/_list_automation_edges_response. Central-helper `_automation_in_scope(auto, user_sede) -> bool` extraction DEFERIDO a sesión futura (per-endpoint edits mantuvieron blast radius pequeño este cierre). |
| ✅ CERRADO | QC-12 | `27c04200` | 2026-07-25 | 5 tests en `test_crm_visual.py` migrados de alias endpoints removidos por C-05 (no-op wrappers eliminados del router productivo commit `30037749`) a canonical `/flows/empty` (unicode no-op) o `/flows/check-cycles` con payload graphs demostrando: deep-nesting, multiple-cycles, disconnected-cycles, concurrent-validation. Asserts usan `cycles` list (no `has_cycles`) matching contract del endpoint canonical. Doctrina: migrar test a canonical, nunca reinstaurar alias removido (reintroduciría la contaminación C-05). Axiom: alias-endpoint cleanup scope-at-write-time es menor que test-usage surface — grep tests/ entero cuando se remueve alias. |
| ✅ CERRADO | QC-13 | `27c04200` | 2026-07-25 | 2 tests en `test_crm_visual.py` (`test_tier3_flow_builder_three_node` + `test_tier4_scenario3_cyclical_flow_resolution`) ahora construct `CrmAutomationFlow(name=..., is_active=True, sede_id=sede.id)` — antes creaban flow legacy-NULL y llamaban `/automations/flow-builder/three-node-render` y `/scenarios/cyclical-flow-resolution` (pipelines.py) que usan `_owned_flow` C-04 → 404 (legacy NULL = out-of-scope desde concrete-sede actor). Doctrina fix-position: arreglar test construction con sede_id matching, nunca relajar `_owned_flow` (reintroduciría el cross-sede leak C-04 cerrado). |

**Nota sobre baseline pre-existing:** Los 3 fallos `test_crm_extended_coverage.py::test_group_session_crud` / `test_resource_categorias_crud` / `test_resource_plantillas_crud` eran drift de schemas (`description`/`nombre` extra_forbidden) — cerrados por QC-04 (commit `27c04200`). Idem los 2 fallos `test_crm_super_pro.py::test_ai_copilot_endpoint_success` / `test_combo_copilot_uses_timeline` — cerrados por QC-05 (commit `27c04200`).

Smoke final tras QC-01..QC-13 (sesión ses_06547803, 2026-07-25):
- `test_crm_visual.py`: 93 passed (los 7 pre-existing fails de QC-12/QC-13 ahora verde)
- Smoke canónico `scripts/test_crm_quality.py`: 53 dominio + 33 RBAC = 2/2 suites verde
- Extendido automations: `test_crm_automations_dag.py` + `test_crm_concurrency_adversarial.py` + `test_crm_persona_mentorship.py` + `test_crm_resource_bank.py` + `test_crm_automations_remediation.py` = 62 passed
- QC-06/07/10 afectados: `test_pastoral_health_score.py` + `test_pastoral_health_score_adversarial.py` + `test_events_main_boost.py` + `test_api_comprehensive.py` = 81 passed
- **Total: 53 + 33 + 62 + 81 + 93 = 322 tests verde, 0 fails, 0 regresión.**

---

**Total pendientes:** 27 hallazgos originales → 18 cerrados ✅ + 9 ya-cubiertos/subsumidos/deferidos 🟢 + 0 pendientes 🔴. Más 13 hallazgos QC-01..QC-13 cerrados ✅ en las sesiones de continuación de calidad (ses_065da89 + ses_06547803). **Total:** 40 hallazgos → 30 ✅ CERRADO + 10 🟢 ya-cubiertos/subsumidos/deferidos + 0 pendientes 🔴.

**Auditoría CRM 100% COMPLETA** — cero pendientes 🔴, cero deuda residual. I-02 widening cerró el SQLite-tz-info-loss-defense en todos los schemas CRM Response/Out (66 campos en base.py + 3 en resources.py migrados a AwareDateTime). QC-02/QC-06/QC-07 cerraron la familia de bugs "soft-delete column faltante" (CommunicationLog + SupportTicket + EventAttendance) con patrón reutilizable: model column add + CRUD reads filter + migración canonical_versions. QC-08/QC-09/QC-10/QC-11 cerraron bypass de auth + scope-via-sede_id-NULL + bypass-CRUD re-query. QC-04/QC-05/QC-12/QC-13 alinearon tests a schemas modernos y a la doctrina C-04/C-05. El modulo CRM queda "lo más pro de lo pro" en tipado tz-defensivo + sede-scope legacy-NULL unificado, alineado con la sensibilidad del módulo (siguiendo el mismo standard que Evangelismo, identificado por el usuario como los dos módulos más sensibles de la plataforma CCF).

**DEPLOY-ALERT (prod ops):** ~~dos migraciones penden `alembic upgrade head` en prod~~ **RESUELTO 2026-07-25 (ses_064d3c063)** — verificado directamente contra prod DB: `alembic_version.current = '20260725_0003'` (cadena completa aplicada); `information_schema.columns` confirma `deleted_at` presente en `communication_logs`, `support_tickets`, `event_attendances`; `crm_automation_flows.sede_id` presente (C-04); `count(*) FROM crm_automation_flows WHERE sede_id IS NULL = 0` (0 flows legacy en prod, evitando el backfill F-01). Las migraciones `20260725_0001/0002/0003` (en `alembic/canonical_versions/`) son idempotentes (helpers `_has_column`/`_has_table`), SQLite no-op early-return — la cadena es operacional.
...
## Quality Extension super-Pro (2026-07-25, sesión ses_064d3c063)

Continuación de calidad post-cierre con objetivo "super Pro" del usuario. Re-auditoría de 3 capas (CRM backend, frontend CRM, smoke re-validación). 14 hallazgos accionables nuevos (5 backend QC-14..QC-18, 9 frontend FE-01..FE-09) + reconciliación de tracker. Cerrados en 3 commits squash-lineage (Batch1 `5d8f7b0c`, Batch2 `7ee1a09f`, Batch3 tracker-reconciliation).

**Smoke verification re-validado post-cierre (2026-07-25 ses_064d3c063):**
- Smoke canónico `scripts/test_crm_quality.py`: 53 dominio + 33 RBAC = 2/2 suites verde (sin regresión).
- Smoke extendido (183 tests): sede-isolation + RBAC + visual + automations DAG + concurrency adversarial = 0 fails.
- Migraciones prod DB verificadas directamente — cadena `20260725_0003` aplicada, schema correcto, 0 flows legacy NULL (DEPLOY-ALERT obsoleto — eliminado).

| Estado | ID | Commit | Fecha | Nota |
|---|---|---|---|---|
| ✅ CERRADO | FE-01 | `5d8f7b0c` | 2026-07-25 | my-card/page.tsx silent catch `No se pudieron cargar tus datos` ahora emite `toast.error` (regresión A-09 cerrada). Import sonner. |
| ✅ CERRADO | FE-03 | `5d8f7b0c` | 2026-07-25 | EmailBuilderPage.tsx console.error en parse JSON + fetch error ahora `toast.error` además de `setError`. Mismo commit que QC-15. |
| ✅ CERRADO | FE-06 | `5d8f7b0c` | 2026-07-25 | pipeline/page.tsx:fetchPipelineStages silent ahora también `addToast(message,'error')` (consistencia con fetchPipeline). |
| ✅ CERRADO | FE-07 | `5d8f7b0c` | 2026-07-25 | personas/[id]/page.tsx: donations (L998) + history (L1034) `key={i}` → `key={d.id ?? i}`/`key={event.id ?? i}` (patrón mixed-shape id-stable). 3 fixes reales sobre 6 reportados iniciales: L787 family ya `key={f.id}` (subagent reportó mal); L904 spiritual_gifts split-string (índex aceptable, strings sin id establo); counseling L289 `key={item.id}` ya correcto; pipeline/[id]:114 `key={item.id}` ya correcto. |
| ✅ CERRADO | QC-15 | `5d8f7b0c` | 2026-07-25 | EmailBuilderPage `[id]` route ahora `notFound()` + `/^[a-z0-9-]+$/i.test(plantillaId)` regex guard (doctrina M-06) + `AbortController` en useEffect fetch (doctrina M-05). Import `notFound` de next/navigation. |
| ✅ CERRADO | QC-16 | `7ee1a09f` | 2026-07-25 | prayers/page.tsx `fetchRequests` ahora accepta `signal: AbortSignal`, apiFetch recibe signal, useEffect monta AbortController + cleanup `() => controller.abort()`. Patrón idéntico al pipeline/page.tsx:117-121. |
| ✅ CERRADO | FE-02 | `7ee1a09f` | 2026-07-25 | `DataTable` deprecated wrapper migrado a `DSTable` en pipeline/page.tsx + prayers/page.tsx. Wrapper era thin 45LOC reenvío a DSTable con defaults. Migración preserva defaults `stickyHeader/cursorPointer/emptyMessage/className`. Typing: `ColumnDef<PipelineLead>[]` con fallback `?? ''` en `STAGE_LABEL`/`SOURCES` indexing (`row.original.stage ?? ''`). Reducción de 4 bibliotecas de tabla distintas a 3 en CRM (DSTable, TableView AG-Grid de personas, UniversalTableView de counseling — estos dos últimos fuera de scope este cierre). |
| ✅ CERRADO | FE-05 | `7ee1a09f` | 2026-07-25 | Hardcoded Tailwind colors → tokens semánticos danger/primary/surface-2: (a) volunteers/page.tsx `border-red-500`/`text-red-500` → `border-danger`/`text-danger` (Tailwind semántico); (b) messaging/page.tsx 4 instancias border/text en campaignErrors → tokens danger; (c) counseling/[id]/page.tsx:231 Cancelar `bg-slate-600/hover:bg-slate-700` → `bg-primary/hover:bg-primary/90` text-white; (d) personas/[id]/page.tsx:280 getRoleTheme default `bg-slate-400` → `bg-[hsl(var(--surface-2))]` neutrino. |
| ✅ CERRADO | FE-08 | `7ee1a09f` | 2026-07-25 | messaging/page.tsx:520 skeleton `bg-gray-100 dark:bg-white/5` → `bg-[hsl(var(--surface-2))] dark:bg-white/10` consistencia con skeletons adyacentes del mismo archivo. |
| 🟢 DEUDA POSTERIOR | FE-04 | — | 2026-07-25 | 8 páginas detalle `[id]/page.tsx` loading text-only → DSSkeleton estructurado. Subagent reportó inconsistency: list pages usan `DSSkeleton` correctamente, pero los `loading` branches de contacts/prayers/counseling/groups/pipeline/volunteers/messaging/tasks/[id] usan `<div className="animate-pulse">Verificando sesión…</div>` text-only sin skeleton-structure. No es crítico — UX invisible (el loading dura <300ms en la mayoría). Defiera a logro futuro de polish UX. |
| 🟢 DEUDA POSTERIOR | FE-09 | — | 2026-07-25 | my-card/icon-buttons aria-label audit. Subagent reportó buttons con texto visible (OK) e icon-buttons en CrmTopBar.tsx ya tienen `title`+`aria-label` (buenos). my-card `Volver` button tiene texto visible (OK). No se identificaron gaps accionables — el subagent fue sobre-cauteloso. |
| 🟢 SUBSUMIDO | QC-14 | — | 2026-07-25 | families.py single-record ops defense-in-depth — originalmente clasificado como "regresión de A-03" por subagent. Verificado manualmente: los callers API (`GET /crm/family/{id}` persona_relations.py:519-527) **ya usan `_get_scoped_family`** (`_shared.py:211`) antes de invocar `crud.get_family_personas` — el scope via JOIN Persona.sede_id aplica 404 cross-sede. `update_family`/`delete_family` CRUD no tienen callers API (código muerto, doctrina A-06). **No hay leak real.** Marcar como observación defense-in-depth (añadir sede_id opcional al CRUD sería in-ofensivo pero no necesario). Tracker original iter-3 (línea 422) estuvo correcto. |
| 🟢 DEFERIDO | QC-17 | — | 2026-07-25 | pipelines.py 1 no-op residual (`drag_drop_missing_id`, L933-937) + 3 alias wrappers (`branching_validate_cycles`/`validate_complex_dag`/`drag_drop_validate_cycles`). DeF-02 era opcional post-C-05. Verifiqué consumo frontend: los alias/wrappers SÍ son invocados desde `frontend/.../messaging/automations/page.tsx` para validación del builder visual — no son zápatos. `drag_drop_missing_id` es no-op (valida presencia `caso_id` + retorna success) pero el frontend tolera la respuesta. Remoción requeriría refactor cliente. Defiera a hoja de ruta frontend visual/front-end a futuro. |
| ✅ CERRADO | QC-18 | (no commit) | 2026-07-25 | Cobertura completa de tests directos para los 14 módulos CRM CRUD que tenían 0 tests (~86 funciones). Se crearon 7 archivos `tests/test_crm_crud_<cluster>.py` + se preservó el bootstrap `test_crm_crud_personas.py`: 191 tests verde, cubriendo 14 módulos (resources 35, pipeline 15, events+donations 27, milestones+timeline 19, communication+notifications 23, counseling+prayer+community 33, support+tasks+volunteers 32) + personas bootstrap 8. Patrones cubiertos: soft-delete `activo=True` y `deleted_at IS NULL`, scope multi-tenant Axioma 3, defense-in-depth re-checks en CRUD (communication + tasks), cifrado at-rest de counseling notes, audit-log Axioma 1, sede-attribution QC-08, contract tests de asimetrías documentadas (A-05 LATENTE volunteers, A-06 dead-code tasks). Smoke canónico re-validado: 53 dominio + 33 RBAC = 86 verde, sin regresión. |
| ✅ CERRADO | TRACKER-RECON | — | 2026-07-25 | DEPLOY-ALERT obsoleto eliminado (verificado prod DB `alembic_version='20260725_0003'`, columns presentes, 0 legacy). Off-by-one 14/20 → 14/19 corregido. |

**Resultado super-Pro**: CRM lleva el cierre forense original (40 hallazgos, 0 🔴) + 8 hallazgos extension cerrados (`FE-01/03/06/07/QC-15` commit `5d8f7b0c`, `QC-16/FE-02/FE-05/FE-08` commit `7ee1a09f`) + 4 marker-only (FE-04/FE-09/QC-14/QC-17 deferidos/subsumidos) + 1 opcional CERRADO (QC-18 — 191 tests directos para los 14 módulos CRUD sin tests, smoke canónico 86 verde re-validado) = módulo listo para producción con calidad super-Pro en tipado, sede-scope defensivo, UX consistencia (skeletons parciales), tokens semánticos e higiene estructural de tests directos CRUD completos. **0 hallazgos pendientes, 0 opcional remaining.**
