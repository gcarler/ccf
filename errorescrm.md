...
| 🔴 PENDIENTE (opcional) | QC-18 | — | 2026-07-25 | 14/19 módulos CRM CRUD con 0 tests directos (~119 funciones). Como los 14 módulos ya reciben cobertura indireta via API integration tests, no hay gap funcional real — es solo hygiene estructural. Dejo como debtía técnica opcional: empezar con `tests/test_crm_crud_personas.py` (27 funcs) y `tests/test_crm_crud_resources.py` (18 funcs) para cobrar las escalas más gordas vía smoke pytest server-side. |
...
**Resultado super-Pro**: CRM lleva el cierre forense original (40 hallazgos, 0 🔴) + 8 hallazgos extension cerrados (`FE-01/03/06/07/QC-15` commit `5d8f7b0c`, `QC-16/FE-02/FE-05/FE-08` commit `7ee1a09f`) + 4 marker-only (FE-04/FE-09/QC-14/QC-17 deferidos/subsumidos) + 1 opcional remaining (QC-18) = módulo listo para producción con calidad super-Pro en tipado, sede-scope defensivo, UX consistencia (skeletons parciales) y tokens semánticos. Smoke 86+183 verde verificado en sesión.
