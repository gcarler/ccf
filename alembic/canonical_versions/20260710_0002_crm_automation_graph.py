...
def upgrade() -> None:
    ...
    res = connection.execute(
        sa.text("SELECT id, next_automation_id FROM crm_automations WHERE next_automation_id IS NOT NULL")
    )
    for row in res.all():
        source_id = row[0]
        target_id = row[1]
        ...
        connection.execute(
            sa.text("INSERT INTO crm_automation_edges (id, source_id, target_id) VALUES (:id, :source_id, :target_id)"),
...
def downgrade() -> None:
    ...
    res = connection.execute(
        sa.text("SELECT source_id, MIN(CAST(target_id AS VARCHAR)) FROM crm_automation_edges GROUP BY source_id")
    )
    for row in res.all():
        source_id = row[0]
        target_id = row[1]
        connection.execute(
            sa.text("UPDATE crm_automations SET next_automation_id = :target_id WHERE id = :source_id"),
            {"target_id": target_id, "source_id": source_id},
        )

    # 3. Drop crm_automation_edges table and indices
    op.drop_index("ix_crm_automation_edges_target_id", table_name="crm_automation_edges")
    op.drop_index("ix_crm_automation_edges_source_id", table_name="crm_automation_edges")
