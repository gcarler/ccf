...
def upgrade():
    op.add_column(
        "sesiones_grupo",
        sa.Column("estado_habilitacion", sa.String(20), nullable=False, server_default="DESHABILITADO"),
    )
    op.add_column("sesiones_grupo", sa.Column("habilitado_por", sa.UUID(), nullable=True))
    op.add_column("sesiones_grupo", sa.Column("habilitado_en", sa.DateTime(timezone=True), nullable=True))
