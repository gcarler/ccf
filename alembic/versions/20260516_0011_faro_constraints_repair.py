...
def upgrade() -> None:
    ...
    if inspector.has_table("sesiones_grupo") and _has_column(inspector, "sesiones_grupo", "reported_by_member_id"):
        if not _has_fk(
            inspector,
            "sesiones_grupo",
    ...
    if inspector.has_table("asistencias") and _has_column(inspector, "asistencias", "absence_reason"):
        if not _has_index(
            inspector,
            "asistencias",
...
def downgrade() -> None:
    ...
    inspector = sa.inspect(bind)
    if inspector.has_table("asistencias") and _has_index(inspector, "asistencias", "ix_asistencias_absence_reason"):
        op.drop_index(
            "ix_asistencias_absence_reason",
    ...
    if inspector.has_table("sesiones_grupo") and _has_fk(
        ...
        with op.batch_alter_table("sesiones_grupo") as batch_op:
            batch_op.drop_constraint("fk_sesiones_grupo_reported_by_member_id", type_="foreignkey")
