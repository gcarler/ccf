...
def upgrade() -> None:
    ...
    if inspector.has_table("communication_logs") and not _has_column(
        inspector, "communication_logs", "campaign_name"
    ):
...
def downgrade() -> None:
    ...
    if inspector.has_table("communication_logs") and _has_column(
        inspector, "communication_logs", "campaign_name"
    ):
        op.drop_index(
            "ix_communication_logs_campaign_name", table_name="communication_logs"
        )
        with op.batch_alter_table("communication_logs") as batch_op:
