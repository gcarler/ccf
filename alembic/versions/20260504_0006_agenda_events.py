...
def upgrade() -> None:
    ...
    if not inspector.has_table("agenda_events"):
        op.create_table(
            ...
            sa.Column(
                "is_all_day", sa.Boolean(), nullable=False, server_default=sa.true()
            ),
        ...
        op.create_index(
            "ix_agenda_events_title", "agenda_events", ["title"], unique=False
        )
        op.create_index(
            "ix_agenda_events_start_at", "agenda_events", ["start_at"], unique=False
        )
        op.create_index(
            "ix_agenda_events_end_at", "agenda_events", ["end_at"], unique=False
        )
        op.create_index(
            "ix_agenda_events_is_all_day", "agenda_events", ["is_all_day"], unique=False
        )
        op.create_index(
            "ix_agenda_events_created_by_user_id",
            "agenda_events",
            ["created_by_user_id"],
            unique=False,
        )
        op.create_index(
            "ix_agenda_events_created_at", "agenda_events", ["created_at"], unique=False
        )
