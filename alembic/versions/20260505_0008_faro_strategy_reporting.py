...
def upgrade() -> None:
    ...
    if inspector.has_table("grupos_evangelismo") and not _has_column(inspector, "grupos_evangelismo", "code"):
    ...
    if inspector.has_table("sesiones_grupo"):
        with op.batch_alter_table("sesiones_grupo") as batch_op:
            ...
            if not _has_column(inspector, "sesiones_grupo", "cancellation_reason"):
    ...
    if inspector.has_table("asistencias"):
        with op.batch_alter_table("asistencias") as batch_op:
            ...
            if not _has_column(inspector, "asistencias", "absence_reason_detail"):
...
def downgrade() -> None:
    ...
    if inspector.has_table("asistencias") and _has_column(inspector, "asistencias", "absence_reason_detail"):
        op.drop_index(
            "ix_asistencias_absence_reason",
            table_name="asistencias",
        )
        with op.batch_alter_table("asistencias") as batch_op:
            if _has_column(inspector, "asistencias", "absence_reason_detail"):
    ...
    if inspector.has_table("sesiones_grupo") and _has_column(inspector, "sesiones_grupo", "topic"):
        with op.batch_alter_table("sesiones_grupo") as batch_op:
            for column in [
                "reported_at",
                "reported_by_member_id",
    ...
    if inspector.has_table("grupos_evangelismo") and _has_column(inspector, "grupos_evangelismo", "code"):
