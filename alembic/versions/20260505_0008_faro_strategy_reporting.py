...
def upgrade() -> None:
    ...
    if inspector.has_table("glory_houses") and not _has_column(inspector, "glory_houses", "code"):
    ...
    if inspector.has_table("glory_house_sessions"):
        with op.batch_alter_table("glory_house_sessions") as batch_op:
            ...
            if not _has_column(inspector, "glory_house_sessions", "offering_amount"):
                batch_op.add_column(sa.Column("offering_amount", sa.Numeric(12, 2), nullable=True))
            ...
            if not _has_column(inspector, "glory_house_sessions", "novelty_type"):
                batch_op.add_column(sa.Column("novelty_type", sa.String(length=50), nullable=True))
            if not _has_column(inspector, "glory_house_sessions", "novelty_detail"):
                batch_op.add_column(sa.Column("novelty_detail", sa.Text(), nullable=True))
            if not _has_column(inspector, "glory_house_sessions", "cancellation_reason"):
            ...
            if not _has_column(inspector, "glory_house_sessions", "reported_by_member_id"):
                batch_op.add_column(sa.Column("reported_by_member_id", sa.Integer(), nullable=True))
    ...
    if inspector.has_table("glory_house_attendance"):
        with op.batch_alter_table("glory_house_attendance") as batch_op:
            if not _has_column(inspector, "glory_house_attendance", "absence_reason"):
                batch_op.add_column(sa.Column("absence_reason", sa.String(length=50), nullable=True))
            if not _has_column(inspector, "glory_house_attendance", "absence_reason_detail"):
                batch_op.add_column(sa.Column("absence_reason_detail", sa.Text(), nullable=True))
        if not _has_column(inspector, "glory_house_attendance", "absence_reason"):
            op.create_index("ix_glory_house_attendance_absence_reason", "glory_house_attendance", ["absence_reason"], unique=False)
...
def downgrade() -> None:
    ...
    if inspector.has_table("glory_house_attendance") and _has_column(inspector, "glory_house_attendance", "absence_reason_detail"):
        op.drop_index("ix_glory_house_attendance_absence_reason", table_name="glory_house_attendance")
        with op.batch_alter_table("glory_house_attendance") as batch_op:
            if _has_column(inspector, "glory_house_attendance", "absence_reason_detail"):
    ...
    if inspector.has_table("glory_house_sessions") and _has_column(inspector, "glory_house_sessions", "topic"):
        with op.batch_alter_table("glory_house_sessions") as batch_op:
            for column in [
                "reported_at",
                "reported_by_member_id",
