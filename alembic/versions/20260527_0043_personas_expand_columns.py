...
def _personas_id_type() -> str:
    ...
    conn = op.get_bind()
    r = conn.execute(
        sa.text("SELECT data_type FROM information_schema.columns WHERE table_name='personas' AND column_name='id'")
    )
    row = r.fetchone()
...
def _create_persona_engagement_view() -> None:
    ...
    op.execute(
        sa.text("CREATE UNIQUE INDEX IF NOT EXISTS ux_mv_persona_engagement_id ON mv_persona_engagement (persona_id)")
    )
...
def _create_kernel_identity_tables() -> None:
    ...
    if not _table_exists("persona_ministries"):
        op.create_table(
            ...
            sa.Column("recognized_by_id", sa.Integer(), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
