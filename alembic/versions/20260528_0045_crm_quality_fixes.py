...
def _constraint_exists(name: str) -> bool:
    conn = op.get_bind()
    r = conn.execute(
        sa.text("SELECT count(*) FROM information_schema.table_constraints WHERE constraint_name=:n"), {"n": name}
    )
    return r.scalar() > 0
...
def upgrade() -> None:
    ...
    if _col_exists("counseling_tickets", "member_id") and not _col_exists("counseling_tickets", "persona_id"):
    ...
    conn.execute(sa.text("ALTER TABLE donations ALTER COLUMN amount TYPE NUMERIC(14,2) USING amount::NUMERIC(14,2)"))
    conn.execute(
        sa.text("""
    ...
    if _col_exists("donations", "fund_id"):
        conn.execute(sa.text("ALTER TABLE donations DROP CONSTRAINT IF EXISTS donations_fund_id_fkey"))
        try:
            conn.execute(
                sa.text(
    ...
    conn.execute(
        sa.text(
            "ALTER TABLE funds ALTER COLUMN current_balance TYPE NUMERIC(14,2) USING current_balance::NUMERIC(14,2)"
        )
    )
    if _col_exists("funds", "target_amount"):
        conn.execute(
            sa.text(
                "ALTER TABLE funds ALTER COLUMN target_amount TYPE NUMERIC(14,2) USING target_amount::NUMERIC(14,2)"
            )
        )
    if not _constraint_exists("uq_funds_name"):
        conn.execute(sa.text("ALTER TABLE funds ADD CONSTRAINT uq_funds_name UNIQUE (name)"))

    # ── member_roles: rename member_id → persona_id, add UniqueConstraint ───
    if _col_exists("member_roles", "member_id") and not _col_exists("member_roles", "persona_id"):
    ...
    if not _constraint_exists("uq_member_role"):
        ...
        conn.execute(sa.text("ALTER TABLE member_roles ADD CONSTRAINT uq_member_role UNIQUE (persona_id, role_id)"))

    # ── personas: birthday TIMESTAMP → DATE ─────────────────────────────────
    conn.execute(sa.text("ALTER TABLE personas ALTER COLUMN birthday TYPE DATE USING birthday::DATE"))
    # personas: latitud/longitud FLOAT → NUMERIC
    conn.execute(sa.text("ALTER TABLE personas ALTER COLUMN latitud TYPE NUMERIC(10,8) USING latitud::NUMERIC(10,8)"))
    conn.execute(sa.text("ALTER TABLE personas ALTER COLUMN longitud TYPE NUMERIC(11,8) USING longitud::NUMERIC(11,8)"))

    # ── crm_etapas_pipeline: UniqueConstraint(pipeline_id, orden) ───────────
    if not _constraint_exists("uq_etapa_pipeline_orden"):
        conn.execute(
            sa.text(
                "ALTER TABLE crm_etapas_pipeline ADD CONSTRAINT uq_etapa_pipeline_orden UNIQUE (pipeline_id, orden)"
            )
        )

    # ── crm_pipelines: add timestamps ───────────────────────────────────────
    if not _col_exists("crm_pipelines", "created_at"):
        op.add_column("crm_pipelines", sa.Column("created_at", sa.DateTime(), server_default=sa.text("NOW()")))
    if not _col_exists("crm_pipelines", "updated_at"):
        op.add_column("crm_pipelines", sa.Column("updated_at", sa.DateTime(), server_default=sa.text("NOW()")))

    # ── crm_plantillas_mensaje: add created_at ───────────────────────────────
    if not _col_exists("crm_plantillas_mensaje", "created_at"):
        op.add_column("crm_plantillas_mensaje", sa.Column("created_at", sa.DateTime(), server_default=sa.text("NOW()")))
    ...
    conn.execute(sa.text("ALTER TABLE crm_casos ALTER COLUMN sla_vencimiento_contacto DROP NOT NULL"))

    # ── crm_interacciones: add duration_seconds ──────────────────────────────
    if not _col_exists("crm_interacciones", "duration_seconds"):
        op.add_column("crm_interacciones", sa.Column("duration_seconds", sa.Integer(), server_default="0"))

    # ── crm_tareas: add created_at, indexes ──────────────────────────────────
    if not _col_exists("crm_tareas", "created_at"):
        op.add_column("crm_tareas", sa.Column("created_at", sa.DateTime(), server_default=sa.text("NOW()")))
    for idx, tbl, col in [
