...
def final_verify():
    try:
        ...
        cursor.execute("UPDATE users SET role = 'admin', is_active = 1 WHERE username = 'admin_ccf'")
        
        # Verificar cómo quedó
        cursor.execute("SELECT id, username, role, is_active FROM users WHERE username = 'admin_ccf'")
