...
| Servicios | `backend/services/evangelism_crm_bridge.py`, `calculo_sesiones.py` | CRM bridge, generacion de sesiones |
...
**Actualizacion QA 2026-08-21 (email de confirmacion con QR + runbook de produccion):** Cierre del flujo de conversion del aniversario 40 (`/aniversario40`): el CTA del hero quedó conectado al evento con reserva real y se auditaron los 3 eslabones del email. Defectos corregidos en el preregistro: (1) los links del QR caían a `https://ccf.co` (placeholder) porque `Settings` no definía `public_base_url` — ahora el setting existe y la resolución (`_settings_public_base_url` / `resolve_public_base_url`) usa `frontend_url` canónico y **nunca** el placeholder; (2) `resend_confirmation` del admin pasaba `public_base_url=""` (URL relativa) — ahora resuelve la URL base; (3) plantilla del email reescrita con layout corporativo (`render_event_confirmation_email` + `_brand_wrap`) y **QR embebido como imagen** vía el nuevo endpoint público `GET /api/public/events/{id}/qr.png` (hash-bound, dependencia `qrcode` en `requirements.txt`), más botón “Abrir mi código QR” y link de cancelación (72h). Validacion: **70 tests** (11 nuevos en `tests/test_event_registration_email.py`), **SMTP AUTH 235 OK** en vivo, y **smoke E2E completo** (registro → `/ticket` 200 → `/qr.png` 200 image/png → `ccf-evt-checkin` → `CHECKED_IN` + `role_at_event` → idempotencia → limpieza) documentado en `docs/RUNBOOK_PRODUCCION.md` §7 (nueva sección con endpoints, config `.env`, SQL de cupos y troubleshooting).
...
**Actualizacion QA 2026-07-21 (cierre Fase 1 RBAC — wrapper antiguo):** Se completo el ultimo eslabon antiguo de la migracion RBAC radical al eliminar el paquete `backend/api/_evangelism_helpers/` (codigo muerto desde la migracion del 2026-07-17). El wrapper `require_pastor_or_admin_with_sede` era la unica referencia evangelism al guard `crm:manage`-coincidence; ningun router lo importaba, funciones equivalentes viven en `backend/crud/evangelism.py:_actor_sede_or_none_evangelismo`. Modulo evangelism ahora verificadamente 100% libre de `require_pastor_or_admin` (grep 0 hits). Comentario referenciante en `backend/crud/evangelism.py` actualizado. Bug secundario corregido: 5 tests usaban campo obsoleto `nombre` en PUT/POST `/grupos` cuando el schema `GrupoEvangelismoCreate/Update` con `extra="forbid"` exige `name`. Validacion fresca: smoke 2/2 (19 passed + 1 xfailed) y suite amplia 226/226 verdes. Commit `339539e9`.

**Actualizacion QA 2026-08-02 (cierre 100% — 7 brechas follow-up + deuda tecnica):** Cierre formal de las 7 brechas de calidad follow-up (`NUEVO-FOLLOWUP-PENDING-FIXES`) que requerian migracion DB agrupada, mas la deuda tecnica eliminada en la sesion `ses_0442c5c6effeUqcqRILFB1Augx`:
- **#3 endpoint DELETE follow-up (soft-delete)** — cerrada `b1f32287`. Endpoint `DELETE /follow-up/{seguimiento_id}` con `require_evangelism_manage`, retorna `{"ok": True}`.
- **#4 default `estado_completado` pendiente** — cerrada `75ce8544`. Migration `alembic/canonical_versions/20260724_0001_seguimiento_integrity.py` cambia default a pendiente.
- **#5 enum CHECK en `tipo`** — cerrada `75ce8544` (misma migracion). `TipoSeguimientoEnum` con CHECK constraint.
- **#6 indices FK en seguimiento** — cerrada `75ce8544` (misma migracion). Indices en FKs `asistencia_id`/`lider_id` aceleran joins.
- **#7 schema `RegistroSeguimientoResponse` con `extra="forbid"`** — cerrada `c7458733`.
- **#10 UI scoping del panel de seguimientos por estrategia** — cerrada `530d6892`. Panel en `strategies/[id]/page.tsx` ahora filtra por estrategia.
- **#12 regression tests cross-sede + update-sobre-soft-deleted** — cerrada `1c26c739`. `tests/test_evangelism_followup_sede_regression.py` (6 tests).
Adicionalmente (sesion `ses_0442c5c6effeUqcqRILFB1Augx`, commit `f75753c7`): eliminacion de 5 funciones helper muertas duplicadas de `evangelism_shared.py` (~128 LOC, copias canonicas en `crm/_shared.py` y `main_utils.py`), eliminacion de `evangelism_projection.py` (wrapper muerto 0% cobertura, sin importadores), y 117 tests nuevos en `tests/test_evangelism_coverages_gaps.py` (events_main 76->95%, main_estrategias 83->90%, grupos_main 87->93%).
**Suite evangelismo end-to-end re-ejecutada 2026-08-02:** `1177 passed, 0 failed, 2 xfailed, 1 xpassed` en 12:33 (1180 colectados). Cero fallos. Backlog activo del modulo: **vacio**.
...
12. **Permisos runtime UI** `[PARCIAL-RUNTIME-AUTH-001]` — cierre secundario el `2026-07-21` (entrada 6 de esta lista). La homologacion inicial del `2026-07-17` se completa ahora con la eliminacion del wrapper antiguo.
...
| `PARCIAL-RUNTIME-AUTH-001` | Cerrada el 2026-07-17 (migracion RBAC) + 2026-07-21 (wrapper antiguo). Migracion RBAC completa: guards evangelism:* en todos los routers + eliminacion del wrapper antiguo `require_pastor_or_admin_with_sede` via borrado del paquete muerto `backend/api/_evangelism_helpers/`. | 14 archivos backend + `backend/core/permissions.py` + `backend/api/_evangelism_helpers/` (eliminado) |
