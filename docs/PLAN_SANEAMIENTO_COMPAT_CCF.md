# Plan de Saneamiento de Deuda Compat CCF

**Fecha:** 2026-06-05
**Estado:** Plan maestro operativo
**Base obligatoria:** `docs/PROTOCOLO_CAMBIOS_SEGUROS_CCF.md`
**Objetivo:** retirar la deuda compat protegida por lotes seguros hasta dejar la plataforma alineada con UUID/personas sin romper contratos vivos.
...
5. tests y arranque;
6. deprecatacion documentada;
7. retiro del compat solo cuando no tenga consumidores.
...
- Debe quedar solo como contrato compat de autenticacion v1.
...
- mantener `/api/auth/users/{id}` solo para compatibilidad compat hasta retiro formal.
...
- CRUD compat usa `user_id:int` en `backend/crud/academy.py`.

Objetivo:

- agregar contratos por persona/auth UUID;
- migrar consumidores;
- dejar rutas compat con aviso de deprecatacion;
...
- conservar wrapper compat temporal si algun modulo requiere entero.

### D4. FKs/columnas a `users.id`

Estado:

- Persisten en CMS, Academy compat, Identity compat, Agents, Governance, CRM y servicios.
- Algunas representan autoria/auditoria; otras representan identidad ministerial real.

Objetivo:

- no migrar en bloque;
- clasificar cada columna como `auth_compat`, `actor_persona`, `author_persona`, `catalog_owner` o `compat_only`;
- crear columnas UUID paralelas solo donde representen persona/actor real.

### D5. PK enteras en tablas transaccionales y modulos compat
...
### Lote 1: Cierre De Superficie Admin/Auth Compat
...
3. Agregar tests de contrato:
   ...
   - `/api/auth/users/{int}` sigue vivo como compat protegido.
...
2. Agregar helpers:
   ...
   - resolver compat `users.id` solo para compatibilidad temporal.
...
5. Mantener rutas `/api/academy/users/{user_id}/...` con resolucion dual y header/log de deprecatacion.
6. Agregar tests de equivalencia: compat y nuevo devuelven el mismo resultado para el mismo usuario vinculado.

Criterio de salida:

- ningun consumidor frontend usa `/academy/users/{user_id}/enrollments` ni `/progress`;
- rutas nuevas UUID tienen tests;
- rutas compat quedan documentadas para retiro.
...
4. Dejar `UserSelectCompat` para modulos que todavia usen `users.id`.
...
- `UserSelect` compat queda sin consumidores nuevos;
...
5. Identity compat:
   - refresh/reset/verification tokens pueden permanecer auth compat hasta retirar auth v1.
...
8. retirar columna compat en migracion posterior.
...
1. Agregar logs de uso a endpoints compat.
...
5. Eliminar columnas compat solo despues de backup y rollback probado.
...
- `test_compat_allowlist_has_owner_and_lote`

### Contrato API

- Admin users UUID CRUD.
- Academy persona enrollments/progress equivalentes a compat.
- User/persona selector endpoints devuelven IDs UUID.
- Compat endpoints devuelven header de deprecatacion.
...
| Patron | Razon | Lote |
|---|---|---|
| `backend/api/auth.py` `/users/{user_id}` | Auth v1 compat | Lote 1 / retiro Lote 6 |
...
| `backend/models_identity.py users.id` | Tokens/preferencias auth compat | Lote 4 o retiro auth v1 |
...
3. Crear endpoints Academy UUID paralelos sin tocar compat.
