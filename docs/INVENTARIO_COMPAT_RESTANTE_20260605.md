# Inventario de Deuda Compat Restante
...
- Academy compat ahora resuelve `persona_id` desde `users.id` y hace dual-write runtime en:
...
## 2. Compat Backend Que Permanece Por Compatibilidad
...
- Academy retiro final de escritura compat tras backfill validado.

### CRM compat con UUID paralelo
...
- El runtime ya hace dual-write cuando recibe identidad compat:
...
- CRM retiro final de escritura/lectura compat tras backfill validado.
...
- CMS retiro final de autoria compat tras backfill validado.
...
- Retiro final de columnas actor/owner compat tras backfill validado.
...
- tablas compat protegidas mientras exista consumidor.
...
2. Mantener columnas compat `users.id` durante un release sin consumidores compat.
3. Retirar escrituras compat por dominio.
4. Luego retirar columnas compat con migraciones de downgrade definido.
...
- un release sin consumidores compat.
