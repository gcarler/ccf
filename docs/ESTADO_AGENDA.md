...
- Superficie backend: **1 032 LOC** en 4 archivos (`api/agenda.py`, `crud/agenda.py`, `models_agenda.py`, `schemas/agenda.py`) + **274 LOC** en `system.py` (`get_global_calendar`). Total: **1 306 LOC** (23 endpoints). Actualizado el **2026-09-07**: +`services/agenda_recurrence.py` (expansión RRULE, +131 LOC) y `tests/test_agenda_recurrence.py` (+14 tests).
...
10. **Eventos recurrentes RFC 5545** `[PEND-AGENDA-RRULE-001]` — cerrada el **2026-09-07**. Wire completo de las columnas RRULE que existían sin uso en `EventoAgenda`: `services/agenda_recurrence.py` (validación + expansión con techos `MAX_OCCURRENCES_PER_SERIES=366` y lookback 366 días), exposición en `AgendaEventCreate`, expansión en `GET /agenda/events/by-date-range` (ocurrencias con `recurrence_id`), expansión acotada en el agregador calendar, selector de repetición + badge de serie en `agenda/events/page.tsx`, semántica PUT de preservación/eliminación de serie, y suite adversarial `tests/test_agenda_recurrence.py` (14 tests: validación, expansión, excepciones, techo COUNT, aislamiento por sede, contrato del agregador). La edición por ocurrencia individual queda explícitamente fuera del alcance v1.
11. **Edición por ocurrencia de series (v2)** `[PEND-AGENDA-RRULE-OCCURRENCE-001]` — cerrada el **2026-09-07**. Semántica RFC 5545 "sólo esta ocurrencia": `PUT`/`DELETE /agenda/events/{id}?occurrence_date=YYYY-MM-DD` con validación de ocurrencia vigente vía la misma expansión de lectura (`occurrence_start_for`), excepciones deduplicadas (`add_exception`), evento derivado puntual con proveniencia `entidad_origen_id = "serie:<id>:<fecha>"` expuesta como `derived_from` (sin migración), href del agregador con `?occurrence=` y UI de alcance ("Sólo esta ocurrencia" / "Toda la serie") en el detalle de evento. Incluye el fix de un bug latente: el orden de `by-date-range` comparaba datetimes naive (SQLite/tests) con aware (expansión). Suite ampliada a **22 tests**.
